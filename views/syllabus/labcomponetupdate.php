         ...
         if(count($cheklabdata)>0){
           ...
           foreach ($cheklabdata as $value) 
            { 
               ...
               $experiment_title=str_replace("<br />", " ", $value['experiment_title']);

               $fuction="getcorpt(".$i.")";?> 
        <div class="col-xs-12 col-sm-12 col-lg-12">
            <div class="col-md-6">
                <input type="hidden" name="experiment_id[]"  value="<?php echo $value["cur_labcomp_id"];?>">
                <?= $form->field($labmodel, 'experiment_title')->textarea(['Autocomplete'=>"off",'name'=>'experiment_title[]','value'=>$experiment_title]) ?>
