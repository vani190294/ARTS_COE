...
$this->title = " VALUE ADDED SUBJECT CIA MARK LIST";
$this->params['breadcrumbs'][] = ['label' => 'CIAMarkList', 'url' => ['create']];
    ...
    <div class="col-xs-12 col-sm-12 col-lg-12">
        ...
        <div class="col-xs-12 col-sm-12 col-lg-12">
            ...
            <div class="form-group col-lg-3 col-sm-3">
                ...
                <?= Html::a("Reset", Url::toRoute(['coe-value-mark-entry/ciamarklist']), ['onClick'=>"spinner();",'class' => 'btn btn-group btn-group-lg btn-warning ']) ?>
            </div>
        </div>
    ...
    if($file_content_available=="Yes")
        {
            ...
            $stu_print_vals = 0;
                ...
                foreach ($cia_list as $get_names) 
                {                    
                    $degree_name = $get_names['degree_name'];
                    $prg_name = $get_names['programme_name'];                    
                    break;
                }
               $month = Categorytype::findOne($_POST['month']);
               $exam_type = Categorytype::findOne($_POST['mark_type']);
               $header .='<div class="box-body table-responsive"><table class="table" border="1" cellpadding="1" align="center" cellspacing="1">
                ...
                $header .='<td colspan='.($countVal+1).'  ALIGN="CENTER">
                  ...
                  <tr>';
                    $header .='<td colspan='.($countVal+1).' ALIGN="CENTER">
                        CONSOLIDATED CIA MARKS FOR THE END SEMESTER EXAMINATIONS '.$_POST["mark_year"].' / '.$month->description.'
                    </td>
                  </tr>
                  <tr>';
                    $header .='<td colspan='.($countVal+1).' ALIGN="CENTER">
                   '.$degree_name.' - '.$prg_name.' 
                    ...
                    <tr>
                        <td align="center"> <b>'.strtoupper(ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_SUBJECT)).' CODE </td></b>';
                        $header .='<td colspan='.($countVal-1).' align="center"> <b>'.strtoupper(ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_SUBJECT)).' NAME</td>
                  ...
                  { 
                    ...
                    foreach($cia_list as $rowsstudent) 
                    { 
                ...
                echo Html::a('<i class="fa fa-file-pdf-o"></i> ' . "PDF",array('cia-mark-list-pdf','exportPDF'=>'PDF'),array('title'=>'Export to PDF','target'=>'_blank','class'=>'pull-right btn btn-warning', 'style'=>'color:#fff'));
                echo Html::a('<i class="fa fa-file-excel-o"></i> ' . "EXCEL ",array('excel-cia-mark-list','exportPDF'=>'PDF'),array('title'=>'Export to Excel','target'=>'_blank','class'=>'pull-right btn btn-info', 'style'=>'color:#fff')); 
