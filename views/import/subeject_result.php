            ...
            <section class="col-lg-12 connectedSortable">
              ...
              <?php 
                ...
                <?php if(!empty($importResults['totalSuccess'])) : ?>
                    <div class="alert alert-success">
                        ...
                        <?= "{$importResults['totalSuccess']} ". ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_SUBJECT).'  importing successfully.' ?>
                    </div>
                <?php endif; ?>
              ...
              <!-- /. tools -->
                ...
                <?php
                    ...
                    $headerTr.= Html::tag('th', ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_SUBJECT).' Name');
                    $headerTr.= Html::tag('th', ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_PAPER_TYPE));
                    $headerTr.= Html::tag('th', ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_SUBJECT_TYPE));
                    $headerTr.= Html::tag('th', ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_PROGRAMME_TYPE));
                    $headerTr.= Html::tag('th', 'Total min Pass');
                    $headerTr.= Html::tag('th', 'Total Max');
                    $headerTr.= Html::tag('th', ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_SUBJECT).' Fee');
                    $headerTr.= Html::tag('th', 'Status');
                    $headerTr.= Html::tag('th', 'Message');
                ?>
                <table style="overflow-x:auto;"  class="table table-bordered table-responsive bulk_edit_table table-hover"  id="subject_import_results">
                    <thead>
                    ...
                    foreach($importResults['dispResults'] as $line) {                       
                        ...
                        $content.= Html::tag('td', isset($line['Q'])?$line['Q']:"");
                        $content.= Html::tag('td', isset($line['R'])?$line['R']:""); 
                        $content.= Html::tag('td', isset($line['N'])?$line['N']:"");
                        $content.= Html::tag('td', isset($line['O'])?$line['O']:"");
                        $content.= Html::tag('td', isset($line['T'])?$line['T']:"");
...
$this->registerJs(<<<JS
    $(function () {
    $('#subject_import_results').DataTable({
      'paging'      : true,
      'lengthChange': false,
      'searching'   : false,
      'ordering'    : true,
