...
<div class="box box-success">
	<div class="box-header">
		<h3 class="box-title"><i class="fa fa-list-ul"></i> <?php echo ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_STUDENT).' DOB UPDATE RESULTS'; ?></h3>
              ...
              <?php 
				...
				<?php if(!empty($importResults['totalSuccess'])) : ?>
					<div class="alert alert-success">
						...
						<?= "{$importResults['totalSuccess']} ". ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_STUDENT).' DOB Updated successfully.' ?>
					</div>
				<?php endif; ?>
            	...
            	<?php
					...
					$headerTr.= Html::tag('th', 'Parent / Guardian Mobile Number');
					$headerTr.= Html::tag('th', 'Email id');
					$headerTr.= Html::tag('th', 'Student mobile number');
					$headerTr.= Html::tag('th', 'Aadhar Number');
					$headerTr.= Html::tag('th', 'Religion');
					$headerTr.= Html::tag('th', 'Blood group');
					$headerTr.= Html::tag('th', 'Caste');
					$headerTr.= Html::tag('th', 'Sub Caste');
					$headerTr.= Html::tag('th', 'Nationality');
					$headerTr.= Html::tag('th', 'Guardian Name');
					$headerTr.= Html::tag('th', 'Status');
					$headerTr.= Html::tag('th', 'Message');
				?>
				<table style="overflow-x:auto;"  cellspacing="0" cellpadding="0" border="0"  class="table table-bordered table-responsive bulk_edit_table table-hover" id = "student_import_results">
					<thead>
					...
					foreach($importResults['dispResults'] as $line) {
						...
						$content.= Html::tag('td', isset($line['B'])?date("d-m-Y", PHPExcel_Shared_Date::ExcelToPHP($line['B'])):""); 
						...
						$content.= Html::tag('td', isset($line['M'])?$line['M']:"");
