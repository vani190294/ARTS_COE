...
<?php Yii::$app->ShowFlashMessages->showFlashes(); 
    ...
    for ($i=1; $i <= $max_parts['part_no'] ; $i++) 
    {
       ...
       $part_5 = 'SELECT A.grade_name FROM coe_mark_entry_master as A JOIN coe_subjects_mapping as B ON B.coe_subjects_mapping_id=A.subject_map_id JOIN coe_student_mapping as C ON C.coe_student_mapping_id=A.student_map_id JOIN coe_subjects as D ON D.coe_subjects_id=B.subject_id   and coe_subjects_mapping_id NOT IN (11697,11698) WHERE A.student_map_id="'.$prev_stu_map_id.'"  and B.semester=6 and D.part_no=5';
        $part_5_details = Yii::$app->db->createCommand($part_5)->queryOne();


        /*if($i==3 && $i==1 || $i==2 && $part_info['part_class']="FIRST CLASS WITH DISTINCTION" )
         {
        

$part_info['part_class']="FIRST CLASS";
    ...
    {
$part_info['part_class'];

      }*/
      if(($temp_part1=='SECOND CLASS' || $temp_part2=='SECOND CLASS') && $i==3 && $part_info['part_class']=='FIRST CLASS WITH DISTINCTION')
      {
       ...
       else if($i==5)
      {
       ...
       $classification = $part_5_details['grade_name'];
      }
      else
      {
      ...
      <tr>
        ...
        <td valign='top'   class='line_height print_cgpa_size print_class' colspan=6 width='200px' style='padding-left:-100px;' > ".$classification." </td>
        <td valign='top'   class='line_height print_cgpa_size' colspan=4 width='60px;' style='padding-left:-50px;' >".$part_info['part_additional_cred']." </td>                                                             
    </tr>
      </table>";
     // print_r($part_info);exit;
      if($i==1)
      {
        $temp_part1=$part_info['part_class'];
      }
      
       if($i==2)
      {
        $temp_part2=$part_info['part_class'];
      }
        ...
        $header.='<td valign="top"   height="150px" colspan="16">'; //first td
          $header.='<table class="makeitBigger" width="100%">
                    <tr>
                        <td valign="top" colspan=3 align="left" width="100" style="padding-top: 18px;">'.strtoupper($value["degree_code"]).'  </td>
                        <td valign="top" colspan=7 align="left" width="320" style="padding-top: 20px;" >'.strtoupper($value["programme_name"]).'</td>
                        <td valign="top" colspan=3 align="right" width="100" style="padding-top: 17px;padding-right:40;" >'.strtoupper($show_date_publication).'</td>
                       

                          <td valign="top" colspan=3  align="center" width="200" style="padding-top: 20px;padding-right:180px;" >'.$getBatchDetails['batch_name'].'-'.($getBatchDetails['batch_name']+$getDegree['degree_total_years']).'</td>
                                </tr>
                              <tr><td valign="top"  colspan="16" height="40px" >&nbsp;</td></tr>
                    <tr>
                        <td valign="top" class="student_name"  colspan="10">'.strtoupper($value["name"]).'</td>
                        <td valign="top"  colspan=3  class="dob" align="right" width="100"  >'.$dob.'</td>

                          <td valign="top" colspan=3  align="center" width="200" style="padding-top: 3px;padding-right:180px;" >'.strtoupper($value["register_number"]).'</td>
                    </tr>
              ...
              $classification = $value['grade_name'];
                ...
                <table style='line-height: 1.2em; ' border='0' width='100%'  >
                    <tr>
                        ...
                        <td valign='top' width='95px' class='year'  ><b>" . strtoupper($year_of_passing) . "</b></td>
                    </tr>
                </table>";
            $total_sub_count++;$stu_subjects++;
           } // ZERO ZERO SUBJECTS
...
//print_r($stu_subjects);exit;
    ...
    $max_parts = SubInfo::find()->where(['sub_batch_id'=>$batch_maping_id])->orderBy('part_no desc')->limit(1)->one();
    //$max_parts = SubInfo::find()->where(['sub_code'=>'19ncu01g'])->orderBy('part_no desc')->limit(1)->one();

    //print_r($max_parts);exit;

    $temp_part1=''; $temp_part2='';
    for ($i=1; $i <= $max_parts['part_no'] ; $i++) 
    {
       $part_info = ConfigUtilities::getPartDetails($previous_reg_number,$i);
         //print_r($previous_reg_number);exit;
