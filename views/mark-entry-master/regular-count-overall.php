            ...
            $stu_print_vals = 0;
               ...
               $header .="<table border=1 align='center' class='table table-striped '>";
                  ...
                  { 
                        ...
                        $getFailCount = Yii::$app->db->createCommand('select count(DISTINCT student_map_id) FROM coe_mark_entry_master as A JOIN coe_subjects_mapping as B ON B.coe_subjects_mapping_id=A.subject_map_id JOIN coe_subjects as C ON C.coe_subjects_id=B.subject_id where B.subject_id="'.$rows["coe_subjects_id"].'" and C.subject_code="'.$rows["subject_code"].'"  and A.year ="'.$rows["year"].'" and A.month ="'.$rows["month"].'" and A.mark_type=27 and A.result like "%Fail%" and A.year_of_passing="" ')->queryScalar();

                        $getPassCount = Yii::$app->db->createCommand('select count(DISTINCT student_map_id) FROM coe_mark_entry_master as A JOIN coe_subjects_mapping as B ON B.coe_subjects_mapping_id=A.subject_map_id JOIN coe_subjects as C ON C.coe_subjects_id=B.subject_id where B.subject_id="'.$rows["coe_subjects_id"].'" and C.subject_code="'.$rows["subject_code"].'"  and A.year ="'.$rows["year"].'" and A.month ="'.$rows["month"].'" and A.mark_type=27 and A.result like "%Pass%"')->queryScalar();

                        $getTotalCount = Yii::$app->db->createCommand('select count(DISTINCT register_number) FROM coe_hall_allocate as A JOIN coe_exam_timetable as E ON E.coe_exam_timetable_id=A.exam_timetable_id and E.exam_year=A.year and E.exam_month=A.month JOIN coe_subjects_mapping as B ON B.coe_subjects_mapping_id=E.subject_mapping_id JOIN coe_subjects as C ON C.coe_subjects_id=B.subject_id where B.subject_id="'.$rows["coe_subjects_id"].'" and C.subject_code="'.$rows["subject_code"].'"  and A.year ="'.$rows["year"].'" and A.month ="'.$rows["month"].'" and E.exam_type=27 and E.exam_year ="'.$rows["year"].'" and E.exam_month ="'.$rows["month"].'" ')->queryScalar();
