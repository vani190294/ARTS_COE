...
$this->title="REVALUATION REPORTS";
    ...
    <div class="col-xs-12 col-sm-12 col-lg-12">
        <div class="col-xs-12 col-sm-12 col-lg-12">
            ...
            <div class="btn-group col-lg-3 col-sm-3" role="group" aria-label="Actions to be Perform"> <br />
                ...
                <?= Html::a("Reset", Url::toRoute(['mark-entry-master/revaluation-reports']), ['onClick'=>"spinner();",'class' => 'btn btn-group btn-group-lg btn-warning ']) ?>
            </div>
    ...
    <?php 
        ...
        if(isset($fetched_data))
        {
            ...
            echo '
                ...
                echo Html::a('<i class="fa fa-file-pdf-o"></i> ' . "PDF",array('revaluation-reports-pdf','exportPDF'=>'PDF'),array('title'=>'Export to PDF','target'=>'_blank','class'=>'pull-right btn btn-warning', 'style'=>'color:#fff'));
                echo Html::a('<i class="fa fa-file-excel-o"></i> ' . "EXCEL ",array('revaluation-reports-excel','exportPDF'=>'PDF'),array('title'=>'Export to Excel','target'=>'_blank','class'=>'pull-right btn btn-info', 'style'=>'color:#fff')); 
                echo '
                            </div>
                        </div>
                      ';
$header .='<table style="overflow-x:auto;"  cellspacing="0" cellpadding="0" border="0"  class="table table-bordered table-responsive bulk_edit_table table-hover" id = "student_import_results">';            
            ...
            $month_name = Categorytype::findOne($_POST['month']);
            $header .=' <tr>
                      <th colspan=9 align="center"><h2>
                        REVALUATION REPORT FOR '.strtoupper($month_name->description).' '.$_POST['MarkEntryMaster']['year'].'</h2>
                      </th>
                    </tr>
            <tr>
                <th>SNO</th>
            ...
            $body .="<tbody>";
                ...
                $sno=1;
                    ...
                    {   
                        ...
                        $body .= "<td>".$values['category_type_id_marks']."</td>";
                        $body .= "</tr>"; $sno++;
                    }
                $body .='</tbody></table>';
                $html = $header.$body;
                $html_1 = $html;
                if(isset($_SESSION['REVALUATION_report_sem']))
                {
                    unset($_SESSION['REVALUATION_report_sem']);
                }
                $_SESSION['REVALUATION_report_sem'] = $html_1;
