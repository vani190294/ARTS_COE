...
$this->title = "PRACTICAL ESE MARK LIST";
    ...
    <div class="col-xs-12 col-sm-12 col-lg-12">
        ...
        <div class="col-xs-12 col-sm-12 col-lg-12">
            ...
            <div class="form-group col-lg-3 col-sm-3">
                ...
                <?= Html::a("Reset", Url::toRoute(['practical-mark-entry/esemarklist']), ['onClick'=>"spinner();",'class' => 'btn btn-group btn-group-lg btn-warning ']) ?>
            </div>
        </div>
            ...
            $stu_print_vals = 0;
               ...
               $header .='<div class="box-body table-responsive"><table class="table" border="1" cellpadding="1" align="center" cellspacing="1">
                ...
                $header .='<td colspan='.($countVal+1).'  ALIGN="CENTER">
                  ...
                  <tr>';
                    $header .='<td colspan='.($countVal+1).' ALIGN="CENTER">
                       PRACTICAL CONSOLIDATED ESE MARKS FOR THE END SEMESTER EXAMINATIONS '.$_POST["mark_year"].' / '.$month.'
                    </td>
                  </tr>
                  <tr>';
                    $header .='<td colspan='.($countVal+1).' ALIGN="CENTER">
                   '.$degree_name.' - '.$prg_name.' 
                    ...
                    <tr>
                        ...
                        $header .='<td colspan='.($countVal-1).' align="center"><b>'.strtoupper(ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_SUBJECT)).' NAME</b></td>
                  ...
                  { 
                         $header .='<tr>
                             <td align="center"><b>
                                '.$rows["subject_code"].'</b></td>
                             <td colspan='.($countVal-1).' align="center"><b>
                                '.$rows["subject_name"].'</b></td>
                    ...
                    { 
                        ...
                        { 
                          ...
                          foreach($ese_list as $stus) 
                          {
                            ...
                            if($subs['subject_code']==$stus['subject_code'] && $rowsstudent['register_number']==$stus['register_number'] && ($subs['ESE_max']==0 || $subs['ESE_max']=='') )
                            {        
                              $ese="-";
                            }
                            else if($subs['subject_code']==$stus['subject_code'] && $rowsstudent['register_number']==$stus['register_number'])
                            {                                     
                                
                             $ab_stus = Yii::$app->db->createCommand("select absent_student_reg from coe_absent_entry where absent_student_reg = '".$rowsstudent['coe_student_mapping_id']."' AND exam_year='".$rowsstudent['year']."' AND exam_month='".$rowsstudent['exam_month']."' AND exam_subject_id='".$stus['coe_subjects_mapping_id']."'")->queryScalar();
                            if(!empty($ab_stus))
                            {
                                $ese="<span style='color: #F00' >AB</span>";
                            }
                            else
                            {
                                $ese=$stus['out_of_100'];
                ...
                $header .='</table></div>';
                if(isset($_SESSION['ese_mark_list'])){ unset($_SESSION['ese_mark_list']);}
                $_SESSION['ese_mark_list'] = $header;
                ...
                echo Html::a('<i class="fa fa-file-pdf-o"></i> ' . "PDF",array('practicalesemarklist-pdf','exportPDF'=>'PDF'),array('title'=>'Export to PDF','target'=>'_blank','class'=>'pull-right btn btn-warning', 'style'=>'color:#fff'));
                echo Html::a('<i class="fa fa-file-excel-o"></i> ' . "EXCEL ",array('practicalesemarklist-excel','exportPDF'=>'PDF'),array('title'=>'Export to Excel','target'=>'_blank','class'=>'pull-right btn btn-info', 'style'=>'color:#fff')); 
