...
$this->registerCssFile("@web/css/newmarkstatement_ug_2021.css");
?>
   ...
   
{
    ...
    foreach ($mark_statement as $value) 
    {
        ...
        if($value['batch_name']>='2022'  && ($value['programme_code']=='U109' || $value['programme_code']=='U111'))
        {
        ...
        if ($previous_reg_number != $value['register_number']) 
        {
            ...
            $cgpa_calc = ConfigUtilities::getCgpaCaluclation_HM($exam_year,$app_month,$batch_mapping_id,$value['student_map_id'],$semester_number);
            $prev_add_pint_code = '';
    ...
    $final_total_credits =array_filter(array()); 
             ...
             $check_add = Yii::$app->db->createCommand('SELECT * FROM coe_additional_credits WHERE exam_year="'.$exam_year.'" AND exam_month="'.$add_month.'" and student_map_id="'.$print_student_map_id.'" and result like "%pass%"')->queryAll();
              ...
              //print_r('SELECT * FROM coe_additional_credits WHERE exam_year="'.$exam_year.'" AND exam_month="'.$add_month.'" and student_map_id="'.$print_student_map_id.'" and result like "%pass%"'); exit;
               ...
               if(!empty($check_arrear) && $year==2020 && $month==29)
                ...
                $check_add_count = Yii::$app->db->createCommand('SELECT count(student_map_id) FROM coe_mandatory_stu_marks WHERE year="'.$exam_year.'" AND month="'.$add_month.'" and student_map_id="'.$print_student_map_id.'" and year_of_passing!=""')->queryScalar();

                $hm_subject = Yii::$app->db->createCommand('SELECT A.*,C.* FROM  coe_subjects as A join coe_subjects_mapping as B  on B.subject_id=A.coe_subjects_id join coe_mark_entry_master as C on C.subject_map_id=B.coe_subjects_mapping_id JOIN cur_honours_subject_list D ON D.subject_code=A.subject_code AND B.semester=D.semester AND B.batch_mapping_id=D.batch_map_id WHERE D.register_number="'.$previous_reg_number.'" AND year="'.$exam_year.'" AND month="'.$add_month.'" and student_map_id="'.$print_student_map_id.'" and mark_type="27" AND year_of_passing!=""')->queryAll();
                ...
                if(!empty($hm_subject))
                {
                    $body .= "
                        <table style='line-height: 1.2em; ' border='0' width='100%' >
                            <tr>    
                                <td valign='top' colspan=2  width='107px'  align='right'>&nbsp;</td>                        
                                <td valign='top' width='450px' class='subjectname' colspan='5'  align='left'><b> ADDITIONAL COURSES #</b></td>
                                <td valign='top' width='52px'  align='left'>&nbsp;</td>
                                <td valign='top' width='45px'  align='left'> &nbsp;</td>
                                <td valign='top' width='55px'  align='center'>&nbsp;</td>
                                <td valign='top'  style='padding-left: 10px;' width='50px'  align='center'><b>&nbsp;</b></td>
                    ...
                    foreach ($hm_subject as $valuess) 
                    {
                        ...
                        <table border='0' width='100%' >
                            <tr>
                                <td width='40px' align='left' class='sem'><b>" . $semester_array[$semester_number]. "</b></td>
                                ...
                                <td width='52px'  class='credits' align='left'><b>" . $valuess['credit_points'] . "</b></td>
                    ...
                    if(empty($check_add) && empty($check_add_count))
                    {
                ...
                $mand_subject = Yii::$app->db->createCommand('SELECT A.*,C.* FROM  coe_mandatory_subjects as A join coe_mandatory_subcat_subjects as B on B.man_subject_id=A.coe_mandatory_subjects_id join coe_mandatory_stu_marks as C on C.subject_map_id=B.coe_mandatory_subcat_subjects_id WHERE year="'.$exam_year.'" AND month="'.$add_month.'" and student_map_id="'.$print_student_map_id.'" and mark_type="27" AND year_of_passing!=""')->queryAll();
                ...
                if(!empty($mand_subject))
                {
                    ...
                    foreach ($mand_subject as $valuess) 
                    {
                        $grade_point_print = $valuess['grade_point']==0?"-":$valuess['grade_point'];
                        $result_stu = $valuess['result'] == "PASS" || $valuess['result'] == "pass" || $valuess['result'] == "Pass" ? "PASS" : $valuess['result'];
