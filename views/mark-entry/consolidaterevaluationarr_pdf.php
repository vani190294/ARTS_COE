...
<?php
    ...
    if($report_type==1)
    {
        if($withar==1)
        {
            $reporttype=strtoupper(' <center> Consolidate Revaluation Programme Wise '.$month_name . '-' . $exam_year.' with Regular </center>');

        }
        else
        {
            $reporttype=strtoupper(' <center> Consolidate Revaluation Programme Wise '.$month_name . '-' . $exam_year.' Arrear</center>');
    ...
    {
        $reporttype=strtoupper(' <center> Consolidate Revaluation Course Wise '.$month_name. '-' . $exam_year.'</center>');
        $colspan=2;
    }
    ...
    $data.='<tr>
            ...
            $notIn = array_filter(['']);
            foreach ($batchdata as $key => $fails) 
            {
               
                $notIn[$fails['coe_batch_id']]=$fails['coe_batch_id'];
            ...
            if(!empty($notIn))
            {
                $implode = " AND coe_batch_id IN (".implode(',', $notIn).")";
                $implode1 = " AND coe_batch_id NOT IN (".implode(',', $notIn).")";
            }
            ...
            if($report_type==1)
            {
              ...
              {               
                ...
                $data.='<tr>
                        <th>SNO</th>     
                        <th>PROGRAMME</th>
                        <th>No Of STUDENT (Regular Student)</th>
                        <th>No Of SCRIPT (Regular Student)</th>
                        <th>No Of STUDENT Changes</th>
                        <th>No Of SCRIPT Changes</th>
                        <th>No Of STUDENT (PassedOut)</th>
                        <th>No Of SCRIPT (PassedOut)</th>
                        <th>No Of STUDENT Changes</th>
                        <th>No Of SCRIPT Changes</th>
                       ';
                ...
                foreach($revaluation as $revaluation1)
                {
                    ...
                    JOIN coe_bat_deg_reg as G ON G.coe_bat_deg_reg_id=B.batch_mapping_id 
                     ...
                     WHERE A.year='" .  $exam_year . "' and A.month='" . $exam_month . "' AND H.programme_code='".$revaluation1['programme_code']."'  and A.reval_status='YES' ".$implode.$arr)->queryScalar();
                    ...
                    JOIN coe_bat_deg_reg as G ON G.coe_bat_deg_reg_id=B.batch_mapping_id 
                     JOIN coe_programme as H ON H.coe_programme_id=G.coe_programme_id
                     WHERE A.year='" .  $exam_year . "' and A.month='" . $exam_month . "' AND H.programme_code='".$revaluation1['programme_code']."'  and A.reval_status='YES' AND A.mark_type=28 ".$implode1)->queryScalar();
                    ...
                    JOIN coe_bat_deg_reg as G ON G.coe_bat_deg_reg_id=B.batch_mapping_id 
                     ...
                     WHERE D.grade_name!=E.grade_name AND A.year='" .  $exam_year . "' and A.month='" . $exam_month . "' AND H.programme_code='".$revaluation1['programme_code']."'  and A.reval_status='YES'".$implode.$arr)->queryScalar();
                    ...
                    JOIN coe_bat_deg_reg as G ON G.coe_bat_deg_reg_id=B.batch_mapping_id 
                     ...
                     JOIN coe_mark_entry_master_reval as D ON D.subject_map_id=A.subject_map_id AND D.student_map_id=A.student_map_id AND A.year=D.year AND A.month=D.month
                     JOIN coe_mark_entry_master as E ON E.subject_map_id=A.subject_map_id AND E.student_map_id=A.student_map_id AND A.year=E.year AND A.month=E.month
                     WHERE D.grade_name!=E.grade_name AND A.year='" .  $exam_year . "' and A.month='" . $exam_month . "' AND H.programme_code='".$revaluation1['programme_code']."'  and A.reval_status='YES' AND A.mark_type=28 ".$implode1)->queryScalar();
               ...
               $data.='<td align="left" >'. $tot_stuar.'</td>';
                ...
                $data.='<td align="left"  colspan=4>'.$tot_scriptu.'</td>';
                $data.='</tr>'; 
