...
$this->title="CGPA Average After Revalution";
    ...
    <div class="col-xs-12 col-sm-12 col-lg-12">
        ...
        <div class="col-xs-12 col-sm-2 col-lg-2">
            ...
            <div class="btn-group col-lg-12 col-sm-12" role="group" aria-label="Actions to be Perform">
                ...
                <?= Html::a("Reset", Url::toRoute(['mark-entry/cgpa-average-after-reval']), ['onClick'=>"spinner();",'class' => 'btn btn-group btn-group-lg btn-warning ']) ?>
            </div>
     ...
     <div class="col-xs-12 col-sm-12 col-lg-12">
            ...
            <div class="col-xs-12" style="padding-top: 30px;" >
                <div class="col-lg-12"  style="overflow-x:auto;"> 
                    ...
                    $explode=$monthname;//explode("/", $monthname);
                    ...
                    $html .= '<tr><td colspan="7" align="center" style=" color: #2173bc; border-top:0px; border-bottom:0px; padding-bottom:10px; font-size: 12px !important;"><b>7278 - CGPA AVERAGE - '.strtoupper($monthname).' '.$Year1.' EXAMINATIONS</b></td></tr></table>';
                   ...
                   // print_r($content_data); exit;
                    ...
                    foreach ($content_data as $key => $value1) 
                    {
                        ...
                        if(!empty($content_1))
                        {
                           ...
                           $total_year_gpa_avg=0;
                            ...
                            $j=0; $pagebreak=1; $pagebreakout=0; $colspan=0; $sn=1;
                                    ...
                                    if($student_enrol>0)
                                    {
                                        if($program['degree_type']=='UG')
                                        {
                                           ...
                                           //echo "<br>".$query_map_id->createCommand()->getRawSql(); exit;
                                            ...
                                            $total_cgpa = Yii::$app->db->createCommand('SELECT sum(gpa) as gpasum, sum(cgpa) as cgpasum, count(gpa) as gpacount, count(cgpa) as cgpacount FROM coe_cgpa_stu_af_reval where year="'.$Year1.'" and month="'.$month1.'" and batch_mapping_id="'.$program['coe_bat_deg_reg_id'].'" ')->queryOne();
                      ...
                      $html1 = "";
                        $body1 ="";
                        $header1=$footer1='';
                      //print_r($revaluation_grade);exit;

                      if(!empty($revaluation_grade))
                      {
                            ...
                            $header1 .= '<table border=1  width="100%" class="table table-striped table-responsive table-hover table-bordered"  align="center">';
                            $header1 .= '<tr><td colspan=9 align="center" style="background-color: #2173bc; color: #fff;"><b>Grade Changes Due to Revaluation</b></td></tr>';
                            $header1 .="
                            <tr height=40px>
                              ...
                              <th align='center'>Batch</th>
                               ...
                               <th align='center'>Mark Before Revaluation(End semester Examinations)</th>
                               <th align='center'>Mark After Revaluation(End semester Examinations)</th>
                            ...
                            $i=1;
                            $footer1 .='</table>';
                        foreach($revaluation_grade as $rows) 
                        { 
                            
                                if($i%45==0 && $i!=1)
                                {
                                    $html1 = $body1; 
                                    $print_stu_data .= $html1;
                                    $html1 = "";
                                    $body1 ="";
                                    $i=1;
                                }
                                 $stu_mark_data = Yii::$app->db->createCommand("select category_type_id_marks from coe_mark_entry where student_map_id='".$rows['student_map_id']."'  and  subject_map_id='".$rows['subject_map_id']."'and year='".$rows['year']."' and month='".$rows['month']."' and mark_type='".$rows['mark_type']."' and category_type_id=89")->queryOne(); 
                                 //print_r($stu_mark_data) ;exit; 

                                
                              
                              if($rows['tempgradename']=="RA" && $rows['tempresult']=="Fail")
                              {
                                   $getgradename="U";
                              ...
                              else
                              {
                               ...
                               // $getgradename=Yii::$app->db->createCommand("SELECT   grade_name FROM `coe_grade_range`   WHERE '".$total."'  between `min_mark` and `max_mark` and `subject_code`='".$rows["subject_code"]."' and semester='".$rows["semester"]."'")->queryScalar();
                                ...
                                $getgradename=Yii::$app->db->createCommand("SELECT   grade_name FROM `coe_mark_entry_master_reval`   WHERE student_map_id= '".$rows['student_map_id']."'  and subject_map_id='".$rows['subject_map_id']."' ")->queryScalar();

                                }
                                if($rows['grade_name']!=$getgradename)
                                {
                                
                                     $body1 .='<tr height=40px>
                                        ...
                                        <td align="center">'.$stu_mark_data['category_type_id_marks'].'</td>
                                         <td align="center">'.$rows["category_type_id_marks"].'</td>
                        ...
                        }// If no Content found for Institution server
                    }
                    ...
                    $print_stu_data = $header1 .$body1.$footer1;
                    $html=$html.$print_stu_data;
