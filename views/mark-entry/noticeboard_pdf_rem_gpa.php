...
<?php
    ...
    $data_1.='<tr>
            ...
            else if(!empty($reval_status) && $reval_status=='yes')
            {
                $total_subs_max_check = Yii::$app->db->createCommand("select count(distinct subject_map_id) as max from coe_subjects_mapping as A JOIN coe_mark_entry as B ON B.subject_map_id=A.coe_subjects_mapping_id JOIN coe_subjects as C ON C.coe_subjects_id=A.subject_id where batch_mapping_id='".$_POST['bat_map_val']."' and B.year='".$_POST['year']."' and B.month='".$_POST['month']."' and category_type_id='".$reval_status_entry['coe_category_type_id']."' ")->queryOne();
                $total_subs_max = $total_subs_max_check['max'];
            }
            ...
            {   
                ...
                if($first_reg_num==0)
                {
                    $insert_fitrst_data .= '<td colspan=2 >&nbsp;</td> </tr><tr>';
                    $first_reg_num = 5;
                }
