...
<?php echo Html::a('<i class="fa fa-file-pdf-o"></i> ' . "PDF",array('print-datewisereports','exportPDF'=>'PDF'),array('title'=>'Export to PDF','target'=>'_blank','class'=>'btn btn-warning', 'style'=>'color:#fff')); 
      ?>
    <?php echo Html::a('<i class="fa fa-file-excel-o"></i> ' . "EXCEL ",array('excel-datewisereports','exportPDF'=>'PDF'),array('title'=>'Export to Excel','target'=>'_blank','class'=>'btn btn-info', 'style'=>'color:#fff')); ?>
...
<?php
  ...
  $totalStudents =0;
  $increment=0;
  foreach($date_wise_reports as $rows) 
  { 
...
<table class="table" border="1" cellpadding="1" >
  ...
  <tr>
    <td colspan="8" ALIGN="center"><b> '.strtoupper(ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_EXAM)).' DATE WISE REPORT FOR - 
      '.$app_month." -".$year." ".strtoupper(ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_EXAM))." DATE ".date('d-m-Y',strtotime($rows['exam_date'])).' SESSION : '.$rows['session'].' </b> 
    </td>
  </tr>
...
<tr>
  ...
  <td>YEAR </td> 
  <td> '.strtoupper(ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_BATCH)).' </td>
...
foreach($date_wise_reports as $rows) 
{
 ...
 $totalStudents+=$rows['count'];
      ...
      $body .="<tr><td height=40 colspan=8> &nbsp; </td></tr>
              <tr>
                <td colspan=4> ".strtoupper('Grand Total '.ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_STUDENT))."</td>
                <td colspan=4>".$totalStudents."</td>
             </tr>";
      $footer .='</table>';
      $html = $header .$body.$footer;

      $print_stu_data .=$html;
      if(isset($_SESSION['date_wise_reports'])){ unset($_SESSION['date_wise_reports']);}
      $_SESSION['date_wise_reports'] = $print_stu_data;
