...
?>
<div class="subjects-mapping-view">
    <?php Yii::$app->ShowFlashMessages->showFlashes();?>
    <h1><?= Html::encode($this->title) ?></h1>

    <p>
        <?php 
            if(Yii::$app->user->can("/subjects-mapping/update") || Yii::$app->user->can("/subjects-mapping/delete"))
            {
        ?>
                <?=
                Html::a('Update', ['update', 'id' => $model->coe_subjects_mapping_id], ['class' => 'btn btn-primary']) 
                ?>
           
                 <?= 
                 Html::a('Delete', ['delete', 'id' => $model->coe_subjects_mapping_id], [
