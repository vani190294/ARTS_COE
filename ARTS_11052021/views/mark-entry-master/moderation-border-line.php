...
$this->title="RESULT PUBLISH WITH MODERATION";
$marks_out_of = isset($model->mark_out_of)?$model->mark_out_of:'';
...
<?php Yii::$app->ShowFlashMessages->showFlashes();?>
    ...
    <?php $form = ActiveForm::begin([
        ...
        <div class="col-xs-12 col-sm-12 col-lg-12">
            ...
            <div class="col-lg-2 col-sm-2">
                <?= $form->field($model, 'semester')->textInput(['id'=>'mark_semester','value'=>$sem,'name'=>'exam_semester']) ?>
        ...
        <div class="col-xs-12 col-sm-12 col-lg-12">
            <div class="col-lg-2 col-sm-2">
                <?= $form->field($model, 'mark_out_of')->textInput(['value'=>$marks_out_of])->label('Moderation Marks') ?>
            </div>
            <div class="form-group col-lg-3 col-sm-3"><br />
                    ...
                    <?= Html::a("Reset", Url::toRoute(['mark-entry-master/moderation-border-line']), ['class' => 'btn btn-group btn-group-lg btn-warning ']) ?>
            </div>
        </div>
    </div>   
       ...
       if($file_content_available=="Yes")
        {
            ...
            $stu_print_vals = 0;
               ...
               /*$header .='<table class="table" border="1" cellpadding="1" align="center" cellspacing="1">
                        ...
                        $stu_withheld = !empty($withheld_list)?2:1;
                         ...
                         { 
                            ...
                            $prev_num=$rowsstudent['register_number'];                   
                             ...
                             $header .='<tr><td align="center">'.$rowsstudent["register_number"].'</td>';
                            ...
                            {                                
                                if($rowsstudent['register_number']==$print_td['register_number']
                                ...
                                )
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            if($print_td["result"]=="Fail" && $print_td["ESE_max"]!=0)
                                            {
                                                $total_min_pass = $print_td["total_minimum_pass"];
                                                $ese_min = $print_td["ESE_min"];
                                                $stu_org_ese = $print_td["ESE"];
                                                $stu_mode_ese = $print_td["ESE_min"];    
                                                $border_marks = $print_td["ESE_min"]-$marks_out_of;
                                                if($stu_org_ese>=$border_marks && $stu_org_ese<$ese_min)
                                                {
                                                    if($stu_withheld==2)
                                                    {
                                                        $total = $cia;
                                                    ...
                                                    {
                                                       if($print_td["grade_name"]=="WH" || $print_td["grade_name"]=="wh")
                                                        {
                                                            $total = $stu_org_ese+$cia;
                                                            $ese = $stu_org_ese;
                                                            $grade_name_pri='RA';
                                                            $result='F';
                                                        }
                                                        else if($print_td["withheld"]=="W" || $print_td["withheld"]=="w" || $print_td["withheld"]=="WH" || $print_td["withheld"]=="wh")
                                                        {
                                                            $total = $stu_org_ese+$cia;
                                                            $ese = $stu_org_ese;
                                                            $grade_name_pri='RA';
                                            ...
                                            else
                                            {
                                                ...
                                                else if($print_td["grade_name"]=="WH" || $print_td["grade_name"]=="wh" || $print_td["grade_name"]=="W" || $print_td["grade_name"]=="w")
                                                {
                                                    $total = $print_td["total"];
                                                    $ese = $print_td["ESE"];
                                                    $grade_name_pri='RA';
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        if($stu_withheld==2)
                                        {
                                            $header .='<td align="center"><table  width=100%><tr>';
                                            $add_css1="border: 1px solid #000; width: 22px; height: 22px; background: #666; color: #FFF; ";
                                            $header .='<td  colspan=5  align="center" style="'.$add_css1.'"> WITHHELD </td>';  
                                            $header .='</tr></table></td>';
                                        }
                                        else
                                        {
                                            ...
                                            $add_css1=$grade_name=="RA" || $grade_name=="AB" || $grade_name=="W" || $grade_name=="w" ?"border: 1px solid #333; width: 22px; height: 22px; background: #666; color: #FFF; ":"color: #000;";
                ...
                $header .='<tr> <td align="center">APPEARED</td>';
                $print_onetime=1;
                foreach($subjectsInfo as $rows) 
                {
                  if($rows["subject_code"]!=$previous_subjects_code) 
                  {
                    foreach ($send_result as $key => $values) 
                    {
                        if($rows["subject_code"]==$values['subject_code'])
                        {
                            ...
                            if($values["result"]=="Fail"  && $values["ESE_max"]!=0)
                            {
                                $total_min_pass = $values["total_minimum_pass"];
                                $ese_min = $values["ESE_min"];
                                $stu_org_ese = $values["ESE"];
                                $stu_mode_ese = $values["ESE_min"];
                                $border_marks = $values["ESE_min"]-$marks_out_of;
                                if($stu_1_withheld==2)
                                {
                                    $stu_count_result[] =0;
                                    $ese_total[] = [ $values['subject_code']=>0];
                                    $total = $cia;
                                    $ese = 0;
                                    $grade_name_pri='RA';
                                    $result='F';
                                ...
                                else
                                {
                                    if($stu_org_ese>=$border_marks && $stu_org_ese<$ese_min)
                                    {
                                            $total = $values["ESE_min"]+$cia;
                                            $ese = $stu_mode_ese;
                                            $grade_name_pri='E';
                                            $result='P';
                                            $stu_count_result[] = $values['subject_code'];
                                            $ese_total[] = [ $values['subject_code']=>$stu_mode_ese];
                                        
                                    }
                                    else
                                    {
                                        $stu_count_result[] = ($values["result"]=="Pass" || $values["result"]=="pass" || $values["result"]=="PASS")?$values['subject_code']:0;
                                        $total = $values["total"];
                                        $ese = $values["ESE"];
                                        $grade_name_pri=$values["grade_name"];
                                        $result=($values["result"]=="Pass" || $values["result"]=="pass" || $values["result"]=="PASS")?'P':'F';                               
                                        $ese_total[] = [ $values['subject_code']=>$values['ESE']]; 
                                    }
                                }
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    $ese = $values["ESE"];
                                    $total = $values["total"];
                                    $stu_get_res = $values["result"];
                                }

                                $stu_count_result[] = ($stu_get_res=="Pass" || $stu_get_res=="pass" || $stu_get_res=="PASS")?$values['subject_code']:0;
                                $result=($stu_get_res=="Pass" || $stu_get_res=="pass" || $stu_get_res=="PASS")?'P':'F';
                                $ese_total[] = [ $values['subject_code']=>$values['ESE']]; 
                            }  
                            $cia_total[] = [ $values['subject_code']=>$values['CIA']];
                        }
                    }
              ...
              $total_appeared_count = max($stu_count_array);
              ...
              $overall_pass_percentage = round((($total_pass_count_calc/$total_appeared_count)*100),2);

              $header .= '<td colspan=2 align="center">'.$overall_pass_percentage.'</td></tr>';
              //$header .= '<td colspan=2>&nbsp;</td></tr>';
              
              $header.='<tr><td align="center">CIA AVERAGE</td>';
              for($i=0;$i<count($cia_total_subs_average);$i++){
                
                $header.='<td align="center">'.round(($cia_total_subs_average[$i]/$stu_count_array[$i]),2).'</td>';
              }
              ...
              for($i=0;$i<count($ese_total_subs_average);$i++){
                ...
                $header.='<td align="center">'.round(($ese_total_subs_average[$i]/$stu_count_array[$i]),2).'</td>';
              }
              $header .= '<td  align="center" colspan=2> -- </td></tr>';
