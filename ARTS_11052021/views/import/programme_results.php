...
<div class="box box-success">
	<div class="box-header">
		<h3 class="box-title"><i class="fa fa-list-ul"></i> <?php echo ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_PROGRAMME).' Import Results'; ?></h3>
              ...
              <?php 
				...
				<?php if(!empty($importResults['totalSuccess'])) : ?>
					<div class="alert alert-success">
						...
						<?= "{$importResults['totalSuccess']} ". ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_PROGRAMME).'  importing successfully.' ?>
					</div>
				<?php endif; ?>
				...
				<?php if(!empty($totalError)) : ?>
					<div class="alert alert-danger">
						...
						<?= "{$totalError} ". ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_PROGRAMME).'  importing error.' ?>
					</div>
				<?php endif; ?>
                           
            	<?php
					$headerTr.= Html::tag('th', 'Sr No');
					$headerTr.= Html::tag('th', ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_PROGRAMME)." Code");
					$headerTr.= Html::tag('th', ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_PROGRAMME)." Descrition");					
					$headerTr.= Html::tag('th', 'Status');
					$headerTr.= Html::tag('th', 'Message');
				?>
				<table style="overflow-x:auto;"  cellspacing="0" cellpadding="0" border="0"  class="table table-bordered table-responsive bulk_edit_table table-hover" id = "programme_import_results">
					<thead>
...
$this->registerJs(<<<JS
    $(function () {
    $('#programme_import_results').DataTable({
      'paging'      : true,
      'lengthChange': false,
      'searching'   : false,
      'ordering'    : true,
