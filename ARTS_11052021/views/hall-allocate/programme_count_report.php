...
<?php echo Html::a('<i class="fa fa-file-pdf-o"></i> ' . "PDF",array('print-programmeexamreports','exportPDF'=>'PDF'),array('title'=>'Export to PDF','target'=>'_blank','class'=>'btn btn-warning', 'style'=>'color:#fff')); 
      ?>
    <?php echo Html::a('<i class="fa fa-file-excel-o"></i> ' . "EXCEL ",array('excel-programmeexamreports','exportPDF'=>'PDF'),array('title'=>'Export to Excel','target'=>'_blank','class'=>'btn btn-info', 'style'=>'color:#fff')); ?>
...
<table class="table" border="1" cellpadding="1" align="center" cellspacing="1">
  ...
  <tr>
    <td colspan="7" ALIGN="center"><b> '.strtoupper(ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_DEGREE))." ".strtoupper(ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_PROGRAMME)).' WISE REPORT FOR - 
      '.$app_month." -".$year.'</b> 
    </td>
  </tr>

<tr>
  <td> '.strtoupper(ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_DEGREE)).' NAME  
  </td>
  <td colspan="2"> <b>'.$rows['degree_name'].'</b>  
  </td> 
  <td> '.strtoupper(ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_PROGRAMME)).' NAME 
  </td>
  <td colspan="3"> <b>'.$rows['programme_name'].'</b>
  </td> 
...
<tr>
  ...
  <td colspan=3>'.strtoupper(ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_SUBJECT)." Name").' </td>
  <td colspan=2>'.strtoupper("Total ".ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_STUDENT)).'</td>
...
$space='&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'; 
  $body .="<tr>
    ...
    <td colspan=3>".$rows['subject_name']."</td>
    <td colspan=2>".$rows['count']."</td>
 </tr>";
...
$previous_subject_code=$rows['programme_name'];
      ...
      $body .="<tr><td height=40 colspan=7> &nbsp; </td></tr>
              <tr>
                <td colspan=5> ".strtoupper('Grand Total '.ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_STUDENT))."</td>
                <td colspan=2>".$totalStudents."</td>
             </tr>";
      $html = $header .$body.$footer;
      $print_stu_data .=$html;
