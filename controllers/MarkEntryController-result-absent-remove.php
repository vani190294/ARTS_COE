...
B.subject_id=A.coe_subjects_id JOIN coe_mark_entry_master as C ON C.subject_map_id=B.coe_subjects_mapping_id JOIN 
coe_bat_deg_reg as E ON E.coe_bat_deg_reg_id=B.batch_mapping_id JOIN coe_programme as D ON D.coe_programme_id=E.coe_programme_id JOIN 
coe_batch as F ON F.coe_batch_id=E.coe_batch_id and JOIN coe_student_mapping as abc ON abc.course_batch_mapping_id=B.batch_mapping_id where C.year<='2019' and student_map_id NOT IN
(select student_map_id FROM coe_mark_entry_master as abc where abc.subject_map_id=C.subject_map_id and result like '%Pass%' ) and C.result not like 
'%Pass%' and (year_of_passing='' or year_of_passing=NULL) and batch_name >= (YEAR(CURDATE())-".$omit_batches.") and status_category_type_id!='".$det_disc_type."' group by batch_mapping_id,subject_map_id order by batch_name,D.programme_code asc"; 
    ...
    public function actionRevaluationentrysub()
    {
        ...
        $subject_trans_result = Yii::$app->db->createCommand("select distinct subject_code as subject_code,subject_fee,subject_name,coe_subjects_id,B.semester,B.coe_subjects_mapping_id,coe_student_mapping_id,F.mark_type,C.name,F.grade_name from coe_subjects as A,coe_subjects_mapping as B,coe_student as C,coe_student_mapping as D,coe_mark_entry as E,coe_mark_entry_master as F,coe_revaluation as G where G.student_map_id=F.student_map_id and G.subject_map_id=F.subject_map_id and A.coe_subjects_id=B.subject_id and B.batch_mapping_id=D.course_batch_mapping_id and C.coe_student_id=D.student_rel_id and F.student_map_id=D.coe_student_mapping_id and E.student_map_id=D.coe_student_mapping_id and F.subject_map_id=B.coe_subjects_mapping_id and E.subject_map_id=B.coe_subjects_mapping_id and F.year='" . $year . "' and F.month='" . $month . "' and C.register_number='" . $stu_reg_num . "' and E.category_type_id!='" . $cat_mod_mark_type . "' and C.student_status='Active' and B.paper_type_id!='" . $cat_paper_type . "' and F.result!='Absent' and status_category_type_id NOT IN('".$det_disc_type."') and A.ESE_max!=0 and A.ESE_min!=0 order by B.semester")->queryAll();
    ...
    public function actionViewrevaluation()
    {
        ...
        if (isset($_POST['view_reval_btn'])) 
        {
            ...
            {
                $join_in_query = ' JOIN coe_dummy_number as F WHERE F.student_map_id=E.student_map_id and F.subject_map_id=E.subject_map_id and ';
    ...
    public function actionRevaluationmarkentryview()
    {
        ...
        //echo $stu_mark['grade_name'];exit;
        $previous_ese_100 = Yii::$app->db->createCommand("select * from coe_mark_entry where student_map_id='" . $stu_mark['student_map_id'] . "' and category_type_id IN ('" . $cat_ese_mark_type . "','".$cat_dummy_mark_type."') and subject_map_id='" . $stu_mark['coe_subjects_mapping_id'] . "'")->queryOne();

        $pre_ese = round(($previous_ese_100['category_type_id_marks'] / 100) * $stu_mark['ESE_max']);
        $revaluation_ese_100 = Yii::$app->db->createCommand("select * from coe_mark_entry where student_map_id='" . $stu_mark['student_map_id'] . "' and category_type_id='" . $cat_rev_mark_type . "' and subject_map_id='" . $stu_mark['coe_subjects_mapping_id'] . "'")->queryOne();
...
public function actionRevaluationSubjectEntry()
    ...
    public function actionMarkpercentreport()
    {
        ...
        foreach ($failed_stus as $key => $fails) {
         ...
         $notIn = array_filter($notIn);
            //echo $batch_name.'--'.$degree_name;exit;
            $query_map_id = new Query();
            $query_map_id->select(['count(distinct student_map_id) as count','course_batch_mapping_id','programme_code','degree_code','batch_name'])
                ->from('coe_student_mapping a')
                ->join('JOIN', 'coe_student b', 'a.student_rel_id=b.coe_student_id')
            ...
            $query_map_id_fail->select(['count(distinct student_map_id) as count','course_batch_mapping_id','programme_code','degree_code','batch_name'])
                ->from('coe_student_mapping a')
                ->join('JOIN', 'coe_student b', 'a.student_rel_id=b.coe_student_id')
                ->join('JOIN', 'coe_bat_deg_reg c', 'c.coe_bat_deg_reg_id=a.course_batch_mapping_id')
                ->join('JOIN', 'coe_batch d', 'd.coe_batch_id=c.coe_batch_id')
            ...
            if (count($students_map_id_pass) > 0) 
            {
                ...
                $sn_no = 1;
                $i = $total_pass = $total_strength_active = $total_strength = $total_fail =0;
                ...
                $data .='<tr><td colspan=3> Grand Total </td><td align="center">'.$total_strength.'</td><td align="center">'.$total_strength_active.'</td><td align="center">'.$total_pass.'</td><td>'.round((($total_pass/$total_strength_active)*100),2).'</td></tr>';
    ...
    public function actionSubjectinformationdata()
    {
        ...
        $sn = 1;
        $query = new Query();
        $query->select('F.batch_name,C.degree_code,B.programme_name,E.semester,D.subject_code,D.subject_name,D.CIA_max,D.ESE_max,D.total_minimum_pass,D.credit_points,D.CIA_min,D.ESE_min,G.description as paper_type,H.description as subject_type,I.description as course_type,paper_no,part_no')
            ->from('coe_bat_deg_reg A')
            ->join('JOIN', 'coe_programme B', 'A.coe_programme_id=B.coe_programme_id')
            ->join('JOIN', 'coe_degree C', 'A.coe_degree_id=C.coe_degree_id')
    ...
    public function actionInternetCopy()
    {
        ...
        if (isset($_POST['internetcopybutton'])) 
        {
            ...
            $getAbsentList = AbsentEntry::find()->where(['exam_year'=>$_POST['year'],'exam_month'=>$_POST['month']])->one(); // Changed this for generating the results for 2019 April May
            $created_at = date("Y-m-d H:i:s");
            $updateBy = Yii::$app->user->getId();
            $config_attempt = ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_CIA_ZEO);
            if(!empty($getAbsentList))
            {
                ...
                {         
                   ...
                   $stuCiaMarksMaster = MarkEntryMaster::find()->where(['subject_map_id'=>$subject_map_id,'student_map_id'=>$student_map_id])->orderBy('coe_mark_entry_master_id desc')->one();
                    ...
                    {
                        /*$GETsTUiNFO = StudentMapping::findOne($student_map_id);
                        $stuDetails = Student::findOne($student_map_id->student_rel_id);
                        $getSubInfo = SubjectsMapping::findOne($subject_map_id);
                        $subDetails = Subjects::findOne($getSubInfo->subject_id);
                        echo $stuDetails['register_number']." ".$subDetails['subject_code']."<br />";*/
    ...
    public function actionInternetCopyPdf()
    {
        $updated_at = date("Y-m-d H:i:s");
        $updated_by = Yii::$app->user->getId();
        Yii::$app->db->createCommand("update coe_mark_entry_master set status_id=1,updated_by='".$updated_by."',updated_at='".$updated_at."' where year='" . $_SESSION['mark_year'] . "' and month='" . $_SESSION['mark_month'] . "'")->execute();
    ...
    public function actionUniversityreportdatacomp()
    {
        ...
        $get_stu_query = "SELECT F.student_map_id,A.register_number,A.dob,A.name,A.gender,D.degree_code,D.degree_name,E.programme_name,H.subject_code,C.regulation_year,H.subject_name,G.semester,H.credit_points,H.CIA_max,H.ESE_max,F.grade_point,F.grade_name,F.year,F.month,B.course_batch_mapping_id,F.year_of_passing,paper_no,F.result,max(F.year_of_passing) as last_appearance, round (sum(H.credit_points*F.grade_point)/sum(H.credit_points),5) as total  FROM coe_student as A JOIN coe_student_mapping as B ON B.student_rel_id=A.coe_student_id JOIN coe_bat_deg_reg as C ON C.coe_bat_deg_reg_id=B.course_batch_mapping_id JOIN coe_degree as D ON D.coe_degree_id=C.coe_degree_id JOIN coe_programme as E ON E.coe_programme_id=C.coe_programme_id JOIN coe_mark_entry_master as F ON F.student_map_id=B.coe_student_mapping_id JOIN coe_subjects_mapping as G ON G.coe_subjects_mapping_id=F.subject_map_id JOIN coe_subjects as H ON H.coe_subjects_id=G.subject_id JOIN coe_batch as I ON I.coe_batch_id=C.coe_batch_id where G.batch_mapping_id='".$bat_map_id."' AND B.coe_student_mapping_id NOT IN(select student_map_id from coe_mark_entry_master where year_of_passing='' and subject_map_id NOT IN(select subject_map_id FROM coe_mark_entry_master where student_map_id=F.student_map_id and result like '%pass%')) and status_category_type_id NOT IN('".$det_cat_type."','".$det_disc_type."') AND F.grade_name not IN ('RA','wh','AB','ab','WH','ra','wd','WD') and F.year='".$year."' and F.month='".$month."'  group by A.register_number order by A.register_number, G.semester, paper_no";
        $final_student = Yii::$app->db->createCommand($get_stu_query)->queryAll();

        $last_appearance_of = Yii::$app->db->createCommand("SELECT max(F.year_of_passing) as last_appearance FROM coe_student as A JOIN coe_student_mapping as B ON B.student_rel_id=A.coe_student_id JOIN coe_bat_deg_reg as C ON C.coe_bat_deg_reg_id=B.course_batch_mapping_id JOIN coe_degree as D ON D.coe_degree_id=C.coe_degree_id JOIN coe_programme as E ON E.coe_programme_id=C.coe_programme_id JOIN coe_mark_entry_master as F ON F.student_map_id=B.coe_student_mapping_id JOIN coe_subjects_mapping as G ON G.coe_subjects_mapping_id=F.subject_map_id JOIN coe_subjects as H ON H.coe_subjects_id=G.subject_id JOIN coe_batch as I ON I.coe_batch_id=C.coe_batch_id  where G.batch_mapping_id='".$bat_map_id."' and status_category_type_id NOT IN('".$det_cat_type."','".$det_disc_type."') AND B.coe_student_mapping_id NOT IN(select student_map_id from coe_mark_entry_master where year_of_passing='') AND F.grade_name not IN ('RA','wh','AB','ab','WH','ra','wd','WD') and F.year<='".$year."'   group by A.register_number")->queryScalar();
