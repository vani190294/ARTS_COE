 ...
 public function actionEsemarklist1()
    {
        ...
        if (Yii::$app->request->post()) {
            ...
            $subject_get_data->select('distinct (H.subject_code) as subject_code, H.subject_name, H.CIA_max, H.ESE_max')
                ...
                ->join('JOIN', 'coe_subjects_mapping as G', 'G.coe_subjects_mapping_id=F.subject_map_id')
                ->join('JOIN', 'coe_subjects H', 'H.coe_subjects_id=G.subject_id');
            $subject_get_data->Where(['F.year' => $_POST['mark_year'], 'B.course_batch_mapping_id' => $_POST['bat_map_val'], 'F.month' => $_POST['month'], 'F.mark_type' => $_POST['mark_type'], 'F.term' => $_POST['term'],'student_status'=>'Active'])->andWhere(['<>', 'status_category_type_id', $det_cat_type])
                ->andWhere(['<>', 'status_category_type_id', $det_disc_type]);
            $query->orderBy('H.subject_code');
            $subjectsInfo = $subject_get_data->createCommand()->queryAll();
            $countQuery = new  Query();
            $countQuery->select('count( distinct H.subject_code) as count')
                ...
                ->join('JOIN', 'coe_subjects_mapping as G', 'G.coe_subjects_mapping_id=F.subject_map_id')
                ->join('JOIN', 'coe_subjects H', 'H.coe_subjects_id=G.subject_id');
            $countQuery->Where(['F.year' => $_POST['mark_year'], 'B.course_batch_mapping_id' => $_POST['bat_map_val'], 'F.month' => $_POST['month'], 'F.mark_type' => $_POST['mark_type'], 'F.term' => $_POST['term'],'student_status'=>'Active'])->andWhere(['<>', 'status_category_type_id', $det_cat_type])
                ->andWhere(['<>', 'status_category_type_id', $det_disc_type]);
            $countOfSubjects = $countQuery->createCommand()->queryAll();
            if (!empty($ese_list)) {
         ...
         //print_r($sem);exit;
            ...
            if (count($students_map_id_pass) > 0) 
            {
                ...
                {               
                    ...
                    /* $query_enrolled->select(['count(distinct e.student_map_id) as count','course_batch_mapping_id','programme_code','degree_code','batch_name','degree_type'])
                ->from('coe_student_mapping a')
                ->join('JOIN', 'coe_student b', 'a.student_rel_id=b.coe_student_id')
                ->join('JOIN', 'coe_bat_deg_reg c', 'c.coe_bat_deg_reg_id=a.course_batch_mapping_id')
                ->join('JOIN', 'coe_batch d', 'd.  coe_batch_id=c.coe_batch_id')
                ->join('JOIN', 'coe_degree f', 'f.coe_degree_id=c.coe_degree_id')
                ->join('JOIN', 'coe_programme g', 'g.coe_programme_id=c.coe_programme_id')
                ->join('JOIN', 'coe_subjects_mapping x', 'x.batch_mapping_id=a.course_batch_mapping_id')
                ->join('JOIN', 'coe_mark_entry_master e', 'e.student_map_id=a.coe_student_mapping_id and e.subject_map_id=x.coe_subjects_mapping_id')
                ->where(['d.coe_batch_id' => $_POST['batch'],'c.coe_batch_id' => $_POST['batch'],'e.month'=>$_POST['month'],'e.mark_type'=>27,'e.year'=>$_POST['year'],'x.semester'=>$sem])
                ->where(['course_batch_mapping_id'=>$deg['course_batch_mapping_id']])
                ->groupBy('course_batch_mapping_id')
                ->orderBy('degree_code,programme_code');
                 $student_enrolled = $query_enrolled->createCommand()->queryScalar();
                 */
                /* $query_cr = new Query();
                 $query_cr->select('distinct(subject_code),b.semester,subject_name,coe_subjects_mapping_id,description,coe_subjects_id,batch_mapping_id')
                 ->from('coe_subjects a')
                 ...
                 ->join('JOIN', 'coe_mark_entry_master c', 'b.coe_subjects_mapping_id=c.subject_map_id')
                  ->join('JOIN', 'coe_category_type d', 'b.subject_type_id=d.coe_category_type_id')
                  ->where([ 'c.year' => $_POST['year'], 'c.month' => $_POST['month'], 'c.mark_type' => 27]);
               ...
               //print_r($subject_list);exit;
                 ...
                 $query_enrolled= new Query();
                 $query_enrolled->select(['count(distinct a.student_map_id) as count','course_batch_mapping_id','programme_code','degree_code','batch_name','degree_type'])
                ->from('coe_mark_entry_master a')
                ->join('JOIN', 'coe_subjects_mapping b','b.coe_subjects_mapping_id=a.subject_map_id')
                 ->join('JOIN', 'coe_student_mapping e','e.coe_student_mapping_id=a.student_map_id')
                ->join('JOIN', 'coe_bat_deg_reg c', 'c.coe_bat_deg_reg_id=e.course_batch_mapping_id')
                ->join('JOIN', 'coe_batch d', 'd.  coe_batch_id=c.coe_batch_id')
                ->join('JOIN', 'coe_degree f', 'f.coe_degree_id=c.coe_degree_id')
                ->join('JOIN', 'coe_programme g', 'g.coe_programme_id=c.coe_programme_id')
                ->where(['d.coe_batch_id' => $_POST['batch'],'c.coe_batch_id' => $_POST['batch'],'a.month'=>$_POST['month'],'a.mark_type'=>27,'a.year'=>$_POST['year'],'b.semester'=>$sem])
                ->where(['course_batch_mapping_id'=>$deg['course_batch_mapping_id']])
                ->groupBy('course_batch_mapping_id')
                ->orderBy('degree_code,programme_code');
                 $student_enrolled = $query_enrolled->createCommand()->queryScalar();

            //print_r($student_enrolled);exit;
                ...
                $query_appeared->select('count(DISTINCT student_map_id)')
                    ...
                    ->join('JOIN', 'coe_mark_entry_master b', 'b.student_map_id=a.coe_student_mapping_id')
                    ->join('JOIN', 'coe_subjects_mapping c', 'c.coe_subjects_mapping_id=b.subject_map_id')
                    ->where([ 'b.year' => $_POST['year'], 'b.month' => $_POST['month']])
                    ->where(['course_batch_mapping_id'=>$deg['course_batch_mapping_id']])
                    ->andWhere(['NOT LIKE', 'b.result', 'Absent']);
 ...
 public function actionMarkpercentreport()

    {
        ...
        $MON_name = Categorytype::findOne($_POST['month']);
         $notIn = array_filter($notIn);
           ...
           //print_r($students_map_id_pass);exit;
   ...
   /* public function actionSubjectinformationdata()
    ...
    public function actionHallticketexportdata()
    {
        ...
        else
        {
            ...
            $query->select('L.batch_name,D.register_number,D.name,D.dob,B.degree_code,C.programme_name,G.semester,F.subject_code,F.subject_name,H.exam_date,K.category_type,I.hall_name,J.row,J.row_column,J.seat_no')
                ...
                ->where(['J.year' => $year,'J.month' => $month,'H.exam_year'=> $year,'H.exam_month'=>$month,'L.coe_batch_id'=>$batch_id]);
                /*if(isset($batch_id) && !empty($batch_id))
                {
    ...
    public function actionUniversityreportdatacomp()
    {
        ...
        $det_cat_transfer=Yii::$app->db->createCommand("select coe_category_type_id from coe_category_type where category_type like '%Transfer%'")->queryScalar();
       ;
        $month_add_contion = $month==29?'month=30':'month IN(29,30)';

        //print_r($month_add_contion);exit;

    //$get_stu_query = "SELECT F.student_map_id,A.register_number,A.dob,A.name,A.gender,D.degree_code,D.degree_name,E.programme_name,H.subject_code,C.regulation_year,H.subject_name,G.semester,H.credit_points,H.CIA_max,H.ESE_max,F.grade_point,F.grade_name,F.year,F.month,B.course_batch_mapping_id,F.year_of_passing,paper_no,F.result,max(F.year_of_passing) as last_appearance, round (sum(H.credit_points*F.grade_point)/sum(H.credit_points),5) as total  FROM coe_student as A JOIN coe_student_mapping as B ON B.student_rel_id=A.coe_student_id JOIN coe_bat_deg_reg as C ON C.coe_bat_deg_reg_id=B.course_batch_mapping_id JOIN coe_degree as D ON D.coe_degree_id=C.coe_degree_id JOIN coe_programme as E ON E.coe_programme_id=C.coe_programme_id JOIN coe_mark_entry_master as F ON F.student_map_id=B.coe_student_mapping_id JOIN coe_subjects_mapping as G ON G.coe_subjects_mapping_id=F.subject_map_id JOIN coe_subjects as H ON H.coe_subjects_id=G.subject_id JOIN coe_batch as I ON I.coe_batch_id=C.coe_batch_id where G.batch_mapping_id='".$bat_map_id."' AND B.coe_student_mapping_id NOT IN(select student_map_id from coe_mark_entry_master where year_of_passing='' and subject_map_id NOT IN(select subject_map_id FROM coe_mark_entry_master where student_map_id=F.student_map_id and result like '%pass%' and year='".$year."' and month='".$month."') and year='".$year."' and ".$month."  ) and status_category_type_id NOT IN('".$det_cat_type."','".$det_disc_type."') AND F.grade_name not IN ('RA','wh','AB','ab','WH','ra','wd','WD') and F.year='".$year."' and F.month='".$month."'  group by A.register_number order by A.register_number, G.semester, paper_no "; 


$get_stu_query = "SELECT F.student_map_id,A.register_number,A.dob,A.name,A.gender,D.degree_code,D.degree_name,E.programme_name,H.subject_code,C.regulation_year,H.subject_name,G.semester,H.credit_points,H.CIA_max,H.ESE_max,F.grade_point,F.grade_name,F.year,F.month,B.course_batch_mapping_id,F.year_of_passing,paper_no,F.result,max(F.year_of_passing) as last_appearance, round (sum(H.credit_points*F.grade_point)/sum(H.credit_points),5) as total  FROM coe_student as A JOIN coe_student_mapping as B ON B.student_rel_id=A.coe_student_id JOIN coe_bat_deg_reg as C ON C.coe_bat_deg_reg_id=B.course_batch_mapping_id JOIN coe_degree as D ON D.coe_degree_id=C.coe_degree_id JOIN coe_programme as E ON E.coe_programme_id=C.coe_programme_id JOIN coe_mark_entry_master as F ON F.student_map_id=B.coe_student_mapping_id JOIN coe_subjects_mapping as G ON G.coe_subjects_mapping_id=F.subject_map_id JOIN coe_subjects as H ON H.coe_subjects_id=G.subject_id JOIN coe_batch as I ON I.coe_batch_id=C.coe_batch_id where G.batch_mapping_id='".$bat_map_id."' AND B.coe_student_mapping_id NOT IN(select student_map_id from coe_mark_entry_master where year_of_passing='' and subject_map_id NOT IN(select subject_map_id FROM coe_mark_entry_master where student_map_id=F.student_map_id and result like '%pass%' and F.year='".$year."' and F.month='".$month."') and F.year='".$year."' and  F.month=".$month."  ) and status_category_type_id NOT IN('".$det_cat_type."','".$det_disc_type."') AND F.grade_name not IN ('RA','wh','AB','ab','WH','ra','wd','WD') and F.year='".$year."' and F.month='".$month."'  group by A.register_number order by A.register_number, G.semester, paper_no ";

    $final_student = Yii::$app->db->createCommand($get_stu_query)->queryAll();
    foreach ( $final_student as $key => $value)
      {
        

         $stu_map = StuInfo::findOne(['stu_map_id'=>$value['student_map_id'] ]);
         $prev=StuInfo::findOne(['stu_map_id'=>$stu['stu_map_id'] ]);
         if(!empty($stu_map['prev_reg']))
         {

            $get_stu_query = "SELECT F.student_map_id,A.register_number,A.dob,A.name,A.gender,D.degree_code,D.degree_name,E.programme_name,H.subject_code,C.regulation_year,H.subject_name,G.semester,H.credit_points,H.CIA_max,H.ESE_max,F.grade_point,F.grade_name,F.year,F.month,B.course_batch_mapping_id,F.year_of_passing,paper_no,F.result,max(F.year_of_passing) as last_appearance, round (sum(H.credit_points*F.grade_point)/sum(H.credit_points),5) as total  FROM coe_student as A JOIN coe_student_mapping as B ON B.student_rel_id=A.coe_student_id JOIN coe_bat_deg_reg as C ON C.coe_bat_deg_reg_id=B.course_batch_mapping_id JOIN coe_degree as D ON D.coe_degree_id=C.coe_degree_id JOIN coe_programme as E ON E.coe_programme_id=C.coe_programme_id JOIN coe_mark_entry_master as F ON F.student_map_id=B.coe_student_mapping_id JOIN coe_subjects_mapping as G ON G.coe_subjects_mapping_id=F.subject_map_id JOIN coe_subjects as H ON H.coe_subjects_id=G.subject_id JOIN coe_batch as I ON I.coe_batch_id=C.coe_batch_id where G.batch_mapping_id='".$bat_map_id."' AND B.coe_student_mapping_id NOT IN(select student_map_id from coe_mark_entry_master where year_of_passing='' and subject_map_id NOT IN(select subject_map_id FROM coe_mark_entry_master where student_map_id=F.student_map_id and result like '%pass%' and F.year='".$year."' and F.month='".$month."') and F.year='".$year."' and  F.month=".$month."  ) and status_category_type_id NOT IN('".$det_cat_type."','".$det_disc_type."') AND F.grade_name not IN ('RA','wh','AB','ab','WH','ra','wd','WD') and F.year='".$year."' and F.month='".$month."' and F.student_map_id= '".$prev."' group by A.register_number order by A.register_number, G.semester, paper_no ";
