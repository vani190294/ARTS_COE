...
use app\models\ExamTimetableIntSearch;
...
use app\models\OdEntryInt;

/**
 * ExamTimetableIntController implements the CRUD actions for ExamTimetableInt model.
 */
class ExamTimetableIntController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    {
        $model = new ExamTimetableInt();
        $searchModel = new ExamTimetableIntSearch();
        ...
        if(isset($_POST['delete'])  && !empty($_POST['finalString']))
        {
          ...
          {     
            ...
            if(!empty($exam_id[$i]))
            {
                $exam_date = ExamTimetableInt::findOne($exam_id[$i]);
                $hall_allo_check = HallAllocateInt::find()->where(['exam_timetable_id'=>$exam_id[$i]])->all();
                $absent_check = AbsentEntryInt::find()->where(['exam_date'=>$exam_date['exam_date']])->all();
          ...
          {             
             $query = (new Query)
                        ->createCommand()
                        ->delete('coe_exam_timetable_int', ['IN','coe_exam_timetable_id',$del_ids])
                        ->execute();
    ...
    {
        $model = new AbsentEntryInt();
        $examTimetable = new ExamTimetableInt();
    ...
    public function actionInternalscorecard()
    {
        $model = new AbsentEntryInt();
        $examTimetable = new ExamTimetableInt();
         $MarkEntry = new MarkEntry();

        return $this->render('internalscorecard', [
            'model' => $model,
            'examTimetable' => $examTimetable,
            'markEntry'=>$MarkEntry
        ]);
    ...
    public function actionInternalscorecardExcel()
    {        
        require(Yii::getAlias('@webroot/includes/use_institute_info.php'));
        $content = $_SESSION['internalscorecard'];
    ...
    public function actionInternalscorecardPdf()
    {
        ...
        $pdf = new Pdf([
            ...
            'mode' => Pdf::MODE_CORE,
            'filename' => 'internalscorecard.pdf',                
            ...
            'cssInline' => ' @media all{
                ...
                ',
            'options' => ['title' => "internalscorecard"],    
            'methods' => [ 
                'SetHeader'=>["OFFICE OF THE CONTROLLER OF EXAMINATIONS "], 
                'SetFooter'=>[ 'INTERNAL EXAM SCORECARD PAGE :{PAGENO}'],
            ]       
        ]);
    ...
    public function actionViewAbPdf()
    {
          ...
          $content .= '<table  style="overflow-x:auto;"  cellspacing="0" cellpadding="0" border="0" id="student_bulk_edit" class="table table-bordered table-responsive bulk_edit_table table-hover" >
                    ...
                    <tr>
                      ...
                      <td colspan=7 align="center"> 
                          ...
                          <center class="tag_line"><b>'.$org_tagline.'</b></center><br>
                          <center class="tag_line"><b>INTERNAL EXAMINATIONS '.$_SESSION['int_number'].' ABSENT LIST</b></center> 
                     </td>
        ...
        else
        {
          ...
          return $this->redirect(['exam-timetable-int/view-absent']); 
        }
    ...
    public function actionAbsent()
    {
        ...
        if(isset($_POST['update']))
        {
            ...
            if($_POST['entry_type']=='Practical' )
            {
                ...
                if(!empty($stu_ab_id))
                {
                    ...
                    { 
                        ...
                        $check_data = "SELECT * FROM coe_absent_entry_int WHERE absent_student_reg='".$stu_ab_id[$i]."' AND exam_type='".$_POST['exam_type']."' AND absent_term='".$_POST['absent_term']."' and exam_month='".$exam_month."' and exam_year='".$exam_year."' AND exam_subject_id='".$subject_map_ab_id." '";
                        ...
                        if(count($available_data)>0)
                        {
                            ...
                            $query_insert = 'INSERT INTO coe_absent_entry_int (`absent_student_reg`,`exam_type`,`absent_term`,`exam_subject_id`,`exam_absent_status`,`exam_month`,`exam_year`,`created_by`,`created_at`,`updated_by`,`updated_at`) VALUES ("'.$stu_ab_id[$i].'","'.$_POST['exam_type'].'","'.$_POST['absent_term'].'","'.$subject_map_ab_id.'","'.$get_cat_entry_type['coe_category_type_id'].'","'.$exam_month.'","'.$exam_year.'","'.$createdBy.'","'.$created_at.'","'.$createdBy.'","'.$created_at.'")';
            ...
            else if ($_POST['entry_type']=='Exam') 
            {
                ...
                for ($i=0; $i <count($stu_ab_id) ; $i++) 
                {
                    ...
                    { 
                        $getExamDetail = ExamTimetableInt::find()->where(['subject_mapping_id'=>$value['coe_subjects_mapping_id'],'exam_date'=>$exam_date,'exam_month'=>$exam_month,'exam_type'=>$_POST['exam_type'],'exam_session'=>$_POST['exam_session']])->one();
                    ...
                    {
                         $check_data = "SELECT * FROM coe_absent_entry_int WHERE absent_student_reg='".$stu_ab_id[$i]."' AND exam_type='".$_POST['exam_type']."' AND absent_term='".$_POST['absent_term']."' AND exam_subject_id =".$subject_map_ab_id." and exam_date='".$exam_date."' and exam_month='".$exam_month."' and exam_session='".$_POST['exam_session']."' and exam_year='".$exam_year."' ";
                        $available_data = Yii::$app->db->createCommand($check_data)->queryAll();
                        ...
                        if(count($available_data)>0)
                        {
                            ...
                            $query_insert = 'INSERT INTO coe_absent_entry_int(`absent_student_reg`,`exam_type`, `exam_year`,`absent_term`,`exam_subject_id`,`exam_absent_status`,`exam_date`,`exam_session`,`exam_month`,`created_by`,`created_at`,`updated_by`,`updated_at`) VALUES ("'.$stu_ab_id[$i].'","'.$_POST['exam_type'].'","'.$exam_year.'","'.$_POST['absent_term'].'","'.$subject_map_ab_id.'","'.$absent_entry_type.'","'.$exam_date.'","'.$_POST['exam_session'].'","'.$exam_month.'","'.$createdBy.'","'.$created_at.'","'.$createdBy.'","'.$created_at.'")';
                            $res[$stu_ab_id[$i]] = Yii::$app->db->createCommand($query_insert)->execute();
                        }
                    }
                    else
            ...
            return $this->redirect(['exam-timetable-int/absent']);
        ...
        {   
                ...
                $year =$_POST['AbsentEntryInt']['exam_year'];
                $exam_month =$model->exam_month;
                
                $ab_locking = ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_ABSENT_LOCKING);
                $last_exam_data = ExamTimetableInt::find()->where(['exam_year'=>$year,'exam_month'=>$exam_month])->orderBy('exam_date DESC')->one();
                $today=date('Y-m-d');
                ...
                if(isset($model->exam_date)  && !empty($model->exam_date))
                {
                    ...
                    $get_exam_month = ExamTimetableInt::find()->where(['exam_date'=>$exam_date])->one();
                ...
                if((stristr($get_exam_type->category_type, "Arrear") || stristr($get_exam_type->description, "Arrear")) && isset($model->exam_date) && empty($model->halls) && !empty($model->exam_date))
                {
                    ...
                    $get_exam_details = ExamTimetableInt::find()->where(['exam_date'=>$exam_date,'exam_session'=>$model->exam_session,'exam_type'=>$model->exam_type,'exam_term'=>$model->absent_term])->andWhere(['IN','subject_mapping_id',$exam_subject_id_array])->one();
                        ...
                        if(stristr($get_type->category_type, "Practical"))
                        {
                           ...
                           $whereCondition = [ 'b.coe_subjects_id'=>$model->exam_subject_id,'e.semester'=>$_POST['AbsentEntryInt']['exam_semester_id']
                            ];
                        }
                        else if(!empty($get_exam_details))
                        {
                            $query->join('JOIN','coe_exam_timetable_int k','k.subject_mapping_id = e.coe_subjects_mapping_id')
                ...
                elseif (stristr($get_type->category_type, "Hall") || stristr($get_type->description, "Hall")) 
                {
                    ...
                    $query->join('JOIN','coe_exam_timetable_int k','k.subject_mapping_id = e.coe_subjects_mapping_id')
                        ->join('JOIN','coe_hall_allocate_int y','y.exam_timetable_id = k.coe_exam_timetable_id and y.register_number=a.register_number')
                ...
                else if ((stristr($get_type->category_type, "Practical") || stristr($get_type->description, "Practical")) && (stristr($get_exam_type->category_type, "Arrear") || stristr($get_exam_type->description, "Arrear")) )  
                {
                    ...
                    $whereCondition = [ 'b.coe_subjects_id'=>$model->exam_subject_id,'e.semester'=>$_POST['AbsentEntryInt']['exam_semester_id'],'e.batch_mapping_id'=>$_POST['AbsentEntryInt']['course_batch_id']
                            ];
                }
                else if ((stristr($get_type->category_type, "Practical") || stristr($get_type->description, "Practical")) )  
                {
                    $status_check = 'Practical-Reg';
                    $check_if_nominal = Nominal::findOne(['course_batch_mapping_id'=>$_POST['AbsentEntryInt']['course_batch_id'],'coe_subjects_id'=>$model->exam_subject_id]);
                ...
                else
                {
                    ...
                    foreach($send_result as $check_pass)
                     {
                        $get_sub_map = SubjectsMapping::find()->where(['subject_id'=>$model->exam_subject_id,'batch_mapping_id'=>$_POST['AbsentEntryInt']['course_batch_id'],'semester'=>$_POST['AbsentEntryInt']['exam_semester_id']])->one();
        ...
        if ($model->load(Yii::$app->request->post())) 
            {            
                ...
                $query->select(['DISTINCT (b.subject_code) as subject_code','b.subject_name','concat(h.degree_name) as degree_name','degree_code','g.programme_name','g.programme_code', 'e.semester','j.category_type as month','i.batch_name','e.batch_mapping_id','e.coe_subjects_mapping_id','k.exam_date','k.exam_year','d.regulation_year','m.category_type as exam_type','n.category_type as exam_session'])
                    ...
                    $whereCondition = [
                        'k.exam_year'=>$_POST['ExamTimetableInt']['exam_year'],
                        'k.exam_month'=>$_POST['ExamTimetableInt']['exam_month'],
                        'i.coe_batch_id'=>$_POST['AbsentEntryInt']['batch_id']
                    ];     
                ...
                {
                    return $this->render('export-exam-timetable-int', [
                ...
                {
                   Yii::$app->ShowFlashMessages->setMsg('ERROR','NO DATA FOUND');
                    return $this->redirect(['export-exam-timetable-int']); 
                }
    ...
    public function actionExternal()
    {
        ...
        { 
            ...
            $section = isset($_POST['AbsentEntryInt']['stu_section_name'])?$_POST['AbsentEntryInt']['stu_section_name']:'';
            ...
            $query = new Query();
                $whereCondition = [                    
                    'e.batch_mapping_id'=>$_POST['AbsentEntryInt']['course_batch_id'],
                    'a.student_status'=>'Active',
                ];
            ...
            if($_POST['AbsentEntryInt']['exam_type']==$exam_type_g)
            {
                    $query->join('JOIN','coe_mark_entry_master l','l.subject_map_id = e.coe_subjects_mapping_id and l.student_map_id=c.coe_student_mapping_id')
                        ->join('JOIN','coe_exam_timetable_int k','k.subject_mapping_id = e.coe_subjects_mapping_id')
                        ->join('JOIN','coe_category_type j','j.coe_category_type_id = k.exam_month');
                    ...
                    $whereCondition = array_merge($whereCondition,[
                        ...
                        'k.exam_type'=>$model->exam_type,
                        'k.exam_term'=>$model->absent_term,
                        'k.exam_year'=>$_POST['AbsentEntryInt']['year'],
                        'k.exam_month'=>$_POST['AbsentEntryInt']['exam_month'],
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) 
        {
            ...
            $same_date->select("B.*")
                ...
                ->join('JOIN','coe_exam_timetable_int B','B.subject_mapping_id=A.coe_subjects_mapping_id')
            ...
            $without_elective_date = Yii::$app->db->createCommand("select * from coe_subjects_mapping as A,coe_exam_timetable_int as B where A.coe_subjects_mapping_id=B.subject_mapping_id and batch_mapping_id='".$batch_map_id."' and exam_date='".$date."' and exam_session='".$model->exam_session."' and subject_type_id!='".$cat_sub_type->coe_category_type_id."'")->queryAll();
    ...
    public function actionUpdate($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) 
        {
            ...
            //$model->qp_code = $_POST['ExamTimetableInt']['qp_code'];
            $model->exam_session = $_POST['ExamTimetableInt']['exam_session'];
            $model->save(false);
    ...
    {
        $exam_date = ExamTimetableInt::findOne($id);
        $checkHallAll = HallAllocateInt::find()->where(['exam_timetable_id'=>$id])->one();
        $AbsentEntryInt = AbsentEntryInt::find()->where(['exam_type'=>$exam_date->exam_type,'exam_date'=>$exam_date->exam_date,'exam_month'=>$exam_date->exam_month,'exam_session'=>$exam_date->exam_session,'exam_subject_id'=>$exam_date->subject_mapping_id])->one();
    ...
    {
        if (($model = ExamTimetableInt::findOne($id)) !== null) {
            return $model;
        } else {
    ...
    public function actionHallWiseAbsent()
    {
        ...
        if (Yii::$app->request->post()) 
        { 
            $exam_year = $_POST['AbsentEntryInt']['year'];
            $exam_month = $_POST['AbsentEntryInt']['month'];
            $exam_date = DATE('Y-m-d',strtotime($_POST['AbsentEntryInt']['exam_date']));
            $exam_session = $_POST['AbsentEntryInt']['exam_session'];
            $halls = $_POST['AbsentEntryInt']['halls'];
            $internal_number = $_POST['ExamTimetableInt']['internal_number'];
            $getData = Categorytype::find()->where(['description'=>'Hall Wise Entry'])->one();
            $reg_nums = ''; $reg_nums1 = ''; $removedreg='';
            if(isset($_POST['stuTotal']))
            {
                ...
                { 
                    ...
                    {
                        $checkExists = AbsentEntryInt::find()->where(['absent_student_reg'=>$stu_map_id,'exam_subject_id'=>$sub_map_id,'exam_type'=>$exam_type,'exam_year'=>$exam_year,'exam_month'=>$exam_month,'internal_number'=>$internal_number])->one();

                        $checkExistsod = OdEntryInt::find()->where(['student_map_id'=>$stu_map_id,'subject_map_id'=>$sub_map_id,'exam_date'=>$exam_date,'exam_type'=>$exam_type,'exam_year'=>$exam_year,'exam_month'=>$exam_month,'internal_number'=>$internal_number])->one();
                        
                        if(!empty($checkExistsod))
                        {
                          
                            $deletedab= Yii::$app->db->createCommand('DELETE FROM coe_od_entry_int WHERE student_map_id='.$stu_map_id.' AND subject_map_id = "'.$sub_map_id.'" AND exam_date = "'.$exam_date.'" AND exam_type='.$exam_type.' AND exam_year='.$exam_year.' AND exam_month='.$exam_month.' AND internal_number='.$internal_number)->execute();                              
                        }

                         $checkExists1 = OdEntryInt::find()->where(['student_map_id'=>$stu_map_id,'subject_map_id'=>$sub_map_id,'exam_type'=>$exam_type,'exam_year'=>$exam_year,'exam_month'=>$exam_month,'internal_number'=>$internal_number])->one();

                        if(empty($checkExists) && empty($checkExists1))
                        {
                            $absent_save = new AbsentEntryInt();
                            ...
                            $absent_save->internal_number = $internal_number;
                    ...
                    {                          
                        $checkExists = AbsentEntryInt::find()->where(['absent_student_reg'=>$stu_map_id,'exam_subject_id'=>$sub_map_id,'exam_date'=>$exam_date,'exam_type'=>$exam_type,'exam_year'=>$exam_year,'exam_month'=>$exam_month,'internal_number'=>$internal_number])->one();
                        if(!empty($checkExists))
                        {
                          
                            $deletedab= Yii::$app->db->createCommand('DELETE FROM coe_absent_entry_int WHERE absent_student_reg='.$stu_map_id.' AND exam_subject_id = "'.$sub_map_id.'" AND exam_date = "'.$exam_date.'" AND exam_type='.$exam_type.' AND exam_year='.$exam_year.' AND exam_month='.$exam_month.' AND internal_number='.$internal_number)->execute();  
                            if($deletedab)
                            {
                                $removedreg.=$_POST['reg_nu_sem_ab_'.$stu_map_id].", ";
                            }
                        }
                    }
                     //Yii::$app->ShowFlashMessages->setMsg('Success','<b>'.$reg_nums.'</b><br /> Added as Absent');

                    if(isset($_POST['od'][$stu_map_id]) && $_POST['od'][$stu_map_id]=='on')
                    {
                        $checkExists = OdEntryInt::find()->where(['student_map_id'=>$stu_map_id,'subject_map_id'=>$sub_map_id,'exam_type'=>$exam_type,'exam_year'=>$exam_year,'exam_month'=>$exam_month,'internal_number'=>$internal_number])->one();

                        $checkExistsab = AbsentEntryInt::find()->where(['absent_student_reg'=>$stu_map_id,'exam_subject_id'=>$sub_map_id,'exam_date'=>$exam_date,'exam_type'=>$exam_type,'exam_year'=>$exam_year,'exam_month'=>$exam_month,'internal_number'=>$internal_number])->one();
                        
                        if(!empty($checkExistsab))
                        {
                        ...
                        $checkExists1 = AbsentEntryInt::find()->where(['absent_student_reg'=>$stu_map_id,'exam_subject_id'=>$sub_map_id,'exam_type'=>$exam_type,'exam_year'=>$exam_year,'exam_month'=>$exam_month,'internal_number'=>$internal_number])->one();

                        if(empty($checkExists) && empty($checkExists1))
                        {
                            $od_save = new OdEntryInt();
                            $od_save->student_map_id = $stu_map_id;
                            $od_save->exam_type = $exam_type;
                            $od_save->absent_term = $absent_term;
                            $od_save->exam_date = $exam_date;
                            $od_save->exam_month = $exam_month;
                            $od_save->exam_session = $exam_session;
                            $od_save->subject_map_id = $sub_map_id;
                            $od_save->exam_status = $absent_status;
                            $od_save->exam_year = $exam_year;
                            $od_save->internal_number = $internal_number;
                            $od_save->created_by = Yii::$app->user->getId();
                            $od_save->created_at = new \yii\db\Expression('NOW()');
                            $od_save->updated_by = Yii::$app->user->getId();
                            $od_save->updated_at = new \yii\db\Expression('NOW()');
                            if($od_save->save(false))
                            {
                                $reg_nums1 .=$_POST['reg_nu_sem_ab_'.$stu_map_id].", ";
                            }
                        }
                    }
                    else
                    {                          
                        $checkExists = OdEntryInt::find()->where(['student_map_id'=>$stu_map_id,'subject_map_id'=>$sub_map_id,'exam_date'=>$exam_date,'exam_type'=>$exam_type,'exam_year'=>$exam_year,'exam_month'=>$exam_month,'internal_number'=>$internal_number])->one();
                        if(!empty($checkExists))
                        {
                ...
                Yii::$app->ShowFlashMessages->setMsg('Success','Absent and OD Updated');
            }
            else
            {
    ...
    public function actionConsolidateOdList()
    {
        $model = new AbsentEntryInt();
        $examTimetable = new ExamTimetableInt();

        Yii::$app->ShowFlashMessages->setMsg('Welcome','Welcome to Consolidate OD List');
        return $this->render('consolidate-od-list', [
            'model' => $model,
            'examTimetable' => $examTimetable,
        ]);
    }

     public function actionConsolidateExcelOdPdf()
    {        
        $content = $_SESSION['consolidate_od_list'];
        $fileName = "CONSOLIDATE OD LIST".date('Y-m-d').'.xls';        
        $options = ['mimeType'=>'application/vnd.ms-excel'];
        return Yii::$app->excel->exportExcel($content, $fileName, $options);
    }

   
    public function actionConsolidateOdPdf()
    {
        ...
        $pdf = new Pdf([
            ...
            'mode' => Pdf::MODE_CORE,
            'filename' => 'CONSOLIDATE OD LIST.pdf',                
            'format' => Pdf::FORMAT_A4,                 
            'orientation' => Pdf::ORIENT_PORTRAIT,                 
            'destination' => Pdf::DEST_BROWSER,                 
            'content' => $content,  
            ...
            'cssInline' => ' @media all{
                ...
                ',
            'options' => ['title' => "CONSOLIDATE OD LIST"],
            'methods' => [ 
                'SetHeader'=>["OFFICE OF THE CONTROLLER OF EXAMINATIONS ".$org_name], 
                'SetFooter'=>[ 'CONSOLIDATE OD LIST PRINTED ON : {DATE d-m-Y H:i:s:A}  PAGE :{PAGENO}'],
            ]
        ]);
    ...
    public function actionHallWiseStusign()
    {
        $model = new AbsentEntryInt();
        $examTimetable = new ExamTimetableInt();
       
        Yii::$app->ShowFlashMessages->setMsg('Welcome','Welcome to Hall Wise Student Attendance Sheet');
        return $this->render('hall-wise-stusign', [
            'model' => $model,
            'examTimetable' => $examTimetable,
        ]);
    ...
    public function actionHallwiseStusignPdf()
    {

        require(Yii::getAlias('@webroot/includes/use_institute_info.php'));
        $content = $_SESSION['hallwisestusign'];
        $pdf = new Pdf([
           
            'mode' => Pdf::MODE_CORE,
            'filename' => 'hallwisestuAttendance.pdf',                
            'format' => Pdf::FORMAT_A4,                 
            'orientation' => Pdf::ORIENT_PORTRAIT,                 
            'destination' => Pdf::DEST_BROWSER,                 
            'content' => $content,  
            //'cssFile' => '@vendor/kartik-v/yii2-mpdf/assets/kv-mpdf-bootstrap.min.css',
            'cssInline' => ' @media all{
                        table{border-collapse: collapse;  font-family:"Roboto, sans-serif"; width:100%; } 
                ...
                ',
            'options' => ['title' => "CIA Exam Hall Wise Student Attendance Sheet"],
            'methods' => [ 
                'SetHeader'=>["OFFICE OF THE CONTROLLER OF EXAMINATIONS, ".strtoupper($org_name)], 
                'SetFooter'=>[ 'CIA EXAM HALL WISE STUDENT ATTENDANCE SHEET PRINTED ON : {DATE d-m-Y h:i:s A}'],
            ]
        ]);
    ...
    public function actionExamDateWiseAbsent()
    {
        ...
        if (Yii::$app->request->post()) 
        { 
            $exam_year = $_POST['AbsentEntryInt']['exam_year'];
            $exam_month = $_POST['AbsentEntryInt']['exam_month'];
            $exam_date = DATE('Y-m-d',strtotime($_POST['AbsentEntryInt']['exam_date']));
            $exam_session = $_POST['AbsentEntryInt']['exam_session'];
            $halls = $_POST['AbsentEntryInt']['exam_subject_id'];
            $getData = Categorytype::find()->where(['description'=>'Hall Wise Entry'])->one();
            $reg_nums = '';
            if(isset($_POST['ab']))
            {
                ...
                { 
                    ...
                    if(isset($_POST['ab'][$stu_map_id]) && $_POST['ab'][$stu_map_id]=='on')
                    {
                        $checkExists = AbsentEntryInt::find()->where(['absent_student_reg'=>$stu_map_id,'exam_subject_id'=>$sub_map_id,'exam_type'=>$exam_type,'exam_year'=>$exam_year,'exam_month'=>$exam_month])->one();
                        if(empty($checkExists))
                        {
                            $absent_save = new AbsentEntryInt();
                            $absent_save->absent_student_reg = $stu_map_id;
    ...
    public function actionNewExportExamTimetable()
    {
        ...
        if(Yii::$app->request->post())  
        {          
                $batch=$_POST['ExamTimetableInt']['coe_batch_id'];
                $year=$_POST['ExamTimetableInt']['exam_year'];
                $month=$_POST['ExamTimetableInt']['exam_month'];
                $exam_session='';//$_POST['ExamTimetableInt']['qp_code'];
                $degree_type=$_POST['ExamTimetableInt']['degree_type'];
                $internal_number=$_POST['ExamTimetableInt']['internal_number'];

                $array= array('1'=>'CIA 1','2'=>'CIA 2','3'=>'Retest CIA 1','4'=>'Retest CIA 2','5'=>'MODEL');    
                $cia= $array[$internal_number];

                $batch_name = Yii::$app->db->createCommand("select batch_name from coe_batch where coe_batch_id='" . $_POST['ExamTimetableInt']['coe_batch_id'] . "'")->queryScalar();
                $month_name = Yii::$app->db->createCommand("select description from coe_category_type where coe_category_type_id='" . $_POST['ExamTimetableInt']['exam_month'] . "'")->queryScalar();
                 
                 $get_sem =0; //ConfigUtilities::SemCaluclation($_POST['ExamTimetableInt']['exam_year'], $month=$_POST['ExamTimetableInt']['exam_month'],$_POST['ExamTimetableInt']['coe_batch_id']); 

                // $degree_code = Yii::$app->db->createCommand("select degree_code from coe_degree where degree_code!='MCA' AND degree_type='" . $_POST['ExamTimetableInt']['degree_type'] . "'")->queryAll();

                 $query_1 = new Query(); 
                    $query_1->select([ 'Distinct (k.exam_date) as exam_date','k.exam_year','d.regulation_year','n.category_type as exam_session','degree_type','g.programme_name','subject_code','subject_name','i.coe_batch_id','e.batch_mapping_id','time_slot' ])
                        ->from('coe_student a')  // If students are available then only data will be exported
                        ->join('JOIN','coe_student_mapping c','c.student_rel_id = a.coe_student_id')
                        ->join('JOIN','coe_bat_deg_reg d','d.coe_bat_deg_reg_id = c.course_batch_mapping_id')   
                        ->join('JOIN','coe_subjects_mapping e','e.batch_mapping_id = d.coe_bat_deg_reg_id')
                    ...
                    $query_1->join('JOIN','coe_exam_timetable_int k','k.subject_mapping_id = e.coe_subjects_mapping_id')
                        ->join('JOIN','coe_category_type j','j.coe_category_type_id = k.exam_month')
                        ->join('JOIN','coe_category_type m','m.coe_category_type_id = k.exam_type')
                        ->join('JOIN','coe_category_type n','n.coe_category_type_id = k.exam_session')
                   ->Where(['k.exam_year'=>$_POST['ExamTimetableInt']['exam_year'],'k.exam_month'=>$_POST['ExamTimetableInt']['exam_month'],'k.internal_number'=>$internal_number]);
                    
                    if(isset($_POST['ExamTimetableInt']['coe_batch_id']) && !empty($_POST['ExamTimetableInt']['coe_batch_id']))
                    {
                         $query_1->andWhere(['d.coe_batch_id'=>$_POST['ExamTimetableInt']['coe_batch_id'],]);
                    }   
                    ...
                    {                        
                         if($degree_type=='B.E')
                        {
                            $query_1->andWhere(['h.degree_type'=>'UG']);
                        }
                        else
                        {
                           $query_1->andWhere(['h.degree_code'=>$degree_type]);
                        }
                    }
                    
                    $query_1->andWhere(['<>', 'status_category_type_id', $det_cat_type])->andWhere(['<>', 'status_category_type_id', $det_disc_type])->andWhere(['NOT IN', 'paper_type_id', $practica])
                    ->andWhere(['=', 'k.internal_number', $internal_number])
                    ->groupBy('k.exam_date')
                    ->orderBy('programme_name,k.exam_date,semester');

                     $query_2 = $query_1->createCommand()->queryAll();
                    //print_r($query_2);exit;
                    
                    $query_1 = new Query(); 
                    $query_1->select([ 'Distinct (k.exam_date) as exam_date','exam_session','time_slot'])
                    ...
                    {                        
                        ...
                        else
                        {
                         ...
                         $query_1->orderBy('k.exam_date,k.exam_session');
                     $query_3 = $query_1->createCommand()->queryAll();
                   //  print_r($query_3);exit;
                     $query = new Query(); 
                    $query->select(['DISTINCT (g.programme_code) as programme_code','g.programme_name','d.regulation_year'])
                        ->from('coe_bat_deg_reg d')  // If students are available then only data will be exported
                        ->join('JOIN','coe_subjects_mapping e','e.batch_mapping_id = d.coe_bat_deg_reg_id')
                        ->join('JOIN','coe_degree h','h.coe_degree_id = d.coe_degree_id')
                        ->join('JOIN','coe_batch i','i.coe_batch_id = d.coe_batch_id')
                        ->join('JOIN','coe_programme g','g.coe_programme_id = d.coe_programme_id');
                    ...
                    $query->join('JOIN','coe_exam_timetable_int k','k.subject_mapping_id = e.coe_subjects_mapping_id');
                    $query->Where(['k.exam_year'=>$_POST['ExamTimetableInt']['exam_year'],'k.exam_month'=>$_POST['ExamTimetableInt']['exam_month'],'k.internal_number'=>$internal_number]);
                    if(isset($_POST['ExamTimetableInt']['coe_batch_id']) && !empty($_POST['ExamTimetableInt']['coe_batch_id']))
                    {
                         $query->andWhere(['d.coe_batch_id'=>$_POST['ExamTimetableInt']['coe_batch_id'],]);
                    }   
                    ...
                    {                        
                         if($degree_type=='B.E')
                        {
                            $query->andWhere(['h.degree_type'=>'UG']);
                        }
                        else
                        {
                           $query->andWhere(['h.degree_code'=>$degree_type]);
                        }
                    }
                    
                    $query->andWhere(['NOT IN', 'paper_type_id', $practica])
                    ->orderBy('programme_name,semester,k.exam_date');
                    //echo $query->createCommand()->getRawSql(); exit;
                    $new_export_exam_time = $query->createCommand()->queryAll();
                //print_r($new_export_exam_time);exit;
                   $query_count = new Query(); 
                    $query_count->select([' DISTINCT (g.programme_code) as programme','IF (degree_code="MBABISEM","",degree_code) as degree_code', 'programme_name','g.coe_programme_id'])
                    ...
                    if(isset($_POST['ExamTimetableInt']['degree_type']) && !empty($_POST['ExamTimetableInt']['degree_type']))
                    {
                        ...
                        {
                           $query_count->andWhere(['h.degree_code'=>$degree_type]);
                        }
                    }

                    $query_count->orderBy('programme_name,k.exam_date');
                    //echo $query_count->createCommand()->getrawsql(); exit;
                    $count_1 =$query_count->createCommand()->queryAll();
                  
                    // print_r($count_1);exit;

                if(isset($new_export_exam_time) && !empty($new_export_exam_time))
                {
                    return $this->render('new-export-exam-timetable', [
                        'model' => $model,
                        'examTimetable' => $examTimetable,
                        'new_export_exam_time'=>$new_export_exam_time, 
                ...
                { 
                   // echo "string"; exit;
                   Yii::$app->ShowFlashMessages->setMsg('ERROR','NO DATA FOUND');
                    return $this->redirect(['new-export-exam-timetable']); 
                }
    ...
    public function actionNewExcelExamtimetable()
    {
        $content =$_SESSION['new_export_exam_time_data'];
        $fileName =ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_EXAM).' Time Table-Printing.xls';        
        $options = ['mimeType'=>'application/vnd.ms-excel'];
        return Yii::$app->excel->exportExcel($content, $fileName, $options);
    }
    ...
    public function actionNewPrintExamtimetable()
    {
        ...
        //unset($_SESSION['student_application_date']);
            $pdf = new Pdf([
                ...
                'mode' => Pdf::MODE_CORE,
                'filename' =>  'Internal Exam Time Table .pdf',                
                'format' => Pdf::FORMAT_LEGAL,                 
                'orientation' => Pdf::ORIENT_LANDSCAPE,                 
                'destination' => Pdf::DEST_BROWSER,                 
                'content' => $content,  
                'cssFile' => '@vendor/kartik-v/yii2-mpdf/assets/kv-mpdf-bootstrap.min.css',
                 'options' => ['title' => 'Internal Exam Time Table '],
                'methods' => [ 
                    'SetHeader'=>["OFFICE OF THE CONTROLLER OF EXAMINATIONS ".$org_name], 
                    'SetFooter'=>['Internal Exam Time Table '.' PRINTED ON : {DATE d-m-Y H:i:s:A}  PAGE :{PAGENO}'],
                ]
