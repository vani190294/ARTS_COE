...
class HallAllocateController extends Controller
{
    ...
    {
        $content = $_SESSION['get_print_valfaculty'];
        $fileName = "Valuation scrutiny Assigned Details " . date('Y-m-d-H-i-s') . '.xls';
        $options = ['mimeType' => 'application/vnd.ms-excel'];
        return Yii::$app->excel->exportExcel($content, $fileName, $options);
    }
    ...
    public function actionPrintRegisterNumbersExcel()
    {
     ...
     public function actionReportAnswerPackets()
    {
        $model = new HallAllocate();
        $examTimetable = new ExamTimetable();
        $absentModel = new AbsentEntry();
        Yii::$app->ShowFlashMessages->setMsg('Welcome','Welcome to Shuffled Answer Packets Alloted');
        return $this->render('report-answer-packets', [
            'model' => $model,
            'examTimetable' => $examTimetable,
            'absentModel' =>$absentModel,
        ]);
    ...
    public function actionReportAnswerScriptsPdf(){
      require(Yii::getAlias('@webroot/includes/use_institute_info.php'));   

        $content = $_SESSION['report_answer_packet'];        
            $pdf = new Pdf([               
                'mode' => Pdf::MODE_CORE,                
                'filename' =>'Report Answer Script.pdf',                
                'format' => Pdf::FORMAT_A4,                
                'orientation' => Pdf::ORIENT_LANDSCAPE,                
                'destination' => Pdf::DEST_BROWSER,                
                'content' => $content,  
                ...
                'options' => ['title' =>"Report Answer Script"],
                'methods' => [
                    'SetHeader' => ["OFFICE OF THE CONTROLLER OF EXAMINATIONS ".$org_name],
                    'SetFooter' => [ "Report Answer Script " . ' PRINTED ON : {DATE d-m-Y H:i:s:A}  PAGE :{PAGENO}'],
                ],
            ]);
        ...
        $headers->add('Content-Type', 'application/pdf');
        return $pdf->render();        
        unset($_SESSION['report_answer_packet']);
    }
    public function actionReportAnswerScriptsExcel()
    {
        $content = $_SESSION['report_answer_packet'];
        $fileName = "Report Answer Script " . date('Y-m-d-H-i-s') . '.xls';
        $options = ['mimeType' => 'application/vnd.ms-excel'];
        return Yii::$app->excel->exportExcel($content, $fileName, $options);
    }

     public function actionPrintDummyNumbers()
    {
        $model = new DummyNumbers();
        $examTimetable = new ExamTimetable();
        $valfaclall = new ValuationFacultyAllocate();
        Yii::$app->ShowFlashMessages->setMsg('Welcome','Welcome to Print Dummy Numbers Report');
        return $this->render('print-dummy-numbers', [
            'model' => $model,
            'examTimetable' => $examTimetable,
            'valfaclall' =>$valfaclall,
        ]);
    }

    public function actionPrintDummyNumbersPdf(){
      require(Yii::getAlias('@webroot/includes/use_institute_info.php')); 
        $content = $_SESSION['get_print_stureg'];        
            $pdf = new Pdf([               
                'mode' => Pdf::MODE_CORE,                
                'filename' =>'Dummy Number Packets.pdf',                
                'format' => Pdf::FORMAT_A4,                
                'orientation' => Pdf::ORIENT_PORTRAIT,                
                'destination' => Pdf::DEST_BROWSER,                
                'content' => $content,  
                ...
                ', 
                'options' => ['title' =>"Dummy Number Packets"],
                'methods' => [
                    'SetHeader' => ["OFFICE OF THE CONTROLLER OF EXAMINATIONS ".$org_name],
                    'SetFooter' => [ "Dummy Number Packets " . ' PRINTED ON : {DATE d-m-Y H:i:s:A}  PAGE :{PAGENO}'],
                ],
            ]);
        ...
        $headers->add('Content-Type', 'application/pdf');
        return $pdf->render();        
        unset($_SESSION['get_print_stureg']);
    }
      ...
      if($_POST['countTo']==$_POST['hallCount']){
        ...
        if($_POST['arrangement_type'] == 'Subject Wise')
        {
            ...
            for($s=0;$s<count($total_subject_galley);$s++){             
              ...
              $query_0 = new Query();              
              $sub_name_check[] = $query_0->select("distinct (a.coe_subjects_mapping_id),a.batch_mapping_id,c.exam_type,d.description")
            ...
            {  
              foreach($sub_name as $subs){
                ...
                if($subs['description']=="Arrear")
                {
                  ...
                  ->where(['b.course_batch_mapping_id'=>$subs['batch_mapping_id'],'a.subject_map_id'=>$subs['coe_subjects_mapping_id'],'a.year_of_passing'=>'','f.student_status'=>'Active','c.exam_date'=>$exam_date,'c.exam_month'=>$exam_month,'c.exam_session'=>$exam->exam_session])->andWhere(['<>', 'status_category_type_id', $det_disc_type])->andWhere(['NOT LIKE', 'result','Pass'] );
                  if(!empty($stu_pass_id))
                  {
                      $query_a->andWhere(['NOT IN', 'student_map_id', $stu_pass_id]);
                  }
                ...
                else if($subs['description']!="Arrear")
                {
                  ...
                  $subject_type = $query_2->select("a.description")
                   ...
                   $getExamTimet = ExamTimetable::find()->where(['exam_date'=>$exam_date,'exam_month'=>$exam_month,'exam_session'=>$exam->exam_session,'subject_mapping_id' =>$subs['coe_subjects_mapping_id']])->all();
                    ...
                    $getHallStu = HallAllocate::find()->where(['month'=>$model->month,'year'=>$model->year])->andWhere(['IN','exam_timetable_id',$examTTIds])->all();
                  ...
                  if($subject_type!='Elective')
                  {
                    ...
                    $subject_student = $query_3->select("f.coe_student_id as stu_id,c.coe_exam_timetable_id")
                      ...
                      ->where(['a.course_batch_mapping_id'=>$subs['batch_mapping_id'],'c.subject_mapping_id' =>$subs['coe_subjects_mapping_id'],'f.student_status'=>'Active','c.exam_date'=>$exam_date,'c.exam_month'=>$exam_month,'c.exam_session'=>$exam->exam_session])->andWhere(['NOT IN', 'status_category_type_id', [$det_cat_type,$det_disc_type]])->andWhere(['NOT IN', 'f.register_number', $removeReg_num])->orderBy('register_number ASC')->createCommand()->queryAll(); 
                    ...
                    $subject_student = $query_4->select("f.coe_student_id as stu_id,d.coe_exam_timetable_id")
                      ...
                      ->where(['abc.course_batch_mapping_id'=>$subs['batch_mapping_id'],'c.coe_subjects_mapping_id' =>$subs['coe_subjects_mapping_id'],'c.semester'=>$getSemester->semester,'a.semester'=>$getSemester->semester,'f.student_status'=>'Active','d.exam_date'=>$exam_date,'d.exam_month'=>$exam_month,'d.exam_session'=>$exam->exam_session])->andWhere(['NOT IN', 'status_category_type_id', [$det_cat_type,$det_disc_type]])->orderBy('register_number ASC')->andWhere(['NOT IN', 'f.register_number', $removeReg_num])->createCommand()->queryAll();
                    ...
                    {                     
                        for($i=1; $i <=$seat_column ; $i++) 
                        {
                          ...
                          if(isset($printData12[$id]['stu_id']))
                          {
                            ...
                            $ex_data = ExamTimetable::findOne($exam_id);
                            ...
                            //$check_sameSeat = HallAllocate::find()->where(['exam_timetable_id'=>$exam_id,'year'=>$model->year,'month'=>$model->month,'row'=>$row,'row_column'=>$column,'seat_no'=>$seat_no])->all();
                           
                            // This is aTemporary Fix for Missing Arrangements 

                            if($missing_entry==='yes')
                            {
                              $getExamTimet = ExamTimetable::find()->where(['exam_date'=>$exam_date,'exam_month'=>$exam_month,'exam_session'=>$exam->exam_session])->all();
                              $examTTIds = array_filter([]);
                              foreach ($getExamTimet as $key => $examIds) {
                                $examTTIds[$examIds['coe_exam_timetable_id']]=$examIds['coe_exam_timetable_id'];
                              }
                              
                              $getHallList = HallAllocate::find()->where(['year'=>$model->year,'month'=>$model->month,'hall_master_id'=>$hall_id->coe_hall_master_id])->andWhere(['IN','exam_timetable_id',$examTTIds])->orderBy('seat_no desc')->one();
        ...
        {     
          ...
          $query_1 = new Query();
          $exam_date =  date("Y-m-d",strtotime($exam->exam_date));
          $sub_name_check = $query_1->select("c.batch_mapping_id,a.subject_mapping_id,a.exam_type,b.description")
            ->from('coe_exam_timetable a')
            ->join('JOIN','coe_category_type b','a.exam_type=b.coe_category_type_id')
            ->join('JOIN','coe_subjects_mapping c','c.coe_subjects_mapping_id=a.subject_mapping_id')
            ...
            foreach($sub_name_check as $subs)
            {
              ...
              if($subs['description']=="Arrear"){
                ...
                $arrear_students = $query_a->select("distinct(f.coe_student_id) as stu_id,b.student_rel_id,c.coe_exam_timetable_id,f.register_number,c.qp_code")
                  ...
                  ->join('JOIN','coe_student f','f.coe_student_id=b.student_rel_id')
                  ->join('JOIN','coe_exam_timetable c','a.subject_map_id=c.subject_mapping_id')
                  ->where(['b.course_batch_mapping_id'=>$subs['batch_mapping_id'],'c.subject_mapping_id'=>$subs['subject_mapping_id'],'a.subject_map_id'=>$subs['subject_mapping_id'],'a.year_of_passing'=>'','f.student_status'=>'Active','c.exam_date'=>$exam_date,'c.exam_month'=>$exam_month,'c.exam_session'=>$exam->exam_session])
                ->andWhere(['<>', 'status_category_type_id', $det_disc_type])
                ->andWhere(['NOT LIKE', 'result','Pass'] )
                ->orderBy('register_number ASC')
                //echo $arrear_students->createCommand()->getrawsql(); exit;
               ...
               array_multisort(array_column($arrear_students, 'register_number'),  SORT_ASC, $arrear_students);
                ...
                if($subject_type!='Elective'){
                  ...
                  $subject_student = $query_3->select("DISTINCT (a.student_rel_id) as stu_id,c.coe_exam_timetable_id,f.register_number,c.qp_code")
                    ...
                    ->join('JOIN','coe_subjects_mapping b','a.course_batch_mapping_id=b.batch_mapping_id')
                    ->join('JOIN','coe_exam_timetable c','c.subject_mapping_id=b.coe_subjects_mapping_id')
                    ->where(['a.course_batch_mapping_id'=>$subs['batch_mapping_id'],'c.subject_mapping_id' =>$subs['subject_mapping_id'],'b.coe_subjects_mapping_id' =>$subs['subject_mapping_id'],'f.student_status'=>'Active','c.exam_date'=>$exam_date,'c.exam_month'=>$exam_month,'c.exam_session'=>$exam->exam_session])
                ...
                array_multisort(array_column($subject_student, 'register_number'),  SORT_ASC, $subject_student);
                  ...
                  $subject_student = $query_4->select("DISTINCT(f.coe_student_id) as stu_id,d.coe_exam_timetable_id,f.register_number,d.qp_code")
                    ...
                    ->join('JOIN','coe_exam_timetable d','d.subject_mapping_id=c.coe_subjects_mapping_id 
                        ...
                        and c.subject_id=a.coe_subjects_id')
                      ///->where(['a.coe_subjects_id' =>$sub_id])->createCommand()->queryAll();
                      ->where(['abcd.course_batch_mapping_id'=>$subs['batch_mapping_id'],'d.subject_mapping_id' =>$subs['subject_mapping_id'],'a.semester'=>$getSemester->semester,'c.semester'=>$getSemester->semester,'c.coe_subjects_mapping_id' =>$subs['subject_mapping_id'],'f.student_status'=>'Active','d.exam_date'=>$exam_date,'d.exam_month'=>$exam_month,'d.exam_session'=>$exam->exam_session])
                      ->andWhere(['<>', 'status_category_type_id', $det_cat_type])
                      ->andWhere(['<>', 'status_category_type_id', $det_disc_type])
                      ->orderBy('register_number ASC')
                      ->createCommand()->queryAll();
...
/*public function actionCreate()   vanitha------
  {        
      require(Yii::getAlias('@webroot/includes/use_institute_info.php'));
      $model = new HallAllocate();
      $hallmaster = new HallMaster();
            ...
            {  
              foreach($sub_name as $subs){
                ...
                if($subs['description']=="Arrear")
                {
                  ...
                  ->join('JOIN','coe_student f','f.coe_student_id=b.student_rel_id')
                  ->join('JOIN','coe_exam_timetable c','a.subject_map_id=c.subject_mapping_id')
                  ->where(['a.subject_map_id'=>$subs['coe_subjects_mapping_id'],'a.year_of_passing'=>'','f.student_status'=>'Active','c.exam_date'=>$exam_date,'c.exam_month'=>$exam_month,'c.exam_session'=>$exam->exam_session])->andWhere(['<>', 'status_category_type_id', $det_disc_type])->andWhere(['NOT LIKE', 'result','Pass'] );
                  if(!empty($stu_pass_id))
                  {
                      $query_a->andWhere(['NOT IN', 'student_map_id', $stu_pass_id]);
                  }
            ...
            foreach($sub_name_check as $subs)
            {
              ...
              if($subs['description']=="Arrear"){
                ...
                $arrear_students = $query_a->select("distinct(f.coe_student_id) as stu_id,b.student_rel_id,c.coe_exam_timetable_id,f.register_number,c.qp_code")
                  ...
                  ->join('JOIN','coe_student f','f.coe_student_id=b.student_rel_id')
                  ->join('JOIN','coe_exam_timetable c','a.subject_map_id=c.subject_mapping_id')
                  ->where(['c.subject_mapping_id'=>$subs['subject_mapping_id'],'a.subject_map_id'=>$subs['subject_mapping_id'],'a.year_of_passing'=>'','f.student_status'=>'Active','c.exam_date'=>$exam_date,'c.exam_month'=>$exam_month,'c.exam_session'=>$exam->exam_session])
                ->andWhere(['<>', 'status_category_type_id', $det_disc_type])
                ->andWhere(['NOT LIKE', 'result','Pass'] )
                ->orderBy('register_number ASC')
                ->createCommand()->queryAll();
  ...
  {        
    ...
    public function actionReprintGalleyArrangement()
    {
        ...
        if ($model->load(Yii::$app->request->post()) ) 
        {
          ...
          $exam_date = $_POST['ExamTimetable']['exam_date'];
          $exam_month = $_POST['HallAllocate']['month'];

            $exam_sn = Yii::$app->db->createCommand("select distinct(a.description) from coe_category_type a,coe_exam_timetable b where a.coe_category_type_id=b.exam_session and b.exam_session='".$exam_session."'")->queryScalar(); 
              ...
              $allocated_value = $query_allocate->select("hall_name,a.register_number,row,row_column,seat_no, c.subject_mapping_id as subject_map_id,coe_student_mapping_id as student_map_id,subject_code,c.exam_date,c.exam_session,c.exam_month,c.exam_year")
                              ...
                              ->join('JOIN','coe_student_mapping e','e.course_batch_mapping_id=d.batch_mapping_id')
                              ->join('JOIN','coe_student f','f.coe_student_id=e.student_rel_id and f.register_number=a.register_number')
                              ->where(['c.exam_date' =>$exam_date,'c.exam_month'=>$exam_month,'c.exam_session'=>$exam_session])
                              ->orderBy('hall_name,seat_no')->createCommand()->queryAll();
    ...
    public function actionHallpdf(){
      require(Yii::getAlias('@webroot/includes/use_institute_info.php'));   
      if(!isset($_SESSION['hall_arrange']))
      {
        Yii::$app->ShowFlashMessages->setMsg('Error','NO DATA FOUND');
                return $this->redirect(['reprint-galley-arrangement']);
      }
        $content = $_SESSION['hall_arrange'];     
            ...
            $pdf = new Pdf([               
                'mode' => Pdf::MODE_CORE,                
                'filename' => 'Galley Arrangement Report.pdf',                
                'format' => Pdf::FORMAT_A4,                
                'orientation' => Pdf::ORIENT_PORTRAIT,                
                'destination' => Pdf::DEST_BROWSER,                
                'content' => $content,  
                'cssFile' => '@vendor/kartik-v/yii2-mpdf/assets/kv-mpdf-bootstrap.min.css',
                'cssInline' => ' @media all{
                       table{font-family:Helvetica Neue, Helvetica, Arial, sans-serif; width:100%; font-size: 12px !important;  }
                        table td{
                            text-align: center;
                            width : 300px;
                            height: 126.8px;
                ...
                ', 
                'options' => ['title' => 'Galley Arrangement Report'],
    ...
    public function actionDeletehallarrangement()
    {
      ...
      if(count($check_subject)>0){
        foreach($check_subject as $sub){
          ...
          } else if (count($check_absent)>0) {
            return 2;           
          } else{            
            foreach($check_subject as $exm_id)
            {
              $getEmaDet = ExamTimetable::findOne($exm_id['coe_exam_timetable_id']);

              $updated_from = ucwords( str_replace('-', ' ', Yii::$app->controller->action->controller->id.' '.Yii::$app->controller->action->id) );
              $data_updated = 'Delete Hall Arrangement For Exam Date '.$getEmaDet['exam_date'].' Exam Session '.$getEmaDet['exam_session'];

              $data_array = ['subject_map_id'=>$getEmaDet['subject_mapping_id'],'updated_link_from'=>$updated_from,'data_updated'=>nl2br($data_updated),'exam_month'=>$month,'exam_year'=>$year,'student_map_id'=>0];            
              $update_track = ConfigUtilities::updateTracker($data_array);


              $delete_dumm = Yii::$app->db->createCommand("delete from coe_dummy_number where exam_date='".$getEmaDet['exam_date']."' and exam_session='".$getEmaDet['exam_session']."' and year='".$year."' and month='".$month."' and subject_map_id='".$getEmaDet['subject_mapping_id']."' ")->execute();
    ...
    public function actionQpcnt(){
      ...
      $qp_code_exam = Yii::$app->db->createCommand('select * from coe_exam_timetable as A JOIN coe_subjects_mapping as B ON B.coe_subjects_mapping_id =A.subject_mapping_id where subject_type_id!='.$getQpPract.' and exam_date="'.$exam_date.'" and exam_session='.$_POST['session'].' and exam_month='.$exam_month.' and exam_year='.$exam_year.' ')->queryAll();
...
// Ends the Subject Wise reports
    ...
    { 
        $model = new MarkEntry();  
        $student = new Student();
        $Practical = Yii::$app->db->createCommand("select coe_category_type_id from coe_category_type where category_type like '%Theory%'")->queryAll();
        $prac_papers = array_filter(['']);

        foreach ($Practical as $key => $value) {
         $prac_papers[$value['coe_category_type_id']] = $value['coe_category_type_id'];
        }
        ...
        if (isset($_POST['get_hall_tickets'])) 
        {
            ...
            $query->groupBy('I.register_number,subject_code');
          ...
          $query1 = new Query();
            ...
            ->andWhere(['<>', 'status_category_type_id', $det_disc_type])
            ->andWhere(['NOT IN','paper_type_id',$prac_papers]) 
            ->andWhere(['between', "A.register_number", $_POST['Student']['register_number_from'],$_POST['Student']['register_number_to']]);
            if($_POST['sec']!='All')
            {
            ...
            $query2->groupBy('A.register_number,subject_code');
      ...
      if ($model->load(Yii::$app->request->post())) 
        {
          ...
          $query->where(['B.year'=>$model->year,'B.month'=>$model->month,'C.exam_date'=>$exam_date ,'C.exam_session'=>$_POST['ExamTimetable']['exam_session']])
                ->andWhere(['<>', 'status_category_type_id', $det_disc_type])
    ...
    public function actionAttendanceSheetPdf(){
        ...
        $content = $_SESSION['attendance_sheet'];
            $pdf = new Pdf([
                ...
                'methods' => [ 
                    'SetHeader'=>["OFFICE OF THE CONTROLLER OF EXAMINATIONS ".$org_name], 
                    'SetFooter'=>['ATTENDANCE SHEET'.' PRINTED ON : {DATE d-m-Y H:i:s:A}  PAGE :{PAGENO}'],
                ]
            ]);
