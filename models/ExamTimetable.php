...
class ExamTimetable extends \yii\db\ActiveRecord
{
     ...
     * @inheritdoc
     */
    public $coe_batch_id,$batch_mapping_id,$semester,$subject_code,$subject_name,$exam_date3;
    public function rules()
    {
        return [
            [['subject_mapping_id', 'exam_month', 'exam_type', 'exam_term', 'exam_date', 'exam_session'], 'required'],
            [['subject_mapping_id', 'exam_year', 'exam_month', 'exam_type','cover_number', 'exam_term', 'exam_session', 'created_by', 'updated_by'], 'safe'],
    ...
    {
        $exam1 = ConfigUtilities::getConfigDesc(ConfigConstants::CONFIG_EXAM);
        $config_list = Categories::find()->where(['category_name' => $exam1])->one();
        $c_id = $config_list['coe_category_id'];
