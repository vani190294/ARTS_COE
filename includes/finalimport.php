...
{ 
    $exam_columns=['A'=>'YEAR','B'=>'MONTH','C'=>'MARK TYPE','D'=>'TERM','E'=>'SEMESTER','F'=>'SUBJECT CODE','G'=>'REGISTER NUMBER','H'=>'CIA OUT OF (100)','I'=>'ESE OUT OF 100'];
    ...
    { 
        ...
        else
        {
            ...
            if(empty($check_cia_marks))
            {
                 ...
                 $ciaMArks_1= $line['H'];
                 $cia_max_1 = $get_sub_info['CIA_max'];
                $cia_marks =  round( ($ciaMArks_1*$cia_max_1/100 ));
                //print_r(  $convert_cia_marks);exit;

                 
                $marks_available = 1;
            } 
            else if(!empty($check_cia_marks))
            {
                $cia_marks = $check_cia_marks['category_type_id_marks'];
                $marks_available = 1;
            } 

            else if(!empty($check_cia_marks_master))
            {
            ...
            $condition_check = $marks_available==1?1:0; $nominal_available = 'NO';
            if($condition_check)
            {  
                ...
                if((!empty($line['I']) || $line['I']==0) && ($line['I']<= ($get_sub_max->CIA_max+$get_sub_max->ESE_max))) // Check if column is not null
                {
                    $ciaMArks = $ese_marks = $line['I']<=0 ? '0': $line['I'];
                          ...
                          foreach ($grade_details as $value) 
                          {
                              if($value['grade_point_to']!='')
                              {
                                  ...
                                  if($total_marks >= $value['grade_point_from'] &&  $total_marks <= $value['grade_point_to'] )
                                  {
                                      ...
                                      {
                                        $result_stu = 'Fail';
                                        $student_res_data = ['result'=>$result_stu,'total_marks'=>$total_marks,'grade_name'=>'RA','grade_point'=>0,'attempt'=>$attempt,'year_of_passing'=>'','ese_marks'=>$convert_ese_marks];        
                                      }      
                                      else
                                      {
                    ...
                    else
                    {
                        ...
                        else if($sub_cat_type->description=="Elective" && $nominal_available == 'YES' && (!empty($ciaMArks) || $line['I']==0) )
                        {
                            $status =1;
                        }
                        else
...
$_SESSION['importResults'] = ['dispResults'=>$dispResults,'totalSuccess'=>$totalSuccess,'result_for'=>'Import Final Marks (Covid-19)'];
?>