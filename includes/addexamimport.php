...
{ 
    $exam_columns=['A'=>'Batch','B'=>'Degree','C'=>'Programme Code','D'=>'Exam Year','E'=>'Month','F'=>'Exam Type','G'=>'Semester','H'=>'Date(MM-DD-YYYY)','I'=>'Session','J'=>'Term','K'=>'Subject Code','L'=>'Q.P. Code','M'=>'Cover_Number'];
...
{    
    ...
    { 
        ...
        $subject = CoeValueSubjects::find()->where(['subject_code'=>$line['K']])->all();   
		
        if(!empty($subject) && !empty($batchMapping))
		{
			foreach ($subject as $valuesss) 
			{
			   $subject_mapping_id = !empty($subject) && !empty($batchMapping)? Sub::findOne(['batch_mapping_id'=>$batchMapping->coe_bat_deg_reg_id,'val_subject_id'=>$valuesss['coe_val_sub_id'],'semester'=>$line['G']]):""; 
        ...
        if(!empty($subject_mapping_id) && !empty($batchMapping))
        {
            ...
            $check_query = "SELECT * FROM sub as A JOIN coe_exam_timetable as B ON B.subject_mapping_id=A.coe_sub_mapping_id WHERE A.batch_mapping_id='".$batchMapping->coe_bat_deg_reg_id."' AND B.exam_date='".$exam_date_conveted."' AND B.exam_session='".$line['I']."'";
            ...
            else
            {
                ...
                $exam->subject_mapping_id = $subject_mapping_id->coe_sub_mapping_id;
                ...
                $exam->cover_number = !empty($line['M']) ? $line['M'] : '';
                $exam->created_at = $created_at;
                $exam->created_by = $created_by;
                $exam->updated_at = $created_at;
                $exam->updated_by = $created_by;

                $check_if_already_inserted = ExamTimetable::find()->where(['subject_mapping_id'=>$subject_mapping_id->coe_sub_mapping_id,'exam_year'=>$line['D'],'exam_month'=>$exam->exam_month,'exam_type'=>$exam->exam_type,'exam_term'=>$exam->exam_term,'exam_session'=>$line['I']])->all();
