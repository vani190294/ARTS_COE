...
{    
    ...
    {      
        ...
        $connection = Yii::$app->db;
            ...
            $check_cia_marks = MarkEntry::find()->where(['student_map_id'=>$student_map_id,'subject_map_id'=>$subject_map_id,'category_type_id'=>$internal_id,'year'=>$year])->orderBy('coe_mark_entry_id DESC')->one();
                     ...
                     { 
                        ...
                        if(!empty($check_mark_entry_master))
                        {
                          ...
                          foreach ($grade_details as $value) 
                          {
                            ...
                            $command2 = $connection->createCommand('UPDATE coe_mark_entry_master SET CIA="'.$ciaMArks.'",ESE="'.$student_res_data['ese_marks'].'",total="'.$total_marks.'",result="'.$student_res_data['result'].'",grade_point="'.$student_res_data['grade_point'].'",grade_name="'.$student_res_data['grade_name'].'",updated_by="'.$created_by.'",updated_at="'.$created_at.'",year_of_passing="'.$year_of_passing.'" WHERE coe_mark_entry_master_id="'.$check_mark_entry_master->coe_mark_entry_master_id.'" ');
                            if($command2->execute())
                            {
                                $totalSuccess+=1;
