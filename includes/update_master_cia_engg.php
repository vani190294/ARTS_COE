                ...
                if(count($check_cia_marks)>0 && !empty($check_cia_marks) && $status_check=='YES')
                {
                    ...
                    if($command1->execute())
                     { 
                        ...
                        $check_mark_entry_mastertemp = MarkEntryMasterTemp::find()->where(['student_map_id'=>$student_map_id,'subject_map_id'=>$subject_map_id,'mark_type'=>$mark_type,'term'=>$term,'year'=>$year,'month'=>$month])->one();
                        $attempt = 1;
                        if(!empty($check_mark_entry_mastertemp))
                        {
                           ...
                           // $command2 = $connection->createCommand('UPDATE coe_mark_entry_master SET CIA="'.$ciaMArks.'",ESE="'.$student_res_data['ese_marks'].'",total="'.$student_res_data['total_marks'].'",result="'.$student_res_data['result'].'",grade_point="'.$student_res_data['grade_point'].'",grade_name="'.$student_res_data['grade_name'].'",updated_by="'.$created_by.'",updated_at="'.$created_at.'",year_of_passing="'.$year_of_passing.'" WHERE coe_mark_entry_master_id="'.$check_mark_entry_master->coe_mark_entry_master_id.'" ');
                            ...
                            $command2 = $connection->createCommand('UPDATE coe_mark_entry_master_temp SET CIA="'.$ciaMArks.'",ESE="'.$student_res_data['ese_marks'].'",total="'.$student_res_data['total_marks'].'", updated_by="'.$created_by.'",updated_at="'.$created_at.'" WHERE coe_mark_entry_master_id="'.$check_mark_entry_mastertemp->coe_mark_entry_master_id.'" ');

                            if($command2->execute() && !empty($check_mark_entry_master))
                            {
                                $command3 = $connection->createCommand('UPDATE coe_mark_entry_master SET CIA="'.$ciaMArks.'",ESE="'.$student_res_data['ese_marks'].'",total="'.$student_res_data['total_marks'].'", updated_by="'.$created_by.'",updated_at="'.$created_at.'" WHERE coe_mark_entry_master_id="'.$check_mark_entry_master->coe_mark_entry_master_id.'" ')->execute();
                                $totalSuccess+=1;
                                $dispResults[] = array_merge($line, ['type' => 'S',  'message' => 'Success']);
                            }
                            else{ 
                                $dispResults[] = array_merge($line, ['type' => 'E', 'message' => 'Temp Table updated Master table not Updated']);
                             }
                        }
                        else
                        {
                            $totalSuccess+=1;
                            $dispResults[] = array_merge($line, ['type' => 'E',  'message' => 'Cia Marks Updated but main table not updated']);
                        }
                     }
                     else{ 
