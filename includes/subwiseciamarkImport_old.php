    ...
    {      
        ...
        else
        {
            ...
            {         
                ...
                $cia_marks_condition = $line['G']==0?1:$line['G'];
                $get_sub_max = Subjects::findOne($subs_id->subject_id);
                if((!empty($cia_marks_condition) || $cia_marks_condition==1  ) && $cia_marks_condition<=$get_sub_max->CIA_max) // Check if column is not null
                {
                    ...
                    else if (!empty($check_mark_entry) && $get_sub_info['ESE_max']==0 && $get_sub_info['ESE_min']==0) 
                    {
                        ...
                        if($MarkEntry1->save(false)) 
                            {
                                $update_stu_mark = Yii::$app->db->createCommand("update coe_mark_entry_master set CIA='" . $ciaMArks . "',total='" . $stu_result_data['total_marks'] . "',result='".$stu_result_data['result']."',grade_point='" . $stu_result_data['grade_point'] . "',updated_by='".$updated_by."',updated_at='".$updated_at."',grade_name='" . $min_grade['grade_name'] . "',year_of_passing='" . $year_of_passing . "' where subject_map_id='" . $subject_map_id . "' and student_map_id='" . $student_map_id. "' and year='" . $year . "' and month='" . $month . "' and mark_type='".$_mark_type."' and term='".$term."'")->execute();
                                $succesS++;
                            }                        
                            else if(empty($check_mark_entry_cia) && $status==1)
                            { 
                                $totalSuccess+=1;
                                $dispResults[] = array_merge($line, ['type' => 'S',  'message' => 'Success']);
                            }
                    ...
                    {   
                        ...
                        if(empty($check_mark_entry_cia) && $MarkEntry1->save(false))
                        { 
                            $totalSuccess+=1;
                            $dispResults[] = array_merge($line, ['type' => 'S',  'message' => 'Success']);
                        }
