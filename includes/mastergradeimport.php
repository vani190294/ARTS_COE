            ...
            {  
                ...
                if((!empty($line['H']) && $line['H']!=0))
                {
                    ...
                    $check_mark_entry = MarkEntry::find()->where(['student_map_id'=>$student_map_id,'subject_map_id'=>$subject_map_id,'mark_type'=>$mark_type,'category_type_id'=>46,'term'=>$term,'year'=>$year,'month'=>$month])->one();
                     ...
                     $check_mark_entry1 = MarkEntry::find()->where(['category_type_id'=>89,'subject_map_id'=>$subject_map_id,'student_map_id'=>$student_map_id,'year'=>$year,'month'=>$month,'term'=>$term,'mark_type'=>$mark_type])->one();

                    $markEntry = new MarkEntry();
                    $markEntry->student_map_id = $student_map_id;
                    $markEntry->subject_map_id = $subject_map_id;
                    $markEntry->category_type_id =89;
                    $markEntry->category_type_id_marks =$line['H'];
                    $markEntry->year = $year;
                    ...
                    else if(empty($check_mark_entry1) && $markEntry->save(false))
                    {
                        $subject_code1 = isset($line['F'])?Subjects::find()->where(['subject_code'=>$line['F']])->one():"";
                        //echo $subject_code1->ESE_max;exit;
                        ...
                        $MarkEntryMaster->total = ($stu_result_data['ese_marks']+$check_mark_entry->category_type_id_marks);
                        ...
                        if($MarkEntryMaster->save(false))
                        {
                            ...
                            $totalSuccess+=1;
                            $dispResults[] = array_merge($line, ['type' => 'S',  'message' => 'Successfully Inserted']);
                        }
                        else
                        {
