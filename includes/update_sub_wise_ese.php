            ...
            if(count($check_cia_marks)>0 && !empty($check_cia_marks) && $status_check=='YES')
            {
                ...
                if(!empty($check_mark_entry_master))
                {
                  ...
                  foreach ($grade_details as $value) 
                  {
                    ...
                    {
                        $command1 = $connection->createCommand('UPDATE coe_mark_entry SET category_type_id_marks="'.$ese_marks.'",updated_by="'.$created_by.'",updated_at="'.$created_at.'" WHERE coe_mark_entry_id="'.$check_ese_marks->coe_mark_entry_id.'" ');
                        $command1->execute();
                    }

                    $command2 = $connection->createCommand('UPDATE coe_mark_entry_master SET CIA="'.$ciaMArks.'",ESE="'.$student_res_data['ese_marks'].'",total="'.$insert_total.'",result="'.$student_res_data['result'].'",grade_point="'.$student_res_data['grade_point'].'",grade_name="'.$student_res_data['grade_name'].'",updated_by="'.$created_by.'",updated_at="'.$created_at.'",is_updated="YES",year_of_passing="'.$year_of_passing.'" WHERE coe_mark_entry_master_id="'.$check_mark_entry_master->coe_mark_entry_master_id.'" ');
