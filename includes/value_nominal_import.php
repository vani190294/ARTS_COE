...
{    
    ...
    { 
        ...
        $subjects_LIST = CoeValueSubjects::find()->where(['subject_code'=>$line['F']])->all();

        if(!empty($subjects_LIST) && count($subjects_LIST)>1)
        {
            foreach ($subjects_LIST as $key => $value) 
            {
                $subjectsMapp = Sub::find()->where(['val_subject_id'=>$value['coe_val_sub_id'],'batch_mapping_id'=>$batchMapping->coe_bat_deg_reg_id,'semester'=>$line['E']])->one();
                if(!empty($subjectsMapp))
                {
                    $subjects = CoeValueSubjects::findOne($subjectsMapp->subject_id);
                    break;
                }
            }
        }
        ...
        {
            $subjects = CoeValueSubjects::find()->where(['subject_code'=>$line['F']])->one();
        }
        ...
        {
            $subjectsMap = Sub::find()->where(['val_subject_id'=>$subjects->coe_val_sub_id,'subject_type_id'=>$stu_elective])->one();
            if(!empty($subjectsMap))
            {
                ...
                $nominal->coe_subjects_id = $subjects->coe_val_sub_id;
                $nominal->created_by = $created_by;
                $nominal->updated_by = $created_by;
                $nominal->created_at = $created_at;
                $nominal->updated_at = $created_at;
                $checkNominal = CoeValueNominal::find()->where(['coe_student_id'=>$student->coe_student_id,'coe_subjects_id'=>$subjects->coe_val_sub_id,'course_batch_mapping_id'=>$batchMapping->coe_bat_deg_reg_id])->all();
