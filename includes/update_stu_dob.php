...
{    
     $stu_columns=['A'=>'Roll no','B'=>'Dob(mm-dd-yyyy)','C'=>'Name','D'=>'Parent / Guardian Mobile Number','E'=>'Email id','F'=>'Student mobile number','G'=>'Aadhar Number','H'=>'Religion','I'=>'Blood group','J'=>'Caste','K'=>'Sub Caste','L'=>'Nationality','M'=>'Guardian Name','N'=>'DOJ(mm-dd-yyyy)'];
...
{    
    ...
    { 
        ...
        $date_of_join = isset($line['N'])?date("Y-m-d", PHPExcel_Shared_Date::ExcelToPHP($line['N'])):""; 
        $update_status = 0;
        if(!empty($check_stu))
         {  
            ...
            if(isset($line['N']) &&!empty($line['N']) )
            {

  $command = $connection->createCommand('UPDATE coe_student SET admission_date="'.$date_of_join.'",updated_by="'.$createdBy.'",updated_at="'.$created_at.'" WHERE register_number="'.$line['A'].'" ');
                $command->execute();
                $update_status = 1;
