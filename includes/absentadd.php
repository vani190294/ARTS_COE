...
{    
    ...
    $subject_code = isset($line['F'])?CoeValueSubjects::find()->where(['subject_code'=>$line['F']])->all():"";
    ...
    { 
        ...
        else
        {
            ...
            if(count($subject_code>0))
            {
                ...
                {
                    $sub_map_id_get = Sub::find()->where(['val_subject_id'=>$sub_map['coe_val_sub_id'],'batch_mapping_id'=>$stu_mapping->course_batch_mapping_id,'semester'=>$line['E']])->one();
            ...
            {
                $subject_map_id = $this->valueReplace($line['F'], Sub::getSubjectMappingId($stu_mapping->course_batch_mapping_id,$line['E']));
            }
            
            $get_sub_info = Yii::$app->db->createCommand('SELECT A.* FROM coe_value_subjects as A JOIN Sub as B ON B.val_subject_id=A.coe_val_sub_id WHERE coe_sub_mapping_id="'.$subject_map_id.'" and semester="'.$line['E'].'" ')->queryOne();
            ...
            $check_cia_marks = InternalMarkEntry::find()->where(['student_map_id'=>$student_map_id,'subject_map_id'=>$subject_map_id,'category_type_id'=>$internal_id])->one();   
            
            $check_cia_marks_master = CoeValueMarkEntry::find()->where(['student_map_id'=>$student_map_id,'subject_map_id'=>$subject_map_id])->one();

            $config_attempt = ConfigUtilities::getConfigValue(ConfigConstants::CONFIG_CIA_ZEO);
            $check_attempt = Yii::$app->db->createCommand('SELECT count(*) FROM coe_value_mark_entry WHERE subject_map_id="' . $subject_map_id . '" AND student_map_id="' . $student_map_id . '" AND result not like "%pass%" ')->queryScalar();
            ...
            if($condition_check==1 || $condition_check==0)
            {  
                $subs_id = Sub::findOne($subject_map_id);
                $sub_cat_type = Categorytype::findOne($subs_id->subject_type_id);

                $pquery='SELECT * FROM coe_add_exam_timetable WHERE subject_mapping_id = "'.$subject_map_id.'" and exam_month="'.$month.'" AND exam_year="'.$year.'" and exam_type='.$mark_type;

                $prev_examAllDet=Yii::$app->db->createCommand($pquery)->queryone();

                //print_r($prev_examAllDet['exam_date']); exit;
                ...
                if($sub_cat_type->description=="Elective" )
                {
                    ...
                    ->from('coe_value_nominal')                                
                    ->where(["coe_student_id"=>$stu_tab_id->student_rel_id,'coe_subjects_id'=>$subs_id->val_subject_id,'semester'=>$line['E']]);
                    $check_nominal = $check_nominal_query->createCommand()->queryAll();
                    $nominal_available = 'YES';
                }
                
                $get_sub_max = $subject_details = CoeValueSubjects::findOne($subs_id->val_subject_id);
                ...
                if((!empty($line['H']) || $line['H']==0) && ($line['H']<= ($get_sub_max->CIA_max+$get_sub_max->ESE_max))) // Check if column is not null
                {
                    ...
                    $check_mark_entry = CoeValueMarkEntry::find()->where(['student_map_id'=>$student_map_id,'subject_map_id'=>$subject_map_id,'mark_type'=>$mark_type,'term'=>$term,'year'=>$year,'month'=>$month])->all();
                    $check_pass = CoeValueMarkEntry::find()->where(['student_map_id'=>$student_map_id,'subject_map_id'=>$subject_map_id,'result'=>'Pass'])->one();
                    ...
                    else
                    {
                        ...
                        $check_mark_entry_ESE = InternalMarkEntry::find()->where(['student_map_id'=>$student_map_id,'subject_map_id'=>$subject_map_id,'category_type_id'=>$external_id,'year'=>$year,'month'=>$month,'mark_type'=>$mark_type,'term'=>$term])->all();
                        
                        $check_ese_marks_dum = InternalMarkEntry::find()->where(['student_map_id'=>$student_map_id,'subject_map_id'=>$subject_map_id,'category_type_id'=>$exe_cat_id_2->coe_category_type_id,'year'=>$year,'month'=>$month,'mark_type'=>$mark_type,'term'=>$term])->one();

                        $check_mark_entry_master_ESE = CoeValueMarkEntry::find()->where(['student_map_id'=>$student_map_id,'subject_map_id'=>$subject_map_id,'year'=>$year,'month'=>$month,'mark_type'=>$mark_type,'term'=>$term])->all();

                        $getAbsentList = AbsentAdd::find()->where(['exam_type'=>$mark_type,'absent_term'=>$term,'exam_subject_id'=>$subject_map_id,'exam_month'=>$month,'exam_year'=>$year,'absent_student_reg'=>$student_map_id])->all();
                        ...
                        $absentInsert->exam_date =$prev_examAllDet['exam_date'];
                        $absentInsert->exam_session =$prev_examAllDet['exam_session'];
                        ...
                        $CoeValueMarkEntry = new CoeValueMarkEntry();  
                        $CoeValueMarkEntry->student_map_id = $student_map_id;
                        $CoeValueMarkEntry->subject_map_id = $subject_map_id;
                        $CoeValueMarkEntry->CIA = ($cia_marks==0 || $cia_marks=='')?0:$cia_marks;
                        $CoeValueMarkEntry->ESE = 0;
                        $CoeValueMarkEntry->total = ($cia_marks==0 || $cia_marks=='')?0:$cia_marks;
                        $CoeValueMarkEntry->result = 'Absent'; // For Data Migration
                        $CoeValueMarkEntry->grade_point = 0;
                        $CoeValueMarkEntry->grade_name ='U';
                        $CoeValueMarkEntry->year = $year;
                        $CoeValueMarkEntry->month = $month; 
                        $CoeValueMarkEntry->term = $term;
                        $CoeValueMarkEntry->mark_type = $mark_type;
                        $CoeValueMarkEntry->status_id = 0; // For Data Migration only
                        $CoeValueMarkEntry->year_of_passing = ''; 
                        $CoeValueMarkEntry->attempt = $student_res_data['attempt'];
                        $CoeValueMarkEntry->created_by = $created_by;
                        $CoeValueMarkEntry->updated_by = $created_by;
                        $CoeValueMarkEntry->created_at = $created_at;
                        $CoeValueMarkEntry->updated_at = $created_at;   
                        
                        if(empty($getAbsentList))
                        {
                           if($absentInsert->save(false) && $MarkEntryMaster->save(false) )
                            {
                                $totalSuccess+=1;
            ...
            else
            {
                ...
                {
                    $dispResults[] = array_merge($line, ['type' => 'E',  'message' => 'NO DATA FOUND PLEASE CHECK CIA MARK']);
                }
                else
                {
    ...
    {                    
        $dispResults[] = array_merge($line, 
            ['type' => 'E',  'message' => 'Data submision is worng11']);
