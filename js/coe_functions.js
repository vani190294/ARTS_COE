...
function resetHalls(){
  if(confirm("Are you sure to reset hall arrangement")){
    ...
    $.ajax({
      ...
      type:'POST',
      data:{year:year,month:month,date:date,session:session,exam_type:exam_type},
      success:function(data){
        if(data==1){
...
function resetHalls1(){
  if(confirm("Are you sure to reset hall arrangement")){
    var year = $('#exam_year').val();
    var month = $('#exam_month_add').val();
    var date = $('#exam_date_add').val();
    var session = $('#exam_session').val();
    $.ajax({
      
      url : base_url+'?r=coe-add-hall-allocate/deletehallarrangement',
      type:'POST',
...
$('#exam_month3').on('change',function(){

    var year = $('#hallallocate-year').val();
    var month = $('#exam_month3').val();

    $.ajax({
        url: base_url+'?r=ajaxrequest/getqpexamdate3',
        type:'POST',
        ...
        success:function(data){       
            ...
            var parsed = $.parseJSON(data);
            var old_ex_date = '';
            $('#exam_date3').html('<option value="" >----Select----</option>');
            $.each(parsed,function(i,parsed)
            {
              ...
              if(old_ex_date!=parsed['exam_date'])
              {
                ...
                $('#exam_date3').append("<option value='"+parsed['exam_date']+"' >"+parsed['exam_date']+"</option>");
              }
    ...
    $.ajax({
        ...
        success:function(data){       
            ...
            $.each(parsed,function(i,parsed)
            {
              ...
              {
                old_ex_date=parsed['exam_date'];
                $('#semester').append("<option value='"+parsed['exam_date']+"' >"+parsed['exam_date']+"</option>");
              }
...
$('#exam_month1').on('change',function(){

    var year = $('#hallallocate-year').val();
    var month = $('#exam_month1').val();

    $.ajax({
        url: base_url+'?r=ajaxrequest/getaddqpexamdate',
        type:'POST',
        data:{year:year,month:month},
        success:function(data){       
            $('#exam_date').html('');                    
            ...
            $.each(parsed,function(i,parsed)
            {
              ...
              {
                old_ex_date=parsed['exam_date'];
                $('#exam_date1').append("<option value='"+parsed['exam_date1']+"' >"+parsed['exam_date']+"</option>");
              }
...
$('#exam_date1').on('change',function(){
    ...
    var date = $('#exam_date').val();    
    $.ajax({
        url: base_url+'?r=ajaxrequest/getaddqpsession',
        type:'POST',
        data:{date:date},
        success:function(data){               
            var parsed = $.parseJSON(data);
            $('#exam_session1').html('');
            $.each(parsed,function(i,parsed){
                $('#exam_session1').append("<option value='"+parsed['coe_category_type_id']+"'>"+parsed['description']+"</option>");
            });
        }
    });
});
...
$('#exam_type').on('change',function(){
    ...
    if(hall_arrangement=="Non-Subject Wise"){               
        ...
        $.ajax({
            ...
            type:'POST',
            data:{method:method,date:date,session:session,exam_month:exam_month,exam_year:exam_year,exam_type:exam_type},
            success:function(data){                 
                var parsed = $.parseJSON(data); 
...
$('#qpsubmit').on('click',function(){
   ...
   $.ajax({
        ...
        type:'POST',
        data:{date:date,session:session,exam_year:exam_year,exam_month:exam_month,exam_type:exam_type},
        success:function(data){
          if(data==0)
          {
            krajeeDialog.alert("No Data Found");
...
function show(exam_year,exam_month,exam_date,exam_session,exam_type)
{
    $.ajax({
        type:'POST',
        url: base_url+"?r=ajaxrequest/getconsolidateablistcover",
        data: {
            
            exam_year: exam_year,exam_month: exam_month,exam_date: exam_date,exam_session:exam_session,exam_type:exam_type
...
$('#exam_type1').on('change',function(){
    ...
    //alert(batch_id+"---"+year+"---"+month+"---"+mark_type);
    $.ajax({
      url: base_url+'?r=mark-entry/crseanalysissubcode1',
      type:'POST',
      data:{year:year,month:month,batch_id:batch_id,mark_type:mark_type},
      success:function(data){
        var parsed = $.parseJSON(data);
        $.each(parsed,function(i,parsed){
          $('#mark_subject_code').append("<option value="+parsed['coe_val_sub_id']+">"+parsed['subject_code']+"</option>");
        });
      }
    });
});
...
$('#hallticket_export').on('click',function(){
  ...
  else
  {
    $.ajax({
      ...
      type:'POST',
      data:{year:year,month:month,check_val:check_val,batch_id:batch_id,semester:semester_val,exam_type:exam_type},
      success:function(data)
      {
        if(data!=0)
        {
...
$('#hallticket_export1').on('click',function(){
  var year = $('#mark_year').val();
  var batch_id = $('#stu_batch_id_selected').val();
  var month = $('#mark_month').val();
  var semester_val = $('#semester_val').val();
  ...
  {
    $.ajax({
      url: base_url+'?r=coe-add-hall-allocate/add-hall',
      type:'POST',
...
$('#university_btn').on('click',function(){
  ...
  else
  {
    $.ajax({
      ...
      type:'POST',
       data:{batch:batch,bat_map_id:bat_map_id,mark_year:mark_year,exam_month:exam_month,exam_term:exam_term},
      success:function(data)
      {
        if(data!=0)
        {
...
$('#myclassroom_btn').on('click',function(){
  ...
  else
  {
    $.ajax({
      ...
      type:'POST',
      data:{year:year,month:month,batch_id:batch_id,batch_map_id:batch_map_id},
      success:function(data)
      {
        if(data!=0)
        {
...
$('#myclassroom_btn2').on('click',function(){
  var year = $('#class_year').val();
  var month = $('#class_month').val();
  var degree ='UG';// $('#degree_name').val();
  var batch_id = $('#stu_batch_id_selected').val();
  ...
  {
    $.ajax({
      url: base_url+'?r=mark-entry/myclassroomdata2',
      type:'POST',
...
$('#ans_batch_id').on('change',function()
{
    $('#ans_exam_month').html('');
    $('#ans_exam_date').html('');
    $('#ans_exam_session').html('');
    $('#ans_subject_code').html('');
    $('#ans_pack_serial').val('');
    var batch_id = $('#ans_batch_id').val();
    $.ajax({
        url: base_url+'?r=ajaxrequest/getqpmonth',
        type:'POST',
        data:{batch_id:batch_id},
            
        success:function(data)
        {
            var exam_month_dropdown = $("#ans_exam_month");
                exam_month_dropdown.html(data);
        }
    });
...
$('#ans_exam_month').on('change',function(){

    var batch_id = $('#ans_batch_id').val();
    var year = $('#ans_exam_year').val();
    var month = $('#ans_exam_month').val();
    $('#ans_exam_date').html('');
    $.ajax({
        url: base_url+'?r=ajaxrequest/getqpexamdate1',
        type:'POST',
        data:{batch_id:batch_id,year:year,month:month},
        success:function(data){       
            $('#ans_exam_date').html('');                    
            var parsed = $.parseJSON(data);
            var old_ex_date = '';
            $('#ans_exam_date').html('<option value="" >----Select----</option>');
            $.each(parsed,function(i,parsed)
            {
              ...
              {
                old_ex_date=parsed['exam_date'];
                $('#ans_exam_date').append("<option value='"+parsed['exam_date']+"' >"+parsed['exam_date']+"</option>");
              }
...
$('#ans_exam_month1').on('change',function(){

    var batch_id = $('#ans_batch_id').val();
    var year = $('#ans_exam_year').val();
    var month = $('#ans_exam_month1').val();
    $('#ans_exam_date').html('');
    $.ajax({
        url: base_url+'?r=ajaxrequest/getaddqpexamdate1',
        type:'POST',
        data:{batch_id:batch_id,year:year,month:month},
        success:function(data){       
            $('#ans_exam_date1').html('');                    
            var parsed = $.parseJSON(data);
            var old_ex_date = '';
            $('#ans_exam_date1').html('<option value="" >----Select----</option>');
            $.each(parsed,function(i,parsed)
            {
              ...
              {
                old_ex_date=parsed['exam_date'];
                $('#ans_exam_date1').append("<option value='"+parsed['exam_date']+"' >"+parsed['exam_date']+"</option>");
              }
...
$('#ans_exam_date').on('change',function(){

    $('#ans_exam_session').html('');
     $('#ans_pack_serial').val('');
    var date = $('#ans_exam_date').val();    
    $.ajax({
        url: base_url+'?r=ajaxrequest/getqpsession',
        type:'POST',
        data:{date:date},
        success:function(data){               
            var parsed = $.parseJSON(data);
             ...
             $('#ans_exam_session').html("<option value=''>-----Select----</option>");
            $.each(parsed,function(i,parsed){
                $('#ans_exam_session').append("<option value='"+parsed['coe_category_type_id']+"'>"+parsed['description']+"</option>");
            });
        }
    });
});
$('#ans_exam_date1').on('change',function(){

    $('#ans_exam_session1').html('');
     $('#ans_pack_serial').val('');
    var date = $('#ans_exam_date1').val();    
    $.ajax({
        url: base_url+'?r=ajaxrequest/getaddqpsession',
        type:'POST',
        data:{date:date},
        success:function(data){               
            var parsed = $.parseJSON(data);
             ...
             $('#ans_exam_session1').html("<option value=''>-----Select----</option>");
            $.each(parsed,function(i,parsed){
                $('#ans_exam_session1').append("<option value='"+parsed['coe_category_type_id']+"'>"+parsed['description']+"</option>");
            });
        }
    });
});

$('#ans_exam_session').on('change',function(){
    ...
    var month = $('#ans_exam_month').val();
    var date = $('#ans_exam_date').val(); 
    var session = $('#ans_exam_session').val();

    $.ajax({
        url: base_url+'?r=ajaxrequest/getqpsubject',
        type:'POST',
        data:{batch_id:batch_id,year:year,month:month,date:date,session:session},
        success:function(data){               
            var parsed = $.parseJSON(data);
           
             $('#ans_qp_code').html("<option value=''>-----Select----</option>");
            $.each(parsed,function(i,parsed){
                $('#ans_qp_code').append("<option value='"+parsed['qp_code']+"'>"+parsed['qp_code']+"</option>");
            });
        }
    });
});
$('#ans_exam_session1').on('change',function(){
    ...
    var month = $('#ans_exam_month1').val();
    var date = $('#ans_exam_date1').val(); 
    var session = $('#ans_exam_session1').val();

    $.ajax({
        url: base_url+'?r=ajaxrequest/getaddqpsubject',
        type:'POST',
        data:{batch_id:batch_id,year:year,month:month,date:date,session:session},
        success:function(data){               
            var parsed = $.parseJSON(data);
           
             $('#ans_qp_code1').html("<option value=''>-----Select----</option>");
            $.each(parsed,function(i,parsed){
                $('#ans_qp_code1').append("<option value='"+parsed['qp_code']+"'>"+parsed['qp_code']+"</option>");
            });
        }
    });
});
...
$('#answer_packets_btn').on('click',function()
{
    ...
    if($('#ans_batch_id').val()=='' || $('#ans_exam_year').val()=='' || $('#ans_exam_month').val()=='')
    {
        krajeeDialog.alert('Please Select Batch, Year and month');
        spinneroff();
        return false;
    }
    else if(ans_pack_script<40 || ans_pack_script>60)
    {
        krajeeDialog.alert('COVER ANSWER SCRIPT MIN 40 MAX 60');
        spinneroff();
        return false;
    }
    else
    {
        $.ajax({
            ...
            type:'POST',
            data:{batch_id:$('#ans_batch_id').val(),exam_date:$('#ans_exam_date').val(), exam_year:$("#ans_exam_year").val(), 
            exam_month:$("#ans_exam_month").val(),
            exam_session:$('#ans_exam_session').val(), ans_pack_script:$('#ans_pack_script').val(),exam_type:$('#exam_type').val()},
            success:function(data)
            {            
                spinneroff();
                //var parsed = JSON.parse(data);
                ...
                if(parsed==1)
                {
                    krajeeDialog.alert('PREVIOUS DATE EXAM ANSWER PACKET NOT COMPLETED');
                    $('#answer_packets_div').hide();
                    $('#answer_packets').hide();
                    $('#answer_packets').html('');
                }
...
$('#answer_packets_btn_add1').on('click',function()
{
    

    if($('#ans_batch_id').val()=='' || $('#ans_exam_year').val()=='' || $('#ans_exam_month1').val()=='')
    {
        krajeeDialog.alert('Please Select Batch, Year and month');
        spinneroff();
        return false;
    ...
    {
        $.ajax({
            url: base_url+'?r=ajaxrequest/getanswerpacketsinfo1',
            type:'POST',
            data:{batch_id:$('#ans_batch_id').val(),exam_date:$('#ans_exam_date1').val(), exam_year:$("#ans_exam_year").val(), 
            exam_month:$("#ans_exam_month1").val(),
            exam_session:$('#ans_exam_session1').val(), ans_pack_script:$('#ans_pack_script').val(),exam_type:$('#exam_type').val()},
            success:function(data)
            {            
                spinneroff();
                //var parsed = JSON.parse(data);
...
function deleteanswerpacket()
{
    if($('#ans_qp_code').val()=='' || $('#ans_exam_date').val()=='' || $('#ans_exam_session').val()=='')
    {
        krajeeDialog.alert('Please Select QP Code, Exam date, session');
        spinneroff();
        return false;
    }
    else
    {
        $.ajax({
            url: base_url+'?r=ajaxrequest/deleteanswerpacket',
            type:'POST',
            data:{batch_id:$('#ans_batch_id').val(),exam_date:$('#ans_exam_date').val(), exam_year:$("#ans_exam_year").val(), exam_month:$("#ans_exam_month").val(),
            exam_session:$('#ans_exam_session').val(), ans_qp_code:$('#ans_qp_code').val()},
            success:function(data)
            {            
                spinneroff();
                var parsed = $.parseJSON(data);
                ...
                if(parsed==1)
                {
                    krajeeDialog.alert($('#ans_exam_date').val()+' '+$('#ans_qp_code').val()+' Answer Packet Deleted');                
                    $.ajax({
                        url: base_url+'?r=ajaxrequest/getanswerpacketsinfo',
                        type:'POST',
                        data:{batch_id:$('#ans_batch_id').val(),exam_date:$('#ans_exam_date').val(), exam_year:$("#ans_exam_year").val(), exam_month:$("#ans_exam_month").val(),
                        ...
                        {            
                            ...
                            if(parsed==1)
                            {
                                krajeeDialog.alert('ANSWER COVER DELETED!!');                                
                                $('#answer_packets_div').hide();
                                $('#answer_packets').hide();
                                $('#answer_packets').html('');
                                return true;
                            ...
                            {
                                krajeeDialog.alert('ANSWER COVER ALLOTED TO VALUATION CANNOT DELETE!!');                                
                                $('#answer_packets_div').hide();
                                $('#answer_packets').hide();
                                $('#answer_packets').html('');
                                return true;
                            ...
                            {
                                krajeeDialog.alert('NO ACTION FOUND PLEASE CHECK!!');                                
                                $('#answer_packets_div').hide();
                                $('#answer_packets').hide();
                                $('#answer_packets').html('');
                                return false;
                ...
                {
                    krajeeDialog.alert('Answer Packet cannot deleted! Valuation Faculty Allocated');
                    return true;
                }
                else
                {
                    krajeeDialog.alert('NO ACTION FOUND');
                    return false;
                }
            }
        });
...
function deleteanswerpacket1()
{
    if($('#ans_qp_code1').val()=='' || $('#ans_exam_date1').val()=='' || $('#ans_exam_session').val()=='')
    {
        krajeeDialog.alert('Please Select QP Code, Exam date, session');
        spinneroff();
        return false;
    ...
    {
        $.ajax({
            url: base_url+'?r=ajaxrequest/deleteanswerpacket1',
            type:'POST',
            data:{batch_id:$('#ans_batch_id').val(),exam_date:$('#ans_exam_date1').val(), exam_year:$("#ans_exam_year").val(), exam_month:$("#ans_exam_month1").val(),
            exam_session:$('#ans_exam_session1').val(), ans_qp_code:$('#ans_qp_code1').val()},
            success:function(data)
            {            
                spinneroff();
                var parsed = $.parseJSON(data);
                ...
                if(parsed==1)
                {
                    krajeeDialog.alert($('#ans_exam_date1').val()+' '+$('#ans_qp_code1').val()+' Answer Packet Deleted');                
                    $.ajax({
                        url: base_url+'?r=ajaxrequest/getanswerpacketsinfo1',
                        type:'POST',
                        data:{batch_id:$('#ans_batch_id').val(),exam_date:$('#ans_exam_date').val(), exam_year:$("#ans_exam_year").val(), exam_month:$("#ans_exam_month1").val(),
                        exam_session:$('#ans_exam_session1').val(), ans_qp_code:$('#ans_qp_code1').val()},
                        success:function(data)
                        {            
                            spinneroff();
...
$('#print_packets_btn').on('click',function()
{
    $.ajax({
            ...
            type:'POST',
            data:{batch_id:$('#ans_batch_id').val(),exam_date:$('#ans_exam_date').val(),exam_session:$('#ans_exam_session').val(),exam_year:$("#ans_exam_year").val(),
            exam_month:$("#ans_exam_month").val(),exam_type:$("#exam_type").val()},
            success:function(data)
            {            
                spinneroff();
                var parsed = $.parseJSON(data);
    ...
    $.ajax({
            ...
            success:function(data)
            {
                ...
                {                       
                    ...
                    var parsed = $.parseJSON(data);
                    var k=0;
                    var table_open ='<table class="table table-responsive table-stripped table-bordered table-hover"><tr class="info"><th>S.No.</th><th>Register Number</th><th>Name</th><th>Marks</th><th>Marks in Words</th></tr>';
                    $.each(parsed,function(i,parsed)
                    {
                        var tr='<tr>';
...
function getStudentInfoPracts(register_number)
{
    ...
    $.ajax({
        ...
        type:'POST',
          data:{year:$('#mark_year').val(),register_num_from:register_num_from,month:$('#exam_month').val(),mark_type:mark_type,sub_map_id:sub_map_id_val,register_number:register_number,examiner_name:examiner_name,chief_exam_name:chief_exam_name,section:section},
          success:function(data)
          {
...
$('#toppersList2').on('click',function()
{
  var batch_id = $('#stu_batch_id_selected').val();
  var programme = $('#stu_programme_selected').val();
  spinner();
  $.ajax({
      url: base_url+'?r=ajaxrequest/topperslistrepo2',
      type:'POST',
      data:{programme:programme,batch_id:batch_id},
      success:function(data){
        spinneroff();
...
/* valuation allocate start*/
$('#exam_month_valfac').on('change',function(){

    var year = $('#hallallocate-year').val();
    var month = $('#exam_month_valfac').val();
    //var fromdate = $('#exam_fromdate').val();
    //var todate = $('#exam_todate').val();
    if(year=='' && month=='')
    {
         krajeeDialog.alert("Check Fields are not empty (Year, Month)");
         return false;
    }
    else
    {
        $.ajax({
            url: base_url+'?r=ajaxrequest/valuationsubject',
            type:'POST',
            data:{year:year,month:month},
            success:function(data){       
                $('#subject_code').html('');                    
                ...
                $.each(parsed,function(i,parsed)
                {
                  ...
                  if(subject_code!=parsed)
                  {
                    subject_code=parsed;
                     $('#subject_code').append("<option value='"+parsed['subject_code']+'-'+parsed['anspacketno']+"' >"+parsed['subject_code']+" ("+parsed['anspacketno']+") ("+parsed['final_pack_script']+")</option>");
...
$('#board').on('change',function(){
    ...
    $.ajax({
        url: base_url+'?r=ajaxrequest/valuationfaculty',
        type:'POST',
        data:{year:year,month:month,board:board},
        success:function(data){       
            $('#val_faculty_id').html('');                    
            var parsed = $.parseJSON(data);
            var old_ex_from_date = '';
            $('#val_faculty_id').html('<option value="" >----Select Faculty----</option>');
            $.each(parsed,function(i,parsed)
            {
              
              if(old_ex_from_date!=parsed)
              {
                old_ex_from_date=parsed;
                $('#val_faculty_id').append("<option value='"+parsed['coe_val_faculty_id']+"' >"+parsed['faculty_name']+"</option>");
              }
...
$('#valuationfacultyallocate').on('click',function(){
    ...
    var board = $('#board').val();
    //var exam_type = $('#exam_type').val();
    var val_faculty_id = $('#val_faculty_id').val();
    var valuation_date = $('#val_faculty_date').val();
    var valuation_session = $('#valuation_session').val();
    //var total_answer_scripts = $('#valuationfacultyallocate-total_answer_scripts').val();

     $('#answer_packets_div1').hide();

    if(val_faculty_id=='' || year=='' || month=='' || fromdate=='' || todate=='' || board=='' || valuation_date=='' || valuation_session=='')
    {
         krajeeDialog.alert("Check Fields are not empty");
         return false;
    }
    else
    {
        $.ajax({
            url: base_url+'?r=ajaxrequest/valuationfacultyallocate',
            type:'POST',
            data:{year:year, month:month, fromdate:fromdate, todate:todate, subject_code:subject_code, board:board,
                val_faculty_id:val_faculty_id, valuation_date:valuation_date, valuation_session:valuation_session},
            success:function(data){       

                spinneroff();
                $('#answer_packets1').html('');       

                var parsed = $.parseJSON(data);            
               
                if(parsed==0 || parsed=='' || parsed=='null')
                {
                    krajeeDialog.alert("Data not insert please check");
                     return false;
                }
                else if(parsed==1)
                {
                    krajeeDialog.alert("Total Script is Zero, Please Check 'Report Answer Packet'");
                     return false;
                }
                else if(parsed==2)
                {
                    krajeeDialog.alert("No Scrutiny For Choosen Board");
                     return false;
                }
                else if(parsed>1)
                {
                    krajeeDialog.alert("Total Answer script Max 60, Already Assigned "+parsed);
                     return false;
                }
                else
                {
                     $('#coe_scrutiny_id').val('').trigger('change');
                    krajeeDialog.alert("Data inserted Successfully..");
                    $('#answer_packets_div1').show();
                    $('#answer_packets1').html(parsed);
...
$('#valuationfacultyallocate_view').on('click',function(){

    var year = $('#hallallocate-year').val();
    var month = $('#exam_month_valfac').val();
     var valuation_date = $('#val_faculty_date').val();
    var valuation_session = $('#valuation_session').val();
    var valuation_status =0;// $('#valuation_status').val();
    var board = $('#board').val();
      $('#viewdata').html(''); 
    $('#showqpsettingreceived').hide();

    if($('#valuation_status').is(':checked'))
    {
        valuation_status =1;
    }
    ...
    if(year=='' || month=='')
    {
         krajeeDialog.alert("Please Check fields year, month not empty");
          spinneroff();
         return false;
    }
    else
    {
        $.ajax({
            url: base_url+'?r=ajaxrequest/valuationfacultyallocate_view',
            type:'POST',
            data:{board:board,year:year, month:month,valuation_date:valuation_date, valuation_session:valuation_session,valuation_status:valuation_status},
            success:function(data){       
                ...
                if(parsed==0 || parsed=='')
                {
                    krajeeDialog.alert("Data not found");
                     return false;
                }
                else
                {
                    ...
                    $('#answer_packets_div1').show();
                    $('#answer_packets1').html(parsed);
                    $('#showqpsettingreceived').show();
                    ('#viewdata').html(parsed);
                    return true;
...
function deletevaluator(value){
    ...
    if (confirm("Are you sure Delete Valuator?")) {
        $.ajax({
            url: base_url+'?r=ajaxrequest/getdeletevaluator',
            type:'POST',
            data:{val_faculty_all_id:value},
            success:function(data){       
               
                var jsonFormat = JSON.parse(data);
                if(jsonFormat==0)
                {
                    krajeeDialog.alert('NO DATA FOUND KINDLY CHECK DATA');
                    return false;
                }
                else if(jsonFormat==1)
                {
                    krajeeDialog.alert('SCRUTINY ASSIGNED PLEASE DELETE SCRUTINY FIRST');
                    return false;
                }
                else if(jsonFormat==3)
                {
                    krajeeDialog.alert('SOMETHING ERROR! PLEASE CHECK');
                    return false;
                }
                else
                {
                    $('#answer_packets_div1').hide();
                    $('#answer_packets1').val('');
                    krajeeDialog.alert('Delete Successfully..');
                    
                    $.ajax({
                            url: base_url+'?r=ajaxrequest/valuationfacultyallocate_view',
                            type:'POST',
                            data:{year:year, month:month,valuation_date:valuation_date, valuation_session:valuation_session,valuation_status:valuation_status},
                            success:function(data){       
...
$('#scrutiny_month').on('change',function(){

    var year = $('#hallallocate-year').val();
    var month = $('#scrutiny_month').val();

    $.ajax({
        url: base_url+'?r=ajaxrequest/getvaluation_date',
        type:'POST',
        data:{year:year,month:month},
        success:function(data){       
            $('#valuation_date').html('');                    
            var parsed = $.parseJSON(data);
            var old_ex_from_date = '';
            $('#valuation_date').html('<option value="" >----Select----</option>');
            $.each(parsed,function(i,parsed)
            {
              ...
              {
                old_ex_from_date=parsed;
                $('#valuation_date').append("<option value='"+parsed['valuation_date']+"' >"+parsed['valuation_date']+"</option>");
              }
...
$('#valuation_date').on('change',function(){
    ...
    $.ajax({
        url: base_url+'?r=ajaxrequest/getvaluation_session',
        type:'POST',
        data:{year:year,month:month,valuation_date:valuation_date},
        success:function(data){       
            $('#valuation_session').html('');                    
            var parsed = $.parseJSON(data);
            var old_ex_from_date = '';
            $('#valuation_session').html('<option value="" >----Select----</option>');
            $.each(parsed,function(i,parsed)
            {
              ...
              {
                old_ex_from_date=parsed;
                $('#valuation_session').append("<option value='"+parsed['valuation_session']+"' >"+parsed['valuation_session']+"</option>");
              }
...
$('#scrutiny_session').on('change',function(){
    ...
    $.ajax({
        url: base_url+'?r=ajaxrequest/scrutinyvaluationfaculty',
        type:'POST',
        data:{year:year,month:month,valuation_date:valuation_date,valuation_session:valuation_session},
        success:function(data){       
            $('#val_faculty_id').html('');                    
            var parsed = $.parseJSON(data);
            var old_ex_from_date = '';
...
$('#valuationscrutinyallocate').on('click',function(){

   var year = $('#hallallocate-year').val();
    var month = $('#scrutiny_month').val();
    var valuation_date = $('#valuation_date').val();
    var valuation_session = $('#valuation_session').val();
    var coe_scrutiny_id = $('#coe_scrutiny_id').val();
    var scrutiny_date = $('#scrutiny_date').val();
    var scrutiny_session = $('#scrutiny_session').val();
    var max_faculty =15;// $('#getscrutiny_faculty').val();

    var val_faculty_id = $('#val_faculty_id').val();

     $('#answer_packets_div1').hide();

    if(year=='' || month==''|| coe_scrutiny_id=='' || scrutiny_date=='' || scrutiny_session=='' || valuation_date=='' || valuation_session=='' || val_faculty_id=='')
    {
         krajeeDialog.alert("Check Fields are not empty");
         return false;
    }
    else
    {
        ...
        var job_start_date = valuation_date.split('-');
        var job_end_date = scrutiny_date.split('-');
        ...
        {
            alert ("Scrutiny date must be greater then or equal to valuation date");
            $('#waiting').hide();
            return false;
        }        
        else
        {
            $.ajax({
            url: base_url+'?r=ajaxrequest/valuationscrutinyallocate',
            type:'POST',
             data:{year:year,month:month,valuation_date:valuation_date,valuation_session:valuation_session,max_faculty:max_faculty,
            coe_scrutiny_id:coe_scrutiny_id,scrutiny_date:scrutiny_date,scrutiny_session:scrutiny_session,val_faculty_id:val_faculty_id},
            success:function(data){       
                ...
                else if(parsed>max_faculty)
                {
                    krajeeDialog.alert("Maximum Faculty 15 Already assigned "+parsed+" faculty");
                     return false;
                }
                else
                {
...
$('#valuationscrutinyallocate_view').on('click',function(){
   ...
   var year = $('#hallallocate-year').val();
    ...
    {
        $.ajax({
            url: base_url+'?r=ajaxrequest/valuationscrutinyallocate_view',
            type:'POST',
            data:{valuation_status:valuation_status,year:year, month:month,scrutiny_date:scrutiny_date, scrutiny_session:scrutiny_session, coe_scrutiny_id:coe_scrutiny_id},
            success:function(data){       
...
function deletescrutinyallocate(value){
    ...
    if (confirm("Are you sure Delete Scrutiny Allocate?")) {
        $.ajax({
            url: base_url+'?r=ajaxrequest/deletescrutinyallocate',
            type:'POST',
            data:{val_faculty_all_id:value},
            success:function(data){       
...
/* valuation allocate end*/


//scrutiny mark entry start

$('#barcodeverify_exam_month').on('change',function()
{
    var year = $('#exam_year').val();
    var month = $('#barcodeverify_exam_month').val();

    $.ajax({
        url: base_url+'?r=ajaxrequest/getbarcodescrutiny_date',
        type:'POST',
        data:{year:year,month:month},
        success:function(data){       
            $('#barcodeverify_scrutiny_date').html('');                    
            var parsed = $.parseJSON(data);
            var old_ex_from_date = '';
            $('#barcodeverify_scrutiny_date').html('<option value="" >----Select Date----</option>');
            $.each(parsed,function(i,parsed)
            {
              ...
              {
                old_ex_from_date=parsed;
                $('#barcodeverify_scrutiny_date').append("<option value='"+parsed['scrutiny_date']+"' >"+parsed['scrutiny_date']+"</option>");
              }
...
$('#barcodeverify_exam_month1').on('change',function()
{
    var year = $('#exam_year').val();
    var month = $('#barcodeverify_exam_month1').val();

    $.ajax({
        url: base_url+'?r=ajaxrequest/getallcoatedsubjectcode',
        type:'POST',
        data:{year:year,month:month},
        success:function(data){       
            $('#subject_code').html('');                    
            var parsed = $.parseJSON(data);
            $('#subject_code').html('<option value="" >----Select----</option>');
            $.each(parsed,function(i,parsed)
            {
              
               $('#subject_code').append("<option value='"+parsed['subject_code']+"' >"+parsed['subject_code']+"</option>");
...
$('#barcodeverify_scrutiny_date').on('change',function(){

    var year = $('#exam_year').val();
    var month = $('#barcodeverify_exam_month').val();
    var date = $('#barcodeverify_scrutiny_date').val();
    $.ajax({
        url: base_url+'?r=ajaxrequest/getbarcodescrutinysession',
        type:'POST',
        data:{year:year,month:month,date:date},
        success:function(data){       
            $('#barcodeverify_scrutiny_session').html('');   
            $('#barcodeverify_scrutiny_session1').html('');                     
            var parsed = $.parseJSON(data);
            var old_ex_from_date = '';
            $('#barcodeverify_scrutiny_session').html('<option value="" >----Select Session----</option>');
            $('#barcodeverify_scrutiny_session1').html('<option value="" >----Select Session----</option>');
            $.each(parsed,function(i,parsed)
            {
              ...
              {
                old_ex_from_date=parsed;
                $('#barcodeverify_scrutiny_session').append("<option value='"+parsed['scrutiny_session']+"' >"+parsed['scrutiny_session']+"</option>");
                $('#barcodeverify_scrutiny_session1').append("<option value='"+parsed['scrutiny_session']+"' >"+parsed['scrutiny_session']+"</option>");
              }
...
$('#barcodeverify_scrutiny_session').on('change',function(){

   var year = $('#exam_year').val();
    var month = $('#barcodeverify_exam_month').val();
    var date = $('#barcodeverify_scrutiny_date').val();
    var session = $('#barcodeverify_scrutiny_session').val();

    $.ajax({
        url: base_url+'?r=ajaxrequest/getbarcodescrutinyassignedfaculty',
        type:'POST',
        data:{year:year,month:month,date:date,session:session},
        success:function(data){       
            $('#barcodeverify_val_faculty_all_id').html('');   
             $('#verifydetails_faculty_all_id').html('');                  
            var parsed = $.parseJSON(data);
            var old_ex_from_date = '';
             $('#barcodeverify_val_faculty_all_id').html('<option value="" >----Select Faculty----</option>');
            $('#view_val_faculty_all_id').html('<option value="" >----Select Faculty----</option>');
            $.each(parsed,function(i,parsed)
            {
              ...
              {
                old_ex_from_date=parsed;
                $('#barcodeverify_val_faculty_all_id').append("<option value='"+parsed['val_faculty_all_id']+"' >"+parsed['faculty_name']+"("+parsed['subject_code']+" - "+parsed['subject_pack_i']+")</option>");
            
                $('#view_val_faculty_all_id').append("<option value='"+parsed['val_faculty_all_id']+"' >"+parsed['faculty_name']+"("+parsed['subject_code']+" - "+parsed['subject_pack_i']+")</option>");
              }
...
$('#barcodeverify_scrutiny_session1').on('change',function(){

   var year = $('#exam_year').val();
    var month = $('#barcodeverify_exam_month').val();
    var date = $('#barcodeverify_scrutiny_date').val();
    var session = $('#barcodeverify_scrutiny_session1').val();

    $.ajax({
        url: base_url+'?r=ajaxrequest/getbarcodescrutinyassignedfaculty1',
        type:'POST',
        data:{year:year,month:month,date:date,session:session},
        success:function(data){       
            $('#barcodeverify_val_faculty_all_id').html('');   
...
$('#barcodeverify_val_faculty_all_id').on('change',function(){

   var year = $('#exam_year').val();
    var month = $('#barcodeverify_exam_month').val();
    var date = $('#barcodeverify_scrutiny_date').val();
    var session = $('#barcodeverify_scrutiny_session').val();
    var fac_all_id = $('#barcodeverify_val_faculty_all_id').val();
    $('#verification_details_data').hide();
    $.ajax({
        url: base_url+'?r=ajaxrequest/getbarcodeverifysavedata',
        type:'POST',
        data:{year:year,month:month,date:date,session:session,fac_all_id:fac_all_id},
        success:function(data){       
            var parsed = $.parseJSON(data);
            if(parsed[0]['result']=='Completed')
            {
                krajeeDialog.alert('Packet Completed! Check Details on Mark entry List Page');
                return false;
            }
            else if(parsed[0]['result']!=0)
            {
                $('#hide_bar_code_data1').show();
                console.log(parsed[0]);
                $('#batch_id').val(parsed[0]['result']);
                $('#displayregno').val(parsed[0]['displayregno']);
                $('#reg_no').val(parsed[0]['displayregno']);
                $('#firstreg_no').val(parsed[0]['firstregno']);                
                $('#lastreg_no').val(parsed[0]['lastregno']);
                if(parsed[0]['absent_regno']!='')
                {
                    $('#veify_details2020').hide();
                    $('#veify_details2021').hide();
                    $('#veify_details_absent').show();
                    $('#absentstudent').val('-1');
                    var reg_no=parsed[0]['displayregno'];
                    var firstreg_no=parsed[0]['firstregno'];
                    var lastreg_no=parsed[0]['lastregno'];

                        $('#show_savebutton').show();
                        if(reg_no!=firstreg_no && reg_no==lastreg_no)
                        {
                            $('#backdiv').show();
                            $('#nextdiv').hide();
                            $('#finshdiv').show();
                        }
                        else if(reg_no==firstreg_no && reg_no!=lastreg_no)
                        {
                            $('#backdiv').hide();
                            $('#nextdiv').show();
                            $('#finshdiv').hide();
                ...
                else if(parsed[0]['result']>=2021)
                {
                    $('#veify_details_absent').hide();
                    $('#veify_details2020').hide();
                    $('#veify_details2021').show();
                }
                else if(parsed[0]['result']<2021)
                {
                    $('#veify_details_absent').hide();
                    $('#veify_details2020').show();
                    $('#veify_details2021').hide();
                }
            }
            else
            {
                $('#hide_bar_code_data1').hide();
                $('#veify_details2020').hide();
                $('#veify_details2021').hide(); 
                $('#veify_details_absent').hide();               
                return false;
...
$('#backsavemarks').on('click',function(){
    var exam_year = $('#exam_year').val();
    var exam_month = $('#barcodeverify_exam_month').val();
    var reg_no=$('#reg_no').val();
    var fac_all_id = $('#barcodeverify_val_faculty_all_id').val();
    var batch_id=$('#batch_id').val();   
    $('#absentstudent').val('');
     $.ajax({
        url: base_url+'?r=ajaxrequest/getscrutinymarks',
        type:'POST',
        data:{batch_id:batch_id,year:exam_year,month:exam_month,reg_no:reg_no,fac_all_id:fac_all_id },
        success:function(data){       
            var parsed = $.parseJSON(data);
            if(parsed[0]['result']==2)
            {                
               // console.log(parsed[0]);
                $('#reg_no').val(parsed[0]['displayregno']);
                $('#displayregno').val(parsed[0]['displayregno']);
                ...
                if(parsed[0]['absent_regno']!='')
                {
                    ...
                    $('#absentstudent').val('-1');
                     var reg_no=parsed[0]['displayregno'];
                    var firstreg_no=$('#firstreg_no').val();
                    var lastreg_no=$('#lastreg_no').val();
                ...
                else if(batch_id>='2021')
                {
                    var i=0;
                    for(var a=1;a<=10;a++)
                    {
                        $('#part_a_21'+a).val(parsed[0]['parta'][i]);
                        i++;
                    }
                    $('#part_a_total21').val(parsed[0]['part_a_total']);

                    var i=0;
                    for(var a=11;a<=15;a++)
                    {
                        $('#part_b_2021'+a).val(parsed[0]['partb'][i]);
                        i++;
                    }

                    $('#part_b_2021total').val(parsed[0]['part_b_total']);

                    var i=0;
                    for(var a=16;a<=20;a++)
                    {
                        $('#part_c_2021'+a).val(parsed[0]['partc'][i]);
                        i++;
                    }

                    $('#part_c_2021total').val(parsed[0]['part_c_total']);
                    $('#part_grandtotal_2021').val(parsed[0]['grand_total']);
                    $('#veify_details2020').hide();
                    $('#veify_details2021').show();
                    $('#veify_details_absent').hide();
                    var reg_no=parsed[0]['displayregno'];
                ...
                else
                {
                    ...
                    for(var a=1;a<=10;a++)
                    {
                        $('#part_a_'+a).val(parsed[0]['parta'][i]);
                        i++;
                    }
                    $('#part_a_total').val(parsed[0]['part_a_total']);

                    var i=0;
                    for(var a=11;a<=15;a++)
                    {
                        $('#part_b_'+a).val(parsed[0]['partb'][i]);
                        i++;
                    }

                    $('#part_b_total').val(parsed[0]['part_b_total']);

                    var i=0;
                    for(var a=16;a<=20;a++)
                    {
                        $('#part_c_'+a).val(parsed[0]['partc'][i]);
                        i++;
                    }

                    $('#part_c_total').val(parsed[0]['part_c_total']);
                    $('#part_grandtotal').val(parsed[0]['grand_total']);
                    $('#veify_details2020').show();
                    $('#veify_details2021').hide();
                    $('#veify_details_absent').hide();
            ...
            else if(parsed[0]['result']==3)
            {
                krajeeDialog.alert('Cover Mismatch Please Check Selected Cover Or Check Reg.No. Selected Cover');
                return false;
            }
            else if(parsed[0]['result']==4)
            {
                krajeeDialog.alert("Marks Not Insert Please Check");
                return false;
            }
            else 
            {
...
$('#nextsavemarks').on('click',function(){
    var exam_year = $('#exam_year').val();
    var exam_month = $('#barcodeverify_exam_month').val();
    var reg_no=$('#reg_no').val();
    var fac_all_id = $('#barcodeverify_val_faculty_all_id').val();
    var batch_id=$('#batch_id').val();
    var absentstudent=$('#absentstudent').val();
    ...
    else if(batch_id>='2021')
    {
        ...
        for(var a=1;a<=10;a++)
        {
            var val=$('#part_a_21'+a).val();
            partA.push(val);
        }

        var part_a_total=$('#part_a_total21').val();

        var partB = []; 
        for(var a=11;a<=15;a++)
        {
            //var bb='#part_b_2021'+a+' ';
            var val=$('#part_b_2021'+a).val();
            //console.log(bb+val);
            partB.push(val);
        }
        var part_b_total=$('#part_b_2021total').val();

        var partC = []; 
        for(var a=16;a<=20;a++)
        {
            var val=$('#part_c_2021'+a).val();
             console.log(val);
            partC.push(val);
        }
        var part_c_total=$('#part_c_2021total').val();

        var grandtotal=$('#part_grandtotal_2021').val();
    }
    else
    {
        ...
        for(var a=1;a<=10;a++)
        {
            var val=$('#part_a_'+a).val();
            partA.push(val);
        }

        var part_a_total=$('#part_a_total').val();

        var partB = []; 
        for(var a=11;a<=15;a++)
        {
            var val=$('#part_b_'+a).val();
            partB.push(val);
        }
        var part_b_total=$('#part_b_total').val();

        var partC = []; 
        for(var a=16;a<=20;a++)
        {
            var val=$('#part_c_'+a).val();
            partC.push(val);
        }
        var part_c_total=$('#part_c_total').val();

        var grandtotal=$('#part_grandtotal').val();
    }

     $.ajax({
        url: base_url+'?r=ajaxrequest/savescrutinymarks',
        type:'POST',
        data:{batch_id:batch_id,year:exam_year,month:exam_month,reg_no:reg_no,fac_all_id:fac_all_id,partA:partA, part_a_total:part_a_total,
            partB:partB, part_b_total:part_b_total ,partC:partC, part_c_total:part_c_total,grandtotal:grandtotal },
        success:function(data){       
            var parsed = $.parseJSON(data);
            $('#absentstudent').val('');
            if(parsed[0]['result']==1)
            {
                krajeeDialog.alert('Register Number Already Completed');
                return false;
            }
            else if(parsed[0]['result']==2)
            {                
                $('#reg_no').val(parsed[0]['nextregno']);
                $('#displayregno').val(parsed[0]['nextregno']);
                $('#veify_details2020').hide();
                $('#veify_details2021').hide();
                $('#veify_details_absent').hide();
                $('#show_savebutton').hide();
                ...
                else
                {
                    if(batch_id>='2021')
                    {
                        ...
                        $('#part_c_2021total').val('');
                        $('#part_grandtotal_2021').val('');
                        $('#veify_details2020').hide();
                        $('#veify_details2021').show();
...
$('#finishsavemarks').on('click',function(){
    var exam_year = $('#exam_year').val();
    var exam_month = $('#barcodeverify_exam_month').val();
    var reg_no=$('#reg_no').val();
    var fac_all_id = $('#barcodeverify_val_faculty_all_id').val();
    ...
    if(confirm("Are you sure you want to Finish it! This can not be Un-Done once the values were Submitted."))
    {
        var absentstudent=$('#absentstudent').val();
        ...
        success:function(data){       
            ...
            {                
                ...
                $('#verification_details_pdf').hide(); 
                $.ajax({
                    url: base_url+'?r=ajaxrequest/getscrutinymarkdetails',
                    type:'POST',
                    data:{year:exam_year,month:exam_month,date:date,session:session,fac_all_id:fac_all_id,scorecard:0},
                    success:function(data){       
                       
                        var jsonFormat = JSON.parse(data);
                        if(jsonFormat[0]['vdata']==0)
                        {                            
                            krajeeDialog.alert('NO DATA FOUND KINDLY CHECK DATA');
                            return false;
                        }
                        ...
                        {  
                            console.log(jsonFormat[0]['valuation_status']);
                            $('#verification_details_data').show();
                            if(jsonFormat[0]['valuation_status']<=5)
                            {
                                 $('#verification_details_pdf').show();
                                 $.ajax({
                                    url: base_url+'?r=ajaxrequest/getbarcodescrutinyassignedfaculty',
                                    type:'POST',
                                    data:{year:exam_year,month:exam_month,date:date,session:session},
                                    success:function(data){       
                                        $('#barcodeverify_val_faculty_all_id').html('');              
                                        var parsed = $.parseJSON(data);
                                        var old_ex_from_date = '';
                            ...
                            $('#verification_details').html(jsonFormat[0]['vdata']);
                        }
                    }
                });
            }
...
$('#show').on('click',function(){
   ...
   // var session = $('#barcodeverify_scrutiny_session1').val();
    ...
    spinner();
    var viewsplitup =0;
    if($('#viewsplitup').is(':checked'))
    {
        viewsplitup =1;
    }
    $.ajax({
        url: base_url+'?r=ajaxrequest/getscrutinymarkdetailsqp',
        type:'POST',
        data:{year:year,month:month,subject_code:subject_code,scorecard:viewsplitup},
        success:function(data){       
           spinneroff();
            var jsonFormat = JSON.parse(data);
                ...
                {
                     $('#verification_details_pdf').show();
                      $('#verifydetails-export-excel').show();
                 }
...
$('#show_markdetails').on('click',function(){
    var exam_year = $('#exam_year').val();
    var exam_month = $('#barcodeverify_exam_month').val();
    var date = $('#barcodeverify_scrutiny_date').val();
    var session = $('#barcodeverify_scrutiny_session1').val();
    var fac_all_id = $('#view_val_faculty_all_id').val();
    $('#verification_details').html('');
    $('#verification_details_pdf').hide(); 
    spinner();
     var viewsplitup =0;
    ...
    $.ajax({
        url: base_url+'?r=ajaxrequest/getscrutinymarkdetails',
        type:'POST',
        data:{year:exam_year,month:exam_month,date:date,session:session,fac_all_id:fac_all_id,scorecard:viewsplitup},
        success:function(data){       
           spinneroff();
            var jsonFormat = JSON.parse(data);
            if(jsonFormat[0]['vdata']==0)
...
$('#update_exam_month').on('change',function(){

    var year = $('#dummynumber_exam_year').val();
    var month = $('#update_exam_month').val();

    $.ajax({
        url: base_url+'?r=ajaxrequest/getallcoatedsubject',
        type:'POST',
        data:{year:year,month:month},
        success:function(data){       
            $('#dummynumber_val_faculty_id').html('');                   
            var parsed = $.parseJSON(data);
            var old_ex_from_date = '';
            $('#dummynumber_val_faculty_id').html('<option value="" >----Select----</option>');
            $.each(parsed,function(i,parsed)
            {
              $('#dummynumber_val_faculty_id').append("<option value='"+parsed['val_faculty_all_id']+'-'+parsed['subject_code']+'-'+parsed['subject_pack_i']+"' >("+parsed['subject_code']+") ("+parsed['subject_pack_i']+")</option>");           
            });
        }
    });
});

function getscrutinyentry(id,value,exam_year,exam_month,fac_all_id)
{
    $('#hide_bar_code_data').hide();
    $('#show_scrutiny_entry_master').html('');
        if(value!='' && value!=null)
        {
            $.ajax({
            url: base_url+"?r=ajaxrequest/getscrutinyentryupdate",
            type:'POST',
            data:{reg_num:value,exam_year:exam_year,exam_month:exam_month,fac_all_id:fac_all_id },
            success:function(data)
            {
                var parsed = JSON.parse(data);
                if(parsed[0]['result']==2)
                {   
                    $('#hide_bar_code_data').show();
                    $('#show_scrutiny_entry_master').html(parsed[0]['master']);
                   
                    if(parsed[0]['design']=='absentstu')
                    {
                        $('#veify_details2020').hide();
                        $('#veify_details2021').hide();
                        $('#veify_details_absent').show();
                    }
                   else if(parsed[0]['design']>='2021')
                   {
                        $('#batch_id').val(parsed[0]['design']);
                       $('#veify_details2020').hide();
                        $('#veify_details2021').show();
                        $('#veify_details_absent').hide();
                        var i=0;
                ...
                {
                    krajeeDialog.alert('Result Published No Marks Update');
                    return false;
                }
                else if(parsed[0]['result']==4)
                {
                    krajeeDialog.alert('Cover Number Mismatch Please Check');
                    return false;
                }
                else
                {
                    krajeeDialog.alert('Data Not Found Please Check');
                    return false;
                }
            }
            });    
...
$('#updatesavescrutinymarks').on('click',function(){
    var exam_year = $('#dummynumber_exam_year').val();
    var exam_month = $('#update_exam_month').val();
    var reg_no=$('#reg_no').val();
    var fac_all_id = $('#dummynumber_val_faculty_id').val();
    var batch_id=$('#batch_id').val();
    ...
    if(confirm("Are you sure you want to Update!"))
    {
        ...
        $.ajax({
        url: base_url+'?r=ajaxrequest/updatescrutinymarks',
        type:'POST',
        data:{batch_id:batch_id,year:exam_year,month:exam_month,reg_no:reg_no,fac_all_id:fac_all_id,partA:partA, part_a_total:part_a_total,
            partB:partB, part_b_total:part_b_total ,partC:partC, part_c_total:part_c_total,grandtotal:grandtotal },
        success:function(data){       
            var parsed = $.parseJSON(data);
            if(parsed[0]['result']==2 || parsed[0]['result']>=5)
            {                
                $('#reg_no').val('');
                $('#hide_bar_code_data').hide();
                $('#veify_details2020').hide();
                ...
                krajeeDialog.alert('Marks Successfully Updated..');
                return true;
            }
            else if(parsed[0]['result']==3)
            {
...
/*valuation mark entry approve start*/

$('#dummynumber_exam_month').on('change',function(){

    var year = $('#dummynumber_exam_year').val();
    var month = $('#dummynumber_exam_month').val();

    $.ajax({
        url: base_url+'?r=ajaxrequest/getbarcodevalfaculty',
        type:'POST',
        data:{year:year,month:month},
        success:function(data){       
           $('#dummynumber_val_faculty_id').html('');                    
            var parsed = $.parseJSON(data);
            $('#dummynumber_val_faculty_id').html('<option value="" >----Select Faculty----</option>');
            $.each(parsed,function(i,parsed)
            {
              
              
                $('#dummynumber_val_faculty_id').append("<option value='"+parsed['subject_code']+"' >"+"("+parsed['subject_code']+")</option>");
...
$('#exam_month_add1').on('change',function(){

    var year = $('#exam_year').val();
    var month = $('#exam_month_add1').val();

    $.ajax({
        url: base_url+'?r=ajaxrequest/getbarcodevalfacultyadd',
        type:'POST',
        data:{year:year,month:month},
        success:function(data){       
           $('#qp_code').html('');                    
            var parsed = $.parseJSON(data);
            $('#qp_code').html('<option value="" >----Select Faculty----</option>');
            $.each(parsed,function(i,parsed)
            {
              
              
                $('#qp_code').append("<option value='"+parsed['answer_packet_number']+"' >"+"("+parsed['answer_packet_number']+")</option>");
             
            });
        }
    });
});
$('#dummynumber_val_faculty_id').on('change',function(){

   var year = $('#dummynumber_exam_year').val();
    var month = $('#dummynumber_exam_month').val();
    var val_faculty_id = $('#dummynumber_val_faculty_id').val();

         $.ajax({
        url: base_url+'?r=ajaxrequest/checkmarkexist',
        type:'POST',
        data:{year:year,month:month,val_facl_id:val_faculty_id},
        success:function(data){                 
            var parsed = $.parseJSON(data);
            if(parsed==5)
            {
                 krajeeDialog.alert("QPCODE Approved please check in scrutiny Report ");
                return false;
                // $('#submit_dummy').hide();
                // $('#submit_dummyreset').hide();
                // $('#hide_dum_data').hide();                    
                // $('#show_dummy_numbers_new').html('');
                // $('#show_reset').show(); 
                // $('#get_students_data1').show();
                // $('#hide_dum_sub_data').hide();
                // $('#show_students_details1').hide();
            }
            else
            {
                $('#submit_dummy').hide();
                ...
                $('#show_reset').show(); 
                $('#hide_dum_sub_data').hide();
                $('#show_students_details1').show();
                $('#get_students_data1').hide();
            }
...
function get_students_details(exam_year,exam_month)
{
   var val_faculty_id = $('#dummynumber_val_faculty_id').val();
    spinner();
    if(val_faculty_id=='')
    ...
    {
        $.ajax({
        url: base_url+'?r=ajaxrequest/getdummystudents_details',
        type:'POST',
        data:{val_faculty_id:val_faculty_id,exam_year:exam_year,exam_month:exam_month},
            success:function(data)
            {
                 spinneroff();
                var parsed = JSON.parse(data);
                ...
                if(parsed[0]['data_pass']==0)
                {
                    krajeeDialog.alert('No Data Found! Please check CIA Mark');
                    $('#submit_dummy').hide();
                    $('#submit_dummyreset').hide();
                    $('#hide_dum_data').hide();                                        
                     return false;
                }
                else if(parsed[0]['data_pass']==1)
                {
                     krajeeDialog.alert('Cover Already Completed or Cover Not Completed Scrutiny Entry');
                    $('#submit_dummy').show();
                     $('#submit_dummyreset').show();
                    $('#hide_dum_data').hide();                                                      
                    return false;
                }
                else
                {
                    ...
                    $('#show_reset').hide(); 
                     ...
                     var parsed = $.parseJSON(data);
                        ...
                        var tr='<tr>';
                        var td='<td>1</td><td>'+parsed[0]['sub_info']['subject_code']+' </td><td>'+parsed[0]['sub_info']['subject_name']+' </td><td>'+parsed[0]['sub_info']['CIA_min']+'</td><td>'+parsed[0]['sub_info']['CIA_max']+'</td><td>'+parsed[0]['sub_info']['ESE_min']+'</td><td>'+parsed[0]['sub_info']['ESE_max']+'</td><td>'+parsed[0]['sub_info']['total_minimum_pass']+'</td><td>'+parsed[0]['sub_info']['credit_points']+'</td>';
                        var tr_dum_close ='</tr>'; 
                        body1 = tr+td+tr_dum_close;
                        
                        $('#hide_dum_sub_data').show();                    
                        $('#show_subject_details').html(body1); 

                    var body='';                    
                    
                    var k=0;
                    $.each(parsed[0]['data_pass'],function(i,parsed1){

                        if(parsed1['mark']=='-1')
                        {
                            var tr='<tr>';
                            var td='<td  width="10%">'+(k+1)+'</td><td width="30%"> <input type ="hidden" value="'+parsed1['subject_mapping_id']+'" name="sub_map_id[] " /><input type ="hidden" value="'+parsed1['coe_student_mapping_id']+'" name="register_numbers[] " />'+parsed1['register_number']+' </td><td width="30%">'+parsed1['CIA']+'</td><td  width="30%"><input type="hidden" id=dum_num_marks'+k+' value='+parsed1['mark']+' readonly  name="ese_marks[]" />ABSENT</td>';
                            var tr_dum_close ='</tr>'; 
                        }
                        else
                        {
                            var tr='<tr>';
                            var td='<td  width="10%">'+(k+1)+'</td><td width="30%"> <input type ="hidden" value="'+parsed1['subject_mapping_id']+'" name="sub_map_id[] " /><input type ="hidden" value="'+parsed1['coe_student_mapping_id']+'" name="register_numbers[] " />'+parsed1['register_number']+' </td><td width="30%">'+parsed1['CIA']+'</td><td  width="30%"><input type="hidden" id=dum_num_marks'+k+' value='+parsed1['mark']+' readonly  name="ese_marks[]" />'+parsed1['mark']+'</td>';
                            var tr_dum_close ='</tr>'; 
                        }
                    ...
                    $('#submit_dummy').show();
                     $('#submit_dummyreset').show();
                    $('#hide_dum_data').show();                    
                    $('#show_dummy_numbers_new').html(body);
                }                
...
/*valuation mark entry approve end*/

//2020 calucation start
function getpartatotal() 
{
    ...
    for(i=1;i<=10;i++)
    {
        var parta_i=$('#part_a_'+i).val();
        if(parta_i>1)
        {
            $('#part_a_'+i).val(0);
            krajeeDialog.alert("Part A "+i+" Maximum mark 1 Please Check");
            return false;
        }
        else if(parta_i=='' || parta_i=='-')
        {
            temp=0;
        }
        else
       ...
       part_subtotal=parseInt(part_subtotal)+parseInt(temp);
       //alert("part_subtotal");
    }
    //alert("part_subtotal");
   ...
   var partC=0;
    var partB=0;
    if($('#part_b_total').val()>0)
    {
        partB=$('#part_b_total').val();
    }
    else
    {
        partB=0;
    ...
    if($('#part_c_total').val()>0)
    {
        partC=$('#part_c_total').val();
    }
    else
    {
        partC=0;
    ...
    if(part_subtotal<=20)
    {
        $('#part_a_total').val(part_subtotal);
    }
    else if(part_subtotal!=0)
    {
         $('#part_a_total').val(0);
         krajeeDialog.alert("Part A maximum mark 20");
         return false;
       
    }

    grandtotal=parseInt(part_subtotal)+parseInt(partB)+parseInt(partC);

    $('#part_grandtotal').val(grandtotal);
    ...
    if(grandtotal>0)
    {
        //console.log(firstreg_no+"="+reg_no+"=="+lastreg_no);
        $('#show_savebutton').show();
        if(reg_no!=firstreg_no && reg_no==lastreg_no)
        {
            $('#backdiv').show();
...
function getpartbtotal()
{
    ...
    for(i=11;i<=15;i++)
    {
        var tt=$('#part_b_'+i).val();

        if(tt>4)
        {
            $('#part_b_'+i).val(0);
            krajeeDialog.alert("Part B "+i+" Total Maximum mark 4 Please Check");
            return false;
        }
        else
        {
    ...
    var partA=0;
    if($('#part_a_total').val()>0)
    {
        partA=$('#part_a_total').val();
    }
    ...
    var grandtotal=parseInt(part_subtotal)+parseInt(partA)+parseInt(partC);
    
    $('#part_b_total').val(part_subtotal);
    $('#part_grandtotal').val(grandtotal);
...
function getpartctotal()
{
    ...
    for(i=16;i<=20;i++)
    {
        var tt=$('#part_c_'+i).val();

        if(tt>6)
        {
            $('#part_c_'+i).val(0);
            krajeeDialog.alert("Part C "+i+" Total Maximum mark 6 Please Check");
            return false;
        }
        else
        {
    ...
    var grandtotal=parseInt(part_subtotal)+parseInt(partA)+parseInt(partB);
    
    $('#part_c_total').val(part_subtotal);
    $('#part_grandtotal').val(grandtotal);
...
//2021 calucation start
function getpartatotal21() 
{
   ...
   var partC=0;
    var partB=0;
    if($('#part_b_2021total').val()>0)
    {
        partB=$('#part_b_2021total').val();
    }
    else
    {
        partB=0;
    ...
    if($('#part_c_2021total').val()>0)
    {
        partC=$('#part_c_2021total').val();
    }
    else
    {
        partC=0;
    ...
    $('#part_a_total21').val(part_subtotal);

    grandtotal=parseInt(part_subtotal)+parseInt(partB)+parseInt(partC);

    $('#part_grandtotal_2021').val(grandtotal);
...
function getpartbtotal21()
{
    ...
    for(i=11;i<=15;i++)
    {
        var tt=($('#part_b_2021'+i).val());

        if(tt>5)
        {
            $('#part_b_2021'+i).val(0);
            krajeeDialog.alert("Part B "+i+" Total Maximum mark 3 Please Check");
            return false;
        }
        else
        {            
    ...
    console.log("part_subtotal"+part_subtotal);

    var partA=0;
    if($('#part_a_total21').val()>0)
    {
        partA=$('#part_a_total21').val();
    }
    ...
    $('#part_b_2021total').val(part_subtotal);
    $('#part_grandtotal_2021').val(grandtotal);
...
function getpartctotal21()
{
    ...
    for(i=16;i<=20;i++)
    {
        var tt=($('#part_c_2021'+i).val());

        if(tt>10)
        {
            $('#part_c_2021'+i).val(0);
            krajeeDialog.alert("Part C "+i+" Total Maximum mark 10 Please Check");
            return false;
        }
        else
        {
    ...
    $('#part_c_2021total').val(part_subtotal);
    $('#part_grandtotal_2021').val(grandtotal);
...
$('#transfer_cert_button').on('click',function(){
  ...
  var class_studying=$('#class_studying').val();
  var reason=$('#reason').val();
  var char=$('#char').val();
  var date_left=$('#date_left').val();
  var date_apply=$('#date_apply').val();
  if(batch=="" || bat_map_id=="" || reg_to=="" || reg_from=="" || date_issue=="" ||class_studying=="" ||reason==""||char==""||date_left=="" ||date_apply=="")
  {
    krajeeDialog.alert("Please select all feilds");
    return false;
  }
  else
  {
    $.ajax({
     ...
     url: base_url+'?r=mark-entry/transfercerficatedata',
    
      type:'POST',
      data:{batch:batch,bat_map_id:bat_map_id,reg_from:reg_from,reg_to:reg_to,date_issue:date_issue,class_studying:class_studying,reason:reason,char:char,date_left:date_left,date_apply:date_apply},
      success:function(data)
      {
        console.log(data);
        if(data!=0)
...
function showboard(exam_year,exam_month,exam_date,exam_type,ans_pack_serial,exam_session)
{
    $.ajax({
        type:'POST',
        url: base_url+"?r=ajaxrequest/getconsolidateboard",
        data:{
            
            exam_year:exam_year,exam_month:exam_month,exam_date:exam_date,exam_type:exam_type,ans_pack_serial:ans_pack_serial,exam_session:exam_session            
        },
        success: function(data)
        {
           var jsonFormat = JSON.parse(data);
...
$('#qpsubmit1').on('click',function(){
   var date = $('#exam_date_add').val();
   var session = $('#exam_session').val();
   var exam_year = $('#exam_year').val();
   var exam_month = $('#exam_month_add').val();
   if(date=='' || session=='')
   {
        krajeeDialog.alert("Select the Required Fields");
        return false;
   ...
   $.ajax({
        url: base_url+'?r=coe-add-hall-allocate/qpcnt',
        type:'POST',
...
$('#exam_month_add').on('change',function(){

    var year = $('#exam_year').val();
    var month = $('#exam_month_add').val();

    $.ajax({
        url: base_url+'?r=ajaxrequest/getqpexamdateadd',
        type:'POST',
        data:{year:year,month:month},
        success:function(data){       
            $('#exam_date_add').html('');                    
            var parsed = $.parseJSON(data);
            var old_ex_date = '';
            $('#exam_date_add').html('<option value="" >----Select----</option>');
            $.each(parsed,function(i,parsed)
            {
              ...
              {
                old_ex_date=parsed['exam_date'];
                $('#exam_date_add').append("<option value='"+parsed['exam_date']+"' >"+parsed['exam_date']+"</option>");
              }
...
$('#exam_date_add').on('change',function(){

    $('#exam_session').html("<option value=''>-----Select----</option>");

    var date = $('#exam_date_add').val();    
    $.ajax({
        url: base_url+'?r=ajaxrequest/getqpsessionadd',
        type:'POST',
        data:{date:date},
        success:function(data){               
            var parsed = $.parseJSON(data);
...
$('#hall_arrangement_add').on('change',function(){

    $('#from').html('');
    $('#to').html('');
    $('#countFrom').val('');
    $('#countTo').val('');
    $('#method_add').html("<option value=''>-----Select----</option>");
    $('#hallallocate-student_count').html('');

    var hall_arrangement = $('#hall_arrangement_add').val(); 
    var date = $('#exam_date_add').val();
    var exam_month = $('#exam_month_add').val();
    var session = $('#exam_session').val();
    ...
    if(hall_arrangement=="Subject Wise"){
        $('#subjectwise').show();
        $('#subject_add').html(""); 
        $('#subject_add').append("<option value=''>-----Select----</option>");
        $.ajax({
            url: base_url+'?r=ajaxrequest/getsubcodeadd',
            type:'POST',
            data:{date:date,hall_arrangement:hall_arrangement,session:session,exam_month:exam_month},
            success:function(data){                                     
                var parsed = $.parseJSON(data);
                $.each(parsed,function(i,parsed){
                $('#subject_add').append("<option value='"+parsed['subject_code']+"'>"+parsed['subject_code']+"</option>");
                });           
            }
        });
        ...
        $('#subjectwise').hide();
        $.ajax({
            url: base_url+'?r=ajaxrequest/getmethodadd',
            type:'POST',
            data:{date:date,hall_arrangement:hall_arrangement,session:session,exam_month:exam_month},
            success:function(data){            
                var parsed = $.parseJSON(data);
                ...
                $('#method_add').html("<option value=''>-----Select----</option>");
                $.each(parsed,function(i,parsed){
                $('#method_add').append("<option value='"+parsed['coe_category_type_id']+"'>"+parsed['category_type']+"</option>");
                });   
            }
        });
    }
...
$('#method_add').on('change',function()
{
    
    var method = $('#method_add').val();
    var exam_year = $('#exam_year').val();
    var exam_month = $('#exam_month_add').val();
    var hall_arrangement = $('#hall_arrangement_add').val();
    var date = $('#exam_date_add').val();
    ...
    $('#from').html("");
    $.ajax({
        url: base_url+'?r=ajaxrequest/gethalladd',
        type:'POST',
        data:{method:method},
        success:function(data)
        {
    ...
    if(hall_arrangement=="Non-Subject Wise"){               
        $('#subjectwise').hide();
        $.ajax({
            url: base_url+'?r=ajaxrequest/getseatcountadd',
            type:'POST',
            data:{method:method,date:date,session:session,exam_month:exam_month,exam_year:exam_year},
            success:function(data){                 
                var parsed = $.parseJSON(data); 
                $('#hall_cnt').attr("value",parsed['available_hall']);                                     
              $('#coeaddhallallocate-student_count').html(parsed['message']);              
            }
        });
    }else if(hall_arrangement=="Subject Wise")
    {
             
            $("#subject_add").each(function(){
                var subject_code = $('#subject_add').val();        
                sub_code_galley+=subject_code;               
            });
            
            $.ajax({
                url: base_url+'?r=ajaxrequest/getsubjectseatcountadd',
                type:'POST',
                data:{method:method,date:date,sub_code_galley:sub_code_galley,session:session,exam_month:exam_month,exam_year:exam_year},
                success:function(data){                     
                    var parsed = $.parseJSON(data);
...
$('#subject_add').on('change',function(){
    $('#method_add').html("<option value=''>-----Select----</option>");
    $('#hallallocate-student_count').html('');
...
$('#print_packets_btn1').on('click',function()
{
    $.ajax({
            url: base_url+'?r=ajaxrequest/getaddprintregisternumber',
            type:'POST',
            data:{batch_id:$('#ans_batch_id').val(),exam_date:$('#ans_exam_date1').val(),exam_session:$('#ans_exam_session1').val(),exam_year:$("#ans_exam_year").val(),
            exam_month:$("#ans_exam_month1").val(),exam_type:$("#exam_type1").val()},
            success:function(data)
            {            
                spinneroff();
                var parsed = $.parseJSON(data);
...
$('#fh_month').on('change',function(){

    var year = $('#fh_year').val();
    var month = $('#fh_month').val();
    ...
    $.ajax({
        ...
        success:function(data){       
            ...
            var parsed = $.parseJSON(data);
            var old_ex_date = '';
            $('#fh_date').html('<option value="" >----Select----</option>');
            $('#to_fh_date').html('<option value="" >----Select----</option>');
            $.each(parsed,function(i,parsed)
            {
              ...
              {
                old_ex_date=parsed['exam_date'];
                $('#fh_date').append("<option value='"+parsed['exam_date']+"' >"+parsed['exam_date']+"</option>");
                $('#to_fh_date').append("<option value='"+parsed['exam_date']+"' >"+parsed['exam_date']+"</option>");
              }
...
$('#fh_date').on('change',function(){
    
    var date = $('#fh_date').val();    
    $.ajax({
        url: base_url+'?r=ajaxrequest/getqpsession',
        type:'POST',
        data:{date:date},
        success:function(data){               
            var parsed = $.parseJSON(data);
            $('#fh_session').html('');
            $('#fh_session').html("<option value=''>-----Select----</option>");
            $.each(parsed,function(i,parsed){
                $('#fh_session').append("<option value='"+parsed['coe_category_type_id']+"'>"+parsed['description']+"</option>");
            });
        }
    });
});

$('#fh_session').on('change',function(){

    var exam_year = $('#fh_year').val();
    var exam_month = $('#fh_month').val();
    var date = $('#fh_date').val();
    var session = $('#fh_session').val();
    $('#from').html('');
    $('#to').html('');
    $('#countFrom').val('');
    $('#countTo').val('');
    ...
    $.ajax({
            url: base_url+'?r=ajaxrequest/getfhseatcount',
            type:'POST',
            data:{date:date,session:session,exam_month:exam_month,exam_year:exam_year},
            success:function(data){                 
                var parsed = $.parseJSON(data); 
                $('#hall_count').val(parsed[0]['hall_count']);
              

                if(parsed[0]['hall_check']==0)
                {
                    
                   
                    $('#show_facultydetails').show();
                    $('#showdelete_fh').hide();
                    $('#show_fh').hide();
                    $('#delete_fh').hide();
                }
                else if(parsed[0]['hall_check']!=0)
                {

                  
                    $('#show_facultydetails').hide();
                    $('#showdelete_fh').show();
                    $('#show_fh').show();
                    $('#delete_fh').show();
                }
...
$('#fhboard').on('change',function(){

    var fhboard = $('#fhboard').val();
   $('#from').html('');
   
    $.ajax({
        url: base_url+'?r=ajaxrequest/getfaculty',
        type:'POST',
         data:{fhboard:fhboard},
        success:function(data)
        {
          var parsed = $.parseJSON(data);                  
              ...
              $.each(parsed,function(i,parsed){
                  ...
                  $('#from').append("<option value='"+parsed['coe_val_faculty_id']+"'>"+parsed['faculty_name']+"</option>");
     ...
     $('#aur').html('');
    $.ajax({
        url: base_url+'?r=ajaxrequest/getfaculty1',
        type:'POST',
        success:function(data)
        {
          var parsed = $.parseJSON(data);
              ...
              $.each(parsed,function(i,parsed){
                  $('#aur').append("<option value='"+parsed['coe_val_faculty_id']+"'>"+parsed['faculty_name']+"</option>");
              });
...
$('#delete_fh').on('click',function(){
    ...
    $.ajax({
            url: base_url+'?r=ajaxrequest/getfhdelete',
            type:'POST',
            data:{date:date,session:session,exam_month:exam_month,exam_year:exam_year},
            success:function(data){                 
                var parsed = $.parseJSON(data); 
                ...
                {
                    spinneroff();
                    krajeeDialog.alert('Faculty Hall Arranged Successfully Deleted');
                    $('#hall_count').val('');
                    $('#showdelete_fh').hide();
                    $.ajax({
                        url: base_url+'?r=ajaxrequest/getqpsession',
                ...
                {
                    krajeeDialog.alert('No Faculty Hall Arranged for Delete choosen date');
                    return true;
                }
                else
                {                   
...
function getValues1()
{
    var hallName1="";
  var tex1 = $( "#to" ).text();
  var count31 =  $('#to option').length;
  var x1=document.getElementById("to");
  for (var i = 0; i < x1.options.length; i++) 
  {
    hallName1 += x1.options[i].value+"&";      
  }
  document.getElementById("hallName").value=hallName1;

  var hallName="";
  var tex = $( "#to_rhs" ).text();
  var count3 =  $('#to_rhs option').length;
  var x=document.getElementById("to_rhs");
  for (var i = 0; i < x.options.length; i++) 
  {
    hallName += x.options[i].value+"&";      
  }
  document.getElementById("hallName_rhs").value=hallName;

}



function show_add(exam_year,exam_month_add,exam_date_add,exam_session,exam_type1)
{
    $.ajax({
        type:'POST',
        url: base_url+"?r=ajaxrequest/getaddconsolidateablistcover",
        data: {
            
            exam_year: exam_year,exam_month_add: exam_month_add,exam_date_add: exam_date_add,exam_session:exam_session,exam_type1:exam_type1,
...
function showboard1(exam_year,exam_month_add,exam_date_add,exam_type1,ans_pack_serial,exam_session)
{
    $.ajax({
        type:'POST',
        url: base_url+"?r=ajaxrequest/getaddconsolidateboard",
        data:{
            
            exam_year:exam_year,exam_month_add:exam_month_add,exam_date_add:exam_date_add,exam_type1:exam_type1,ans_pack_serial:ans_pack_serial,exam_session:exam_session
...
$('#answer_packets_btn1').on('click',function()
{
    var ans_pack_script=$('#ans_pack_script').val();
    ...
    {
        $.ajax({
            url: base_url+'?r=ajaxrequest/getanswerpacketsinfo3',
            type:'POST',
            data:{batch_id:$('#ans_batch_id').val(),exam_date:$('#ans_exam_date').val(), exam_year:$("#ans_exam_year").val(), exam_month:$("#ans_exam_month").val(),
            exam_session:$('#ans_exam_session').val(), ans_pack_script:$('#ans_pack_script').val(), ans_qp_code:$('#ans_qp_code').val(), ans_pack_serial:$('#ans_pack_serial').val(),exam_type:$('#exam_type').val()},
            success:function(data)
            {            
                spinneroff();
                  //var parsed = JSON.parse(data);
...
$('#answer_packets_btn_add').on('click',function()
{
    var ans_pack_script=$('#ans_pack_script').val();
    ...
    {
        $.ajax({
            url: base_url+'?r=ajaxrequest/getaddanswerpacketsinfo',
            type:'POST',
            data:{batch_id:$('#ans_batch_id').val(),exam_date:$('#ans_exam_date1').val(), exam_year:$("#ans_exam_year").val(), exam_month:$("#ans_exam_month1").val(),
            exam_session:$('#ans_exam_session1').val(), ans_pack_script:$('#ans_pack_script').val(), ans_qp_code:$('#ans_qp_code1').val(), ans_pack_serial:$('#ans_pack_serial').val(),exam_type:$('#exam_type').val()},
            success:function(data)
            {            
                spinneroff();
                  //var parsed = JSON.parse(data);
...
function showboardlist(exam_year,exam_month,exam_type,ans_pack_serial)
{
    $.ajax({
        type:'POST',
        url: base_url+"?r=ajaxrequest/getconsolidateboard1",
        data:{
            
            exam_year:exam_year,exam_month:exam_month,exam_type:exam_type,ans_pack_serial:ans_pack_serial,
...
function showboardadd1(exam_year,exam_month_add,ans_pack_serial)
{
    $.ajax({
        type:'POST',
        url: base_url+"?r=ajaxrequest/getaddconsolidateboard1",
        data:{
            
            exam_year:exam_year,exam_month_add:exam_month_add,ans_pack_serial:ans_pack_serial
...
function showexternal(exam_year,exam_month_add1,qp_code)
{
    $.ajax({
        type:'POST',
        url: base_url+"?r=ajaxrequest/getaddexternal",
        data:{
            
            exam_year:exam_year,exam_month_add1:exam_month_add1,qp_code:qp_code
...
function valuator_exam_month()
{
    ...
    $.ajax({
         type:'POST',
         url: base_url+"?r=ajaxrequest/getvaluatepacket",
       
        data:{year:year,month:month},
        success:function(data){       
            $('#v_val_faculty_all_id').html('');     
            var parsed = $.parseJSON(data);
            $('#v_val_faculty_all_id').html('<option value="" >----Select----</option>');
            
            $.each(parsed,function(i,parsed)
            {
              
                $('#v_val_faculty_all_id').append("<option value='"+parsed['val_faculty_all_id']+"' >"+parsed['subject_code']+"("+parsed['subject_pack_i']+")</option>");
...
function checkvaluateentry()
{

    var year = $('#exam_year').val();
    var month = $('#exam_month').val();
    var val_faculty_all_id = $('#v_val_faculty_all_id').val();
    $('#hide_bar_code_data1').hide();
    $('#valuator_entry').hide();
    $('#valuator_entry').html('');  
         ...
         $.ajax({
            ...
            type:'POST',
         url: base_url+"?r=ajaxrequest/checkvaluateentry",
       
        data:{year:year,month:month,val_faculty_all_id:val_faculty_all_id},
        success:function(data){                 
            var parsed = $.parseJSON(data);
            if(parsed[0]['result']==1)
            {
                $('#hide_bar_code_data1').hide();
                krajeeDialog.alert("Packet Already Completed! Please check");
                return false;
                  
            }
            else if(parsed[0]['result']==2)
            {
                krajeeDialog.alert("No Data Found! Please check");
                return false;
            }
            else
            {
                $('#hide_bar_code_data1').show();
                $('#valuator_entry').show();
                $('#valuator_entry').html(parsed[0]['bodydata']);  
            }
...
function checkvaluateentrynew()
{
    ...
    var date=$('#barcodeverify_scrutiny_date').val();
    var session=$('#barcodeverify_scrutiny_session').val();
    var val_faculty_all_id = $('#barcodeverify_val_faculty_all_id').val();
    ...
    $('#valuator_entry').html('');  
         ...
         $.ajax({
            ...
            type:'POST',
         url: base_url+"?r=ajaxrequest/checkvaluateentrynew",
       
        data:{year:year,month:month,date:date,session:session,val_faculty_all_id:val_faculty_all_id},
        success:function(data){                 
            var parsed = $.parseJSON(data);
            if(parsed[0]['result']==1)
            {
...
function checkvaldummymark()
{
    var valdummymark = $('#valdummymark').val();
   //alert(valdummymark);exit;

    if(valdummymark<0)
    {
        $('#savedummymark').hide();
        $('#valdummymark').val(0);
        krajeeDialog.alert("Minus mark Not allowed! Please check");
                return false;
    }
    else if(valdummymark>100)
    {
        $('#savedummymark').hide();
        $('#valdummymark').val(0);
        krajeeDialog.alert("Maximum mark 100! Please check");
                return false;
    }
    else
    {
...
function valuator_exam_month1()
{
    ...
    $.ajax({
           ...
           type:'POST',
           url: base_url+"?r=ajaxrequest/getvaluatepacket1",
          data:{year:year,month:month},
        success:function(data){       
            $('#v_val_faculty_all_id').html('');     
            var parsed = $.parseJSON(data);
...
function getvaluateentryreport()
{
    ...
    var val_faculty_all_id = $('#v_val_faculty_all_id').val();
     $('#verification_details').html('');
    $('#verification_details_pdf').hide(); 
    $('#savevaluatorentryid').hide();

    $.ajax({
         type:'POST',
        url: base_url+"?r=ajaxrequest/getvaluateentryreport",
        
        data:{year:year,month:month,val_faculty_all_id:val_faculty_all_id},
        success:function(data){                 
            var parsed = $.parseJSON(data);
            var jsonFormat = JSON.parse(data);
            if(jsonFormat[0]['vdata']=='')
            {
                krajeeDialog.alert('NO DATA FOUND KINDLY CHECK DATA');
                return false;
            }
...
function getvaluatorentryupdate(exam_year,exam_month,dum_num)
{
    
    $('#valuator_entry').hide();
    $('#savevaluatorentryid').hide();
    if(dum_num!='' && dum_num!=null)
    {
        $.ajax({

        url: base_url+"?r=ajaxrequest/getvaluatorentryupdate",
        type:'POST',
        data:{dum_num:dum_num},
        success:function(data)
        {
            var jsonFormat = JSON.parse(data);          
            if(jsonFormat[0]['result']==0)
            {
                krajeeDialog.alert('Not Found Please Check');
                return false;
            }
            else
            {
                $('#valuator_entrymark').val(jsonFormat[0]['mark']);
                $('#valuator_entry').show();
                $('#savevaluatorentryid').show();
                return true;
            }
        }
        });    
    ...
    function savevaluatorentryupdate()
   {
    
    var dummynumbers=$('#dummy_number').val();
    var mark=$('#valuator_entrymark').val();

    if(mark>100)
    {
        krajeeDialog.alert("Maximum Mark 100");
        return false;
    }
    else if(mark=='')
    {
        krajeeDialog.alert("Please Enter Mark");
        return false;
    }
    else
    {
        $.ajax({
               ...
               url: base_url+"?r=ajaxrequest/savevaluatorentryupdate",
              type:'POST',
              data:{mark:mark,dummynumbers:dummynumbers},
              success:function(data){       
                var parsed = $.parseJSON(data);
                if(parsed==1)
                {
                    $('#valuator_entrymark').val('');
                    $('#dummy_number').val('');
                    $('#valuator_entry').hide();
                    $('#savevaluatorentryid').hide();
                    krajeeDialog.alert("Marks updated Successfully");
                     return false;
                }
                else if(parsed==2)
                {
                    krajeeDialog.alert("Marks Not updated Please Check");
                    return false;
                }
                else if(parsed==0)
                {
...
function getpartatotal211(stuloopid) 
{
    var regno1='#regtot'+stuloopid;
    //console.log(regno1);
    $(regno1).val('');
    var grandtotal=0;
    var part_subtotal=0;
    ...
    for(i=1;i<=10;i++)
    {
       var parta='#parta'+stuloopid+i;
       var partaval=$(parta).val();
        if(partaval>1)
        {
            $('#part_a_'+i).val(0);
            krajeeDialog.alert("Part A  Maximum mark 1 Please Check");
            return false;
        }
    }
    ...
    for(i=11;i<=15;i++)
    {
       ...
       var partaval=$(parta).val();
        ...
        {
            $('#part_a_'+i).val(0);
            krajeeDialog.alert("Part B  Maximum mark 3 Please Check");
            return false;
        }
    }
    ...
    for(i=16;i<=20;i++)
    {
       ...
       var partaval=$(parta).val();
        ...
        {
            $('#part_a_'+i).val(0);
            krajeeDialog.alert("Part C Maximum mark 5 Please Check");
            return false;
        }
    }
    for(i=1;i<=20;i++)
    {
        ...
        //console.log('partaval'+temp);
        part_subtotal=parseInt(part_subtotal)+parseInt(temp);
        $(regno1).val(part_subtotal);
...
//console.log(stuloopid+'part_subtotal: '+part_subtotal);
      
}

function gettotal(stuloopid) 
{
    
    //console.log(regno1);
     var total='#regtot'+stuloopid;
      var total1=$(total).val();
      

    
     var mark=$('#mark_100').val();
if(total1==mark)
{
...
else
{
    ...
    var total1=$(total).val();
    krajeeDialog.alert("Please check the valuator marks");
    return false;
